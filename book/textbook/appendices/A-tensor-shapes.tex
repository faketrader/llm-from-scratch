...
\begin{document}
\fi
\bookappendix{张量运算速查}{app:tensor-shapes}{1}

本附录集中列出正文中反复出现的张量形状。它用于核对轴的语义、矩阵乘法的公共轴、广播方向以及重排前后的元素对应关系；机制推导见\bookxref{ch:tensors}、\bookxref{ch:attention}和\bookxref{ch:transformer}。除非局部另行说明，序列张量采用 $[B,T,d]$，图像张量采用 $[B,C,H_{\mathrm{img}},W_{\mathrm{img}}]$，方括号按轴顺序给出逻辑形状。
...
张量形状由各轴的长度及其语义共同确定。矩阵乘法要求被归约的公共轴相容，广播规定参数沿哪些轴共享，转置与重排则改变元素的索引关系。重塑必须保持元素总数，同时满足批量、位置、头和特征等轴的对应关系。
...
词嵌入查表 & 编号 $[B,T]$，嵌入表 $[V,d]$ & $[B,T,d]$；按词元编号读取对应的嵌入行。\\
...
每个门控专家把 $[n_e,d]$ 映射为 $[n_e,d_f]$ 再返回 $[n_e,d]$，其中 $n_e$ 是该专家实际接收的条目数。专家输出按原词元索引聚合回 $[B,T,d]$。只检查最终形状无法发现词元排列错误，还要验证分派索引、专家权重和逆置换彼此一致。容量与路由机制见\bookxref{ch:moe}。
...
\begin{enumerate}
 \item 轴的名称与尺寸共同标识其含义，需区分批量、位置、头、专家与特征。
 ...
 \item 残差、损失和逐元素门控要求参与运算的元素在位置与语义上一一对应。
\end{enumerate}
