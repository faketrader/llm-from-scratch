...
\begin{document}
\fi
\bookappendix{参数量、显存与算力估算}{app:resource-estimation}{2}

本附录给出建立资源预算时可直接套用的公式和记录表。所有结果都应先声明模型结构、执行阶段、数值格式、批量与长度分布，再区分理论元素数、运行时峰值和实测值。公式的推导及系统边界见\bookxref{ch:resource-budget}，硬件供给、并行切分和服务测量分别见\bookxref{ch:compute-infrastructure}、\bookxref{ch:distributed-training}与\bookxref{ch:serving-architecture}。

\section{先统一计量口径}

\begin{table}[htbp]
\centering
\caption{资源估算中必须分开的计量对象}\label{tab:appendix-resource-units}
...
参数核算应逐项回答以下问题：输入输出是否共享；前馈是两矩阵还是门控三矩阵；键值头数是多少；是否含偏置、查询键归一化、位置表、专家、适配器或任务头；参数是否跨层共享。MoE 中总专家参数按专家总数 $E$ 计算，本次执行的专家矩阵主要按每词元激活数 $k$ 计算。
...
\begin{table}[htbp]
\centering
\caption{一种混合精度 Adam 配置的每参数状态示例}\label{tab:appendix-adam-bytes}
\begin{tabular}{lcc}
\toprule
...
\end{equation}
建立线性基线。系数 $c_a$ 表示每层每位置实际保存的隐藏宽度等效元素数，必须根据模型结构、融合算子、重计算策略和实测峰值校准。若显式保存注意力概率，还可能出现 $O(LBH_qT^2)$ 元素；分块注意力和激活重计算会改变保存集合。
...
\end{equation}
推理峰值则通常包含权重、KV 缓存、当前批次激活、采样或束搜索状态、工作区和运行时保留。各项是否同时处于峰值应由执行时间线确认；保守初算可以先相加，测量后再收紧。
...
最终预算至少给出三个结果：容量是否可行、期限是否可行、哪些系数仍需在目标硬件上测量。参数量只能回答模型中有多少独立标量；显存、计算与时间必须沿实际执行路径分别建立模型。
