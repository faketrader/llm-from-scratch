...
\noindent\textsf{\bookedition\quad 版本 \bookversion}\par\medskip

\optional 本书用外侧页边的危险弯道标记标示可暂时跳过的扩展内容。未标记的正文属于所在章节的学习主线，包括核心概念、基本机制、必要推导与基础算例；其中的长推导仍承担后续概念所需的论证。
...
图标表示学习路径上的可延后性，与内容难度没有固定对应关系。扩展内容应能够从主线中独立移开，后续主线论述不得依赖其中尚未交代的定义或结论。危险弯道图标采用单色形式，黑白打印不影响辨认。

建议先完成共同基础，再沿所选方向阅读相关章节，结合问题回读带图标的部分。遇到不熟悉的符号可查书前符号约定，术语可查书末中文术语索引或英文术语索引；实现中需要反复核对的形状与资源公式，可分别查\bookappendixref{app:tensor-shapes}与\bookappendixref{app:resource-estimation}。
...
共同基础以理解一次模型请求为目标。先从\bookxrefshort{ch:introduction}建立模型、训练和系统的职责，再从\bookxrefshort{ch:probability}和\bookxrefshort{ch:tensors}掌握条件概率、交叉熵的含义、张量形状、梯度更新与数据划分。随后依次选读\bookxrefshort{ch:tokenization}的分词与嵌入，\bookxrefshort{ch:attention}的查询、键、值与因果掩码，\bookxrefshort{ch:transformer}的整体结构与前向数据流，以及\bookxrefshort{ch:position-representation}的顺序信息。\bookxrefshort{ch:autoregressive}补齐下一词元分布、采样、终止及事实性边界。
...
\subsection*{模型训练方法}
专项前置是完整的概率、自动微分与注意力反向传播，以及 Transformer 和自回归训练目标。先阅读\bookxrefshort{ch:optimization-generalization}，掌握优化方法与泛化分析。入门依次阅读\bookxrefshort{ch:data-engineering}、\bookxrefshort{ch:resource-budget}、\bookxrefshort{ch:training-stability}、\bookxrefshort{ch:supervised-finetuning}、\bookxrefshort{ch:parameter-efficient-finetuning}和\bookxrefshort{ch:efficient-finetuning}，建立数据、监督信号、可训练参数、资源与恢复之间的关系。学习终点是能设计适配方案，并用独立测试集检验目标能力、遗忘与失败切片。
...
\bookxrefshort{ch:mlm}、\bookxrefshort{ch:moe}、\bookxrefshort{ch:long-context}和\bookxrefshort{ch:open-models}按模型与任务选择。视觉理解沿\bookxrefshort{ch:vision-transformer}进入\bookxrefshort{ch:multimodal-alignment}；视觉生成沿\bookxrefshort{ch:diffusion-probability}进入\bookxrefshort{ch:conditional-vision}，并补齐概率与视觉架构前置。涉及具体模型时，先掌握其使用的机制，再分析配置与技术报告。

路线中的主题选读表示当前学习目标所需的范围，章内危险弯道表示相对于该章主线可延后的内容。两者分别服务于方向选择和章内阅读；深入一个主题时，仍需完整掌握其依赖的概念与推导。

\section*{先修基础与推荐书目}
本书面向能够熟练阅读 Python 代码，并具备多元微积分、线性代数、概率统计与经典神经网络基础的读者。全书第一篇恢复后续推导所需的数学与张量语言，重点聚焦于大语言模型特有的条件概率分解、自注意力机制、因果生成以及评测与安全规范。

对于需要系统巩固统计学习推导或通用深度学习实现的读者，推荐结合以下两部经典著作进行先修储备或同步查阅：
\begin{itemize}
  \item \textbf{数理基础与经典统计学习}：李航《统计学习方法》（第2版，清华大学出版社）。该书系统推导监督学习与无监督学习的核心模型，包括概率图模型、极大似然估计、感知机、支持向量机、EM 算法、隐马尔可夫模型及矩阵分解。对于深入理解本书中的概率建模假设、优化目标形式、自监督学习损失与信息论基础具有直接的数理支撑作用。
  \item \textbf{深度学习框架与动手实现}：Aston Zhang、Zachary C. Lipton、Mu Li（李沐）、Alexander J. Smola 等《动手学深度学习》（Dive into Deep Learning，中文版由人民邮电出版社出版）。该书结合数学原理与可执行代码，系统讲授张量运算、自动求导、多层感知机、卷积神经网络、循环神经网络及基础注意力机制。对于建立神经网络前向计算与反向传播的数值直觉、熟练掌握张量编程，以及完成本书中的机制实现与库迁移提供了坚实的实践基石。
\end{itemize}

两部参考著作系统覆盖了经典统计学习与通用深度学习的基础全貌；本书则在此基础上展开大语言模型从序列表示、自注意力机制、预训练与微调、对齐推理到智能体与系统工程的完整体系。读者可根据自身基础查漏补缺，建立从经典理论到大模型系统工程的连续认知。

\section*{内容体系}
正文解释定义、假设、推导和系统关系，例题用明确的数据与中间量展示计算。

例如，公式推导可以证明某种归一化具有行和为一的性质；小矩阵例题可以逐项展示其计算过程；程序对照可以检查特定实现是否符合该定义。领域答案正确率还取决于任务数据与评价标准。理论性质、实现正确性与任务效果分别采用对应的评价方法。

本书按照概念依赖组织章节，核心定义与必要推导均在正文展开。涉及实验性结论时，应明确对象、数据、训练预算与测量条件，避免把局部观察推广为普遍规律。

本书的“从零”指从基本表示和运算理解机制，并据此形成独立实现与判断的能力。驱动、编译器和基础数值库作为既有计算基础使用。完成原理验证后，使用成熟实现仍需核对其默认值、数值约定和边界条件。\footnote{同一个数学算子可以有不同的存储布局、融合策略和求和顺序；实数算术中的等价不必表现为浮点结果逐位相同。}
