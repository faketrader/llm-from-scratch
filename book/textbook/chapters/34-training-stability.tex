...
\begin{equation}
 ...
 \qquad \widehat{\vect{m}}_t=\frac{\vect{m}_t}{1-\beta_1^t}.
...
\end{align}
$\vect{v}_t$ 估计二阶\emph{原点矩}，而非减去均值平方的方差。分母按照各坐标过去的梯度尺度调节步长。它是对角形式的预条件更新，不含坐标之间的曲率耦合，与逆 Hessian 方法分属两类。
...
\end{equation}
当 $|g|\gg\epsilon$ 时，其幅度接近学习率，方向与梯度相反。这说明 Adam 的更新幅度由原始梯度范数推断不出来；当历史梯度方向反转时，一阶矩还可能暂时保留旧方向。
...
\term{学习率预热}{Learning Rate Warmup}{}在训练初期逐步增大学习率；\term{余弦衰减}{Cosine Decay}{}在后续阶段平滑减小学习率。它们规定更新尺度随训练进度的变化，与错误标签、数值溢出或损失归一化错误属于不同层面的问题。
...
读取一个微批次、尝试一次更新、成功更新参数和消费一批有效词元是不同事件。调度依据成功更新次数时，数值溢出导致的跳步不推进日程；依据消费词元数时，跳步是否消费数据以及日程如何推进必须另行定义。记录一个含义不明的“step”恢复不出训练轨迹。
\end{warning}

当批量或有效序列长度随时间变化时，按词元数调度可以直接对应数据预算，与按步调度产生的参数轨迹则可能不同。改变总预算 $S$ 后重新构造余弦曲线，也会改变恢复点之后的学习率；这属于更改训练方案，应保存新的日程版本。

\section{有限精度、损失缩放与梯度裁剪}
...
保存随机种子与保存随机数生成器的当前位置是两件事。种子只给出起点；训练中 Dropout、采样和数据增强已经消耗了许多随机数。恢复时还应避免模型初始化或数据预取在载入随机状态后意外消费它们。

\textbf{语义续训}保留目标、资产和必要状态，允许数值误差带来的轨迹差异；\textbf{数值接近的续训}进一步要求相同运算组织并给定误差容限；\textbf{逐位一致的续训}要求算子、归约顺序、并发及随机行为均满足确定性条件。完整状态是精确恢复的必要条件，它本身并不保证不同设备和软件版本间逐位相同。\footnote{浮点加法不满足严格结合律。即使输入相同，改变并行归约树也可能改变末位结果；训练迭代可能逐步放大这种差异。}

\subsection{一致性边界与原子发布}
...
发布过程可采用“写分片、验证清单、提交代号”的顺序。分片写入唯一代号的暂存位置，记录大小与摘要；全部必需分片持久化后，提交包含资产版本和状态边界的清单。读取者只接受已经提交且清单完整的代。文件系统重命名和对象存储的发布机制具有不同语义，应由存储层提供明确保证，“文件存在”与“检查点完整”并不是同一件事。
...
也可以选择在溢出后重放同一批次，但必须恢复相应随机状态、数据游标以及前向可能修改的缓冲区，并限制重试次数。改变这一策略会改变数据消费轨迹，恢复时隐式切换会使前后两段不可比。
...
\end{equation}
参数接近零时该比值敏感，应同时报告绝对量。损失尖峰若伴随词元数锐减，先检查归一化和数据；若伴随非有限激活，定位首次异常算子；若梯度有限但更新异常，则检查矩状态、稳定项和学习率。只看经过裁剪后的范数会掩盖持续发生的梯度异常。图\ref{fig:loss-spike-anatomy}给出了典型损失尖峰的演进轨迹与自愈处置决策流程，表\ref{tab:rev-training-instability-taxonomy}梳理了预训练常见数值异常的诊断应对清单。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth]{\subfix{../../figures/theory/loss-spike-anatomy.pdf}}
\caption{训练损失尖峰（Loss Spike）演进轨迹与自愈机制决策树。监测到损失与梯度突增时，依次检查非有限值、触发跳步并动态收缩损失缩放因子；连续不可恢复时执行检查点回退与污染数据跳过。}\label{fig:loss-spike-anatomy}
\end{figure}

\begin{table}[htbp]
\centering
\caption{大语言模型预训练常见训练异常表象、根因分析与工程应对措施}\label{tab:rev-training-instability-taxonomy}
...
更新停滞 (Stagnation) & 损失基本不降，参数相对更新范数趋零 & 学习率衰减过快、过大权重衰减或梯度消失 & 检查学习率日程底线 $\eta_{\min}$，调整 AdamW 衰减分组与豁免名单 \\
\bottomrule
\end{tabularx}
\end{table}
