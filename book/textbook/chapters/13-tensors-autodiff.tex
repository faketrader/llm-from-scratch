...
\State Execute the forward pass in topological order, save local derivative state, and obtain scalar loss $\mathcal L$
\State Set every adjoint to zero and set $\overline{\mathcal L}\gets1$
\For{each node $\vect{v}=f(\vect{u}_1,\ldots,\vect{u}_k)$ in reverse topological order}
  \For{each input edge $(\vect{u}_i,\vect{v})$, including repeated edges from the same node}
    \State $\overline{\vect{u}}_i\gets\overline{\vect{u}}_i+\mat{J}_{f,u_i}^{\mathrm{T}}\overline{\vect{v}}$
  \EndFor
\EndFor
\State Allow the optimizer to update parameters only after the entire reverse pass is complete
\State \Return the adjoints of all trainable parameter nodes
\end{myalgorithmic}
