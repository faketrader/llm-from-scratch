...
\begin{aligned}
 ...
 \sigma(z) &= \frac{1}{1+\conste{}^{-z}}, \\
 \tanh(z) &= \frac{\conste{}^z-\conste{}^{-z}}{\conste{}^z+\conste{}^{-z}}.
\end{aligned}
\end{equation}
ReLU 计算简单，正区间梯度不衰减；Sigmoid 将数值压到 $(0,1)$，适合表示二元概率；$\tanh$ 输出以零为中心。Sigmoid 和 $\tanh$ 在绝对值较大时导数接近零，深层或长序列中容易出现梯度消失。图\ref{fig:activation-functions-curve}给出了常见激活函数及其一阶导数的几何形态，表\ref{tab:activation-functions}总结了其数学性质与工程特点。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{\subfix{../../figures/theory/activation-functions-curve.pdf}}
\caption[常见激活函数及其导数曲线]{常见激活函数及其导数曲线。Sigmoid 与 Tanh 存在饱和区与导数极小值；ReLU 在正半轴梯度恒定；GELU 与 SiLU 具备平滑非单调性，在大输入时接近 ReLU，在小负区间提供平滑负反馈。}\label{fig:activation-functions-curve}
\end{figure}

\begin{table}[htbp]
\centering
\caption{常见神经网络激活函数特性比较}\label{tab:activation-functions}
...
\begin{equation}
\begin{aligned}
 q_j &= \frac{\conste{}^{u_j}}{\sum_{k=1}^{C}\conste{}^{u_k}}, \\
...
本章主要讨论二维批处理，并使用表\ref{tab:tensor-notations}所列符号。单个向量在定义梯度和雅可比时视为列向量；批量矩阵中每个样本按行存放。两种约定之间通过转置明确转换。
\begin{table}[htbp]
\centering
\caption{张量运算与反向传播数学符号}\label{tab:tensor-notations}
...
\begin{equation}
\begin{aligned}
 \mat{Z} &= \mat{X}\mat{W}+\mathbf1_B\vect{b}^{\mathrm{T}}, \\
...
\caption[张量存储、跨步视图与内存重排关系]{张量存储、跨步视图与内存重排关系。逻辑转置仅修改步长而不搬运内存，后续操作若依赖连续内存则触发显式拷贝重整。}\label{fig:tensor-memory-layout}
\end{figure}
...
\end{align}
与梯度的定义逐项比较，得到
\begin{equation}\label{eq:tensor-linear-backward}
 \boxed{
 \begin{aligned}
  \overline{\mat{X}} &= \mat{G}\mat{W}^{\mathrm{T}}, \\
  \overline{\mat{W}} &= \mat{X}^{\mathrm{T}}\mat{G}, \\
...
仍可合并为一个仿射映射。加入非线性后，不同输入区域可具有不同的有效映射，两层网络因而能够表达单个仿射映射无法表示的函数。反向传播需要乘以依赖前向状态的局部导数，因此必须保存或重新计算该状态。
...
\begin{equation}\label{eq:tensor-vjp}
\begin{aligned}
 \dif\mathcal L &= \overline{\vect{z}}^{\mathrm{T}}\mat{J}_f\dif \vect{x}, \\
 \overline{\vect{x}} &= \mat{J}_f^{\mathrm{T}}\overline{\vect{z}}.
\end{aligned}
\end{equation}
...
一个变量若经由两条路径影响损失，\emph{两条路径的贡献必须相加}。考虑 $a=x^2$、$b=3x$、$\mathcal L=ab$。前向在 $x=2$ 时得到 $a=4$、$b=6$、$\mathcal L=24$；反向先得到 $\overline a=b=6$、$\overline b=a=4$，再得到
\begin{equation}
...
\end{tikzpicture}
\caption[共享输入计算图]{共享输入计算图。实线表示前向依赖，虚线表示从乘法节点返回的梯度。}\label{fig:autodiff-shared-input}
\end{figure}

\subsection{正向模式和反向模式}
自动微分按照基本运算的导数规则，对实际执行的组合函数计算导数。它不把表达式化简成一个符号公式，也不用微小扰动近似导数，因此除基本运算与浮点计算本身的误差外，不引入有限差分那样的步长截断误差。

正向模式给定输入方向 $v$，随前向传播计算 $\mat{J}_fv$；反向模式给定输出权重 $w$，从后向前计算 $\mat{J}_f^{\mathrm{T}}w$。如果一个函数具有大量参数而只有一个标量损失，反向模式以输出端种子 $\overline{\mathcal L}=1$ 开始，\emph{一次反向遍历}即可得到所有参数的梯度。逐坐标使用正向模式则通常需要多次遍历。因此，参数量远大于标量损失的深度学习场景在本质上决定了\emph{反向自动微分}的核心地位。若输入方向很少、需要多个输出的方向响应，正向模式也有价值。两种模式的计算特性如表\ref{tab:autodiff-modes}所示。

\begin{table}[htbp]
\centering
\caption{正向与反向自动微分特性对比}\label{tab:autodiff-modes}
...
反向模式 (Reverse) & 向量--雅可比积（VJP） $\vect{w}^{\mathrm{T}}\mat{J}_{\vect{f}}$ & $\mathcal{O}(n_{\text{out}})$ 次反向遍历 & 标量或低维损失目标（$n_{\text{out}} \ll n_{\text{in}}$），为深度学习反向传播的数学基础；需要缓存前向激活 \\
\bottomrule
\end{tabularx}
\end{table}
...
在一次前向和反向之间，参数以及反向所需的中间量必须保持一致。若计算 $\overline{\mat{W}}_2$ 后立刻更新 $\mat{W}_2$，再使用更新后的 $\mat{W}_2$ 计算隐藏层梯度，那么该梯度已经不再对应原来的前向函数。正确流程是先完成当前损失的全部反向传播，再由优化器统一更新参数。

\begin{warning}[梯度计算与参数更新的时序隔离]
在反向传播完成前，严禁对正在计算梯度的参数执行原地更新；过早修改参数会破坏计算图中尚未反向遍历的上游节点状态。正确流程始终遵循：全图反向传播累积完毕后，优化器统一执行单步参数更新并妥善管理梯度缓冲区。
\end{warning}
...
计算图截断（如 \texttt{detach()}）保留前向激活值但阻断反向求导链。离散索引、阈值判断与采样操作需要借助重参数化或直通估计器（Straight-Through Estimator）处理。前向运行模式与梯度追踪机制在系统实现中解耦：Dropout 等随机丢弃算子在训练模式下改变前向输出，而梯度追踪开关仅控制是否构建计算图并保存反向依赖。在关闭梯度记录时，模型仍可能带有采样随机性；评估阶段的确定性输出依赖显式切换至评估模式。

\begin{algorithm}[标量损失的反向自动微分]
\label{alg:tensor-reverse}
\AlgoInput{有限无环计算图、输入与固定参数，以及每个节点的前向操作和局部反向规则。}
\AlgoOutput{损失对所有可训练参数的梯度。}
\begin{myalgorithmic}[1]
\State 按拓扑序执行前向传播，保存局部导数状态，获得标量损失 $\mathcal L$
\State 将所有伴随变量置为零，并令 $\overline{\mathcal L}\gets1$
\For{each 逆拓扑序节点 $\vect{v}=f(\vect{u}_1,\ldots,\vect{u}_k)$}
  \For{each 输入边 $(\vect{u}_i,\vect{v})$（含来自同一节点的重复边）}
    \State $\overline{\vect{u}}_i\gets\overline{\vect{u}}_i+\mat{J}_{f,u_i}^{\mathrm{T}}\overline{\vect{v}}$
  \EndFor
\EndFor
\State 仅在完整反向传播结束后允许优化器更新参数
\State \Return 所有可训练参数节点的伴随变量
\end{myalgorithmic}
...
\begin{equation}
\begin{aligned}
 q_j &= \frac{\exp(u_j)}{\mat{S}}, \\
 \mat{S} &= \sum_{k=1}^{C}\exp(u_k).
\end{aligned}
\end{equation}
...
模型输出未归一化分数后，应直接传递给内置数值稳定变换的损失算子。若先显式执行 Sigmoid 映射，再将所得概率作为未归一化分数传入包含对数稳定变换的二元损失，将导致双重非线性复合并引发严重的梯度饱和与数值错误。输入张量的数学语义取决于函数定义，而非代码中的变量命名。

\begin{example}
考虑两层感知机（Multilayer Perceptron，MLP）：
\begin{align*}
 \mat{Z} &= \mat{X}\mat{W}_1+\mathbf1_B\vect{b}_1^{\mathrm{T}}, & \mat{A} &= \operatorname{ReLU}(\mat{Z}), \\
 \mat{U} &= \mat{A}\mat{W}_2+\mathbf1_B\vect{b}_2^{\mathrm{T}}, & \mat{Q} &= \softmax(\mat{U}).
\end{align*}
取 $B=d=h=C=2$，给定
\begin{equation*}
...
\begin{align*}
 ...
 \mat{A}&=\begin{bmatrix}3&1\\0&3\end{bmatrix},\\
...
\end{tikzpicture}
\caption[两层网络前向与反向计算路径]{两层网络前向与反向计算路径。图中 $\mat{G}=\overline{\mat{U}}$，$\mat{D}=\overline{\mat{Z}}$；各仿射层还分别产生权重与偏置梯度。}\label{fig:math-two-layer-autodiff}
\end{figure}
...
\end{align*}
输入梯度不参与本层参数更新，它描述当前损失对输入特征的局部敏感度。若 $\mat{X}$ 来自更早的可训练模块，这个梯度正是该模块收到的反向传播信号。

以 $\mat{W}_{1,11}$ 为例，其梯度为 $-\frac{a}{2}\approx-0.059601$。仅对这个坐标施加正扰动 $\delta$ 时，一阶损失变化约为 $-0.059601\delta$。如果所有参数沿负梯度作足够小的共同更新，则一阶变化为 $-\eta\|\nabla_{\vect{\theta}}\mathcal L\|^2$。这属于可微邻域内的局部线性近似；步长过大可能跨越激活折点或引发高阶项发散。
...
\begin{equation}
\begin{aligned}
 \overline b_j &= \sum_i\overline Z_{ij}, \\
...
\end{equation}
反向传播完成后，偏置梯度通过沿广播轴求和恢复原有一维形状，与偏置张量的物理参数维度严格对齐。
...
\begin{equation}
\begin{aligned}
 \mathcal L &= \frac1N\sum_{it}M_{it}\ell_{it}, \\
...
\begin{equation}\label{eq:tensor-embedding}
\begin{aligned}
 \overline{\mat{E}} &= \mat{S}^{\mathrm{T}}\overline H, \\
...
\subsection{梯度校验的适用条件}
梯度校验应固定输入、标签、参数及随机选择，并避免扰动跨越 ReLU 折点或离散分支。原点本身位于不可微处时，中心差分可能给出左右变化率的平均值，与实现选定的次梯度不一致。随机失活若在两次求值时使用不同掩码，差商还会混入函数本身变化造成的噪声。

梯度校验检查局部导数与计算图的一致性，轴的含义、标签和损失定义则需要结合任务核对。例如，若错误地沿样本轴执行 Softmax，数值梯度校验仍能自洽通过，但模型实际计算的是跨样本归一化而非单个样本内的类别概率分布。校验流程应先确认任务目标与张量轴的语义绑定，再验证闭式分量公式与有限差商的一致性。
