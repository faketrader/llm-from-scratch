...
反向传播计算损失对参数的梯度，优化算法据此确定参数的更新方向和幅度。学习率不合适可能使训练振荡，初始化尺度不当可能使信号衰减；训练误差持续降低时，对新样本的预测也可能变差。优化研究如何降低既定目标，泛化研究从有限数据学到的预测规律能否适用于未参与拟合的数据。
...
\end{align}
总体风险 $R_P$ 是希望降低的\emph{期望统计量}，经验风险 $\widehat R_S$ 是有限样本上的\emph{经验估计}，正则化目标 $F_S$ 是实际交给优化算法的\emph{代理函数}。三者可能具有不同的最小点。正则化项 $\Omega$ 不进入测试误差，它通过改变训练时偏好的解，间接影响总体风险。
...
\begin{equation}\label{eq:opt-spectral}
\begin{aligned}
 a_{k+1,j} &= (1-\eta\lambda_j)a_{k,j}, \\
 a_{k,j} &= (1-\eta\lambda_j)^k a_{0,j}.
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
 \eta_{\mathrm{bal}} &= \frac2{L+\mu}, \\
 \rho(\eta_{\mathrm{bal}}) &= \frac{L-\mu}{L+\mu} = \frac{\kappa-1}{\kappa+1}, \\
 \kappa &= \frac L\mu.
\end{aligned}
\end{equation}
...
\begin{example}[学习率与下降、振荡、发散]
取 $F(u,v)=\frac{u^2+9v^2}{2}$，初值 $(u_0,v_0)=(1,1)$。此时 $L=9$、$\mu=1$，稳定区间为 $0<\eta<\frac{2}{9}$。由式\eqref{eq:opt-spectral}直接计算得到表\ref{tab:opt-quadratic-steps}。
\begin{table}[htbp]
\centering
\caption{二次目标梯度下降在不同步长下的前两步演化}
\label{tab:opt-quadratic-steps}
\begin{tabular}{ccccc}
\toprule
...
\end{tabular}
\end{table}
初始目标值为 $5$。第二行虽然发生变号，仍稳定下降；第三行在陡峭方向的误差放大因子为 $-1.25$，因而发散。参数或梯度发生正负交替时，收敛性取决于误差幅度是否衰减；学习率过大可使误差逐步放大。演化轨迹如图\ref{fig:opt-stable-unstable}所示。
...
\end{tikzpicture}
\caption[二次目标陡峭方向收敛轨迹]{二次目标陡峭方向收敛轨迹。实线表示振荡收敛，虚线表示振荡发散。点由 $v_k=(1-9\eta)^k$ 精确计算，连线仅用于识别离散更新顺序。}\label{fig:opt-stable-unstable}
\end{figure}
\end{example}
...
\begin{equation}
\begin{aligned}
 \vect{g}(\vect{\theta}) &= \frac1B\sum_{b=1}^B \vect{h}_{I_b}(\vect{\theta}), \\
...
\end{equation}
有放回独立抽样使交叉协方差为零，所以
\begin{equation}\label{eq:opt-sgd-var}
\begin{aligned}
 \operatorname{Cov}(\vect{g}\mid S,\vect{\theta}) &= \frac{\mat{\Sigma}}{B}, \\
 \mathbb E\|\vect{g}-\bar{\vect{h}}\|_2^2 &= \frac{\operatorname{tr}\mat{\Sigma}}{B}.
\end{aligned}
\end{equation}
...
动量通过累计过去梯度减弱方向的短时变化，自适应优化器进一步按坐标调节更新尺度。它们改变上述标量学习率的动力学，$\frac{2}{L}$ 不再构成完整稳定界。本章建立的目标、条件期望与曲率分析仍是解释其行为的基础；主流优化算法更新机制与状态显存开销如表\ref{tab:opt-algorithms}所示，Adam、AdamW 的矩估计、偏差修正和恢复状态在训练过程一章推导。

\begin{table}[htbp]
\centering
\caption{主流深度学习优化算法对比}
\label{tab:opt-algorithms}
...
\State 校验正整数 $r, M$ 与非负整数 $K$
\State 初始化 $\vect{\theta}_0$，并在固定评估模式下计算初始验证损失 $v_0$
\State 保存 $\vect{\theta}_0$；令 $v_{\min}\gets v_0$，$v_{\mathrm{pat}}\gets v_0$，$h\gets0$
\For{$k=0,\ldots,K-1$}
  \State 按指定规则采样训练小批量，并计算 $\vect{\theta}_k$ 处的平均数据梯度 $\vect{g}_k$
  \State $\vect{\theta}_{k+1}\gets\vect{\theta}_k-\eta_k(\vect{g}_k+\lambda\vect{\theta}_k)$，正则化项仅计入一次
  \If{$k+1$ 是 $r$ 的倍数，或 $k+1=K$}
    \State 在固定协议下计算验证数据损失 $v$，不执行反向传播且不更新预处理状态
    \If{$v<v_{\min}$}
      \State 保存 $\vect{\theta}_{k+1}$ 并令 $v_{\min}\gets v$
    \EndIf
    \If{$v_{\mathrm{pat}}-v>\delta_{\min}$}
      \State $v_{\mathrm{pat}}\gets v$ 且 $h\gets0$
    \Else
      \State $h\gets h+1$
    \EndIf
    \If{$h\ge M$}
  ...
  \EndIf
\EndFor
\State \Return 保存的最佳参数；仅在配置冻结后使用留出测试集
\end{myalgorithmic}
批量损失的求导遵循\bookxref{ch:tensors}[中的反向传播规则]。恢复后继续训练还必须保存随机状态、数据位置和优化器状态，仅保存用于预测的参数并不足以续训。
\end{algorithm}
...
\begin{equation}\label{eq:opt-forward-moment}
\begin{aligned}
 \mathbb E[z_j] &= 0, \\
 \mathbb E[z_j^2] &= \sum_i\mathbb E[W_{ij}^2]\mathbb E[a_i^2] = n_{\mathrm{in}}s_W^2q_a.
\end{aligned}
\end{equation}
...
\begin{aligned}
 ...
 W_{ij} &\sim \mathcal N\!\left(0,\frac2{n_{\mathrm{in}}}\right),
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
 \mathbb E[\operatorname{ReLU}(z)] &= \sqrt{\frac v{2\constpi{}}}, \\
...
残差连接、动态自注意力权重、门控前馈乘法与层归一化共同重塑了深度网络的方差传播规律。网络初始化需基于整体拓扑推导，并在前向与反向传播中监测各子层的激活尺度与梯度范数，避免全局范数平抑局部层异常。
...
\end{equation}
在这个特定更新规则中，目标中的二次惩罚等价于每步乘性权重衰减。若先用一个自适应对角矩阵 $\mat{D}_k$ 缩放梯度，则惩罚项变为 $\lambda \mat{D}_k\vect{\theta}_k$，与统一的 $\lambda\vect{\theta}_k$ 衰减取值不同。\emph{二次正则化}与\emph{解耦权重衰减}的区别由此产生。

\keyconcept{在 Adam 等自适应学习率优化器中，经典 $L_2$ 正则化会与历史梯度的二阶矩缩放发生耦合，破坏正则强度的恒定缩放；必须采用解耦权重衰减（AdamW）才能实现真正的独立范数收缩。}
...
\begin{equation}
\begin{aligned}
 \mathbb E[s\mid \vect{a}] &= \vect{w}^{\mathrm{T}}\vect{a}, \\
...
推理阶段关闭 Dropout 并直接使用缩放后的原激活向量以维持期望尺度。由于非线性激活函数通常不满足 Jensen 算子交换律（$\mathbb E[\phi(s)]\ne\phi(\mathbb E[s])$），确定性单次前向输出仅为子网络集成的一种确定性近似。工程实现上须严格区分训练模式与评估模式（\texttt{model.eval()}），并与禁用梯度追踪（\texttt{torch.no\_grad()}）协同配置。
...
实际选择停止时刻需要预先明确验证指标、评估频率、最小改善幅度和耐心窗口，并保存被选中的参数状态。测试集不参与这一选择。对验证集反复增添候选配置，会逐渐把验证集的信息吸收到选择结果中，最终评估仍要依靠封存的测试集或外层流程。主流泛化与正则化技术作用机制与选型对比如表\ref{tab:opt-regularization}所示。

\begin{table}[htbp]
\centering
\caption{主流泛化与正则化技术选型对比}
\label{tab:opt-regularization}
...
\end{equation}
再对 $\mat{X}$ 积分即可得到总体平方风险。偏差衡量平均预测偏离真实条件均值的程度，方差衡量训练样本与训练随机性改变时预测波动的程度。此处的模型预测方差衡量有限样本估计对训练集的敏感度，与前文随机梯度下降中单步小批量采样引入的随机梯度方差概念不同。

这一加法分解依赖平方损失及条件均值目标，用于准确率或交叉熵时形式不再成立。深度网络的测试表现也不随参数量呈简单的 U 形关系；数据规模、结构、优化过程和正则化共同决定预测函数的分布，预测方差的分析要结合这些条件。

\begin{example}[收缩估计的偏差与方差]
...
\begin{align*}
 \operatorname{Bias}^2 &= (1-a)^2\mu^2, \\
...
\end{tikzpicture}
\caption[学习与评估的信息边界]{学习与评估的信息边界。虚线表示允许依据验证结果重新训练；最终测试结果不回流到本轮配置选择。}\label{fig:opt-data-boundary}
\end{figure}
...
\begin{equation}
\begin{aligned}
 \widehat\mu_j &= \frac1n\sum_{i\in S_{\mathrm{train}}}x_{ij}, \\
...
\begin{warning}[预处理统计与评估数据污染]
特征归一化统计量（均值、方差）、词表构建及缺失值填充规则，严禁在包含验证集或测试集的全量数据上预先计算；所有预处理参数必须且仅能在训练折内单向拟合。在评估大语言模型时，若预训练语料混入测试基准样本，将造成严重的评测污染，得出虚高的泛化结论。
\end{warning}

完整发布的端到端模型制品应冻结模型权重、预处理归一化参数、词表与类别映射、提示词模板及后处理决策阈值。任何上游特征流水线或阈值调优均属于统计假设空间搜索，须纳入严格的数据隔离与版本追踪管理。
...
训练损失下降而验证损失上升是模型过拟合的典型表征，但对比前须确保二者评估口径严格对齐：训练日志常包含数据增强、随机 Dropout 扰动或滑动平均估计，而验证集通常关闭随机扰动并在全量留出集上评估无正则损失。规范的泛化诊断应在相同的评估模式（Eval Mode）下分别计算训练集与验证集的无偏数据损失。常见训练失效模式与诊断对照如表\ref{tab:opt-failure-diagnosis}所示。

\begin{table}[htbp]
\centering
\caption{训练与泛化失效模式归因对照}
\label{tab:opt-failure-diagnosis}
...
\end{tabularx}
\end{table}
排查模型训练与泛化异常需建立分层证据链：有限差分校验解析梯度的局部正确性，单批次过拟合测试验证优化与反向传播链路闭环，留出验证集监控泛化误差，而线上影子流量则追踪数据分布漂移。极小的梯度范数可能源自临近驻点，也可能暗示深层网络饱和或梯度消失；诊断时需结合多层梯度范数与更新信噪比综合研判。
