...
\State 初始化 $\vect{\theta}_0$；在固定评估模式下计算初始验证损失 $v_0$
\State 保存 $\vect{\theta}_0$；$v_{\min}\gets v_0$，$v_{\mathrm{pat}}\gets v_0$，$h\gets0$
\For{$k=0,\ldots,K-1$}
  \State 按明确抽样规则取训练批量，在 $\vect{\theta}_k$ 处求平均数据梯度 $\vect{g}_k$
  \State $\vect{\theta}_{k+1}\gets\vect{\theta}_k-\eta_k(\vect{g}_k+\lambda\vect{\theta}_k)$，正则项只计一次
  \If{$k+1$ 是 $r$ 的倍数，或 $k+1=K$}
    \State 按固定口径计算验证数据损失 $v$，验证过程不反向、不更新预处理状态
    \If{$v<v_{\min}$}
      \State 保存 $\vect{\theta}_{k+1}$；$v_{\min}\gets v$
    \EndIf
