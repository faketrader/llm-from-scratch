...
缓存复用以因果结构为基础，其存储形式包括连续布局、分页寻址和前缀共享。\bookxref{ch:transformer}定义网络层，\bookxref{ch:long-context}讨论长距离能力。本章侧重执行状态；算子优化和投机生成由\bookxref{ch:inference-algorithms}展开，服务调度由\bookxref{ch:batching-scheduling}展开。

\section{增量解码的等价性}

\subsection{预填充与解码阶段特征}
...
数学上等价的计算，在不同内核中仍可能因浮点求和顺序、精度与归约布局而产生数值差异。输出概率接近时，这些差异还可能改变贪心选择的词元。验证时先比较同一输入下的分数或概率，再检查误差是否来自浮点归约、掩码或位置处理。
...
\end{tikzpicture}
\caption[单层增量解码数据流]{单层增量解码数据流。新查询读取全部合法键值，历史查询和历史前馈输出不再重算。}\label{fig:cache-incremental}
\end{figure}
\end{example}
...
\end{tikzpicture}
\caption[多头、分组与多查询注意力键值共享]{多头、分组与多查询注意力键值共享。以四个查询头为例，MHA、GQA 与 MQA 的缓存键值头数依次为 4、2、1。}\label{fig:cache-heads}
\end{figure}
...
取 $N=32,h_q=32,d_h=128,b_e=2,L=8192,B=1$。每词元 MHA 缓存为 $2\times32\times32\times128\times2=524288$ 字节，即512 KiB。GQA 取8个键值头，每词元128 KiB；MQA 每词元16 KiB。如表\ref{tab:cache-variants-memory}所示，不同架构的裸 KV 缓存显存占用存在数量级差异。

\begin{table}[htbp]
\centering
\caption[不同注意力架构 KV 缓存显存占用]{不同注意力架构 KV 缓存显存占用。典型配置下 MHA、GQA 与 MQA 在单请求与四请求下的裸缓存显存对比。}\label{tab:cache-variants-memory}
\begin{tabular}{lrrr}
...
\end{equation}
由于 RoPE 旋转矩阵 $\mat{R}_j$ 显式依赖历史位置索引 $j$，将其直接吸收到查询侧需要满足交换律 $\mat{R}_t^{\top}\mat{R}_j\mat{U}_{K,i} = (\cdots)\mat{R}_j$。然而高维正交旋转矩阵一般不与通用投影矩阵交换，因此无法在压缩缓存中直接剥离 $j$ 相关的旋转变换。\keyconcept{RoPE 旋转矩阵与一般低秩投影矩阵不可交换，多头潜在注意力必须将旋转位置编码与可矩阵吸收的内容向量严格解耦。}忽略非交换性会导致缓存压缩推导违背线性代数结合律。
...
\end{tikzpicture}
\caption[分页注意力逻辑块到物理块映射]{分页注意力逻辑块到物理块映射。页表解决物理显存非连续存储布局，逻辑位置与有效长度决定模型可见上下文。}\label{fig:cache-pages}
\end{figure}

\subsection{分页缓存的容量分配}
...
\subsection{批内变长}
对长度分别为 $L_1,\ldots,L_B$ 的请求，每个新查询仅能读取自身有效前缀。连续批次中的行号表示当前执行槽位，请求身份则跨调度步骤保持稳定。请求完成后压缩批次、交换行或加入新请求时，需按请求身份同步更新缓存映射。
...
当多个分支共享\emph{未满块}又要追加不同词元时，需要\term{写时复制}{Copy-on-Write}{CoW}：为修改分支分配新块，复制有效前缀，再追加。否则一个请求会覆盖另一个请求的缓存。\emph{已满共享块}无需复制，后续分支直接分配各自的新块即可。
...
\subsection{缓存淘汰策略}
活动请求持有引用的缓存需要保留，直到请求释放引用或完成可恢复的状态转移。\term{缓存淘汰}{Cache Eviction}{}通常针对没有活动引用但为未来复用保留的块；若要抢占活动请求，应保存可恢复映射并换出，或释放后按同一输入重新计算。前者消耗传输，后者消耗计算，两者的语义与普通缓存命中不同。
...
\end{tikzpicture}
\caption[分页缓存块生命周期状态机]{分页缓存块生命周期状态机。写时复制边表示创建私有副本，原共享块保持不变；请求取消与正常结束均按所有权释放引用，未完成访问前禁止复用物理存储。}\label{fig:cache-lifecycle}
\end{figure}

\begin{algorithm}[前缀缓存的引用与追加]
\AlgoInput{计算身份、词元前缀、请求身份及所需新增容量。}
\AlgoOutput{请求页表和可继续执行的私有尾部。}
\begin{enumerate}
...
\subsection{历史状态淘汰条件}
模型若原本采用包含当前位置的窗口 $w$，未来查询只读取最近 $w$ 个键值，则窗口之外的历史键值不再作为该层的直接输入，可在保持必要元数据的条件下回收。删除全注意力模型的旧键值，则改变式\eqref{eq:cache-attn}的求和集合，属于近似策略。表\ref{tab:kv-compression-taxonomy}比较了 KV 缓存压缩与显存管理方法，以及各自的精度和执行条件。
...
\caption{KV 缓存压缩与显存管理方法}
\label{tab:kv-compression-taxonomy}
\begin{tabularx}{\linewidth}{@{}
  >{\raggedright\arraybackslash}p{40mm}
  >{\raggedright\arraybackslash}p{47mm}
  >{\raggedright\arraybackslash}X@{}}
\toprule
技术路线与代表方案 & 压缩收益与精度影响 & 核心机制与工程权衡 \\
\midrule
头维度共享\newline \mbox{MQA / GQA} & $2\times\sim 8\times$；极低（需预训练） & 减少独立键值头数，显著降低访存带宽压力 \\
低秩潜在投影\newline \mbox{MLA（DeepSeek）} & 约 $4\times\sim 7\times$；极低（原生） & 潜变量投影结合解耦 RoPE，兼顾模型容量与缓存开销 \\
存储分页管理\newline \mbox{PagedAttention} & 碎片率降至 $<4\%$；无损 & 离散虚拟分页，消除按最大长度预留造成的显存碎片 \\
低比特量化\newline \mbox{FP8 / INT4 KV} & $2\times\sim 4\times$；需离群值保护 & 按词元或通道量化，提升并发批处理吞吐 \\
局部窗口截断\newline \mbox{StreamingLLM} & 上限 $\mathcal{O}(W)$；损失长程依赖 & 保留少量初始注意力沉点与局部滑动窗口 \\
稀疏注意力\newline \mbox{H2O / SnapKV} & 约 $2\times\sim 5\times$；取决于检索需求 & 根据注意力得分动态淘汰冗余历史键值 \\
\bottomrule
\end{tabularx}
\end{table}

滑窗模型的高层新状态可能包含通过中间位置传递的更早信息。\emph{增量滑窗缓存}仅在各层保留最近键值，与从原始文本只截取最近 $w$ 个词元重新运行的\emph{截断文本重算}存在本质计算差异：前者深层神经元仍隐式保留更早历史通过中间层逐级传递的残差信息，而后者直接抹去窗口前的一切上下文交互。工程上的数值等价性校验必须严格以增量滑窗前向作为基准。

混合全局层与滑窗层时，缓存保留范围按层不同。累计逻辑位置序列长度与当前物理显存驻留词元数具有不同的控制语义：前者决定位置编码与因果可见性掩码，后者决定物理缓存分配与显存生命周期回收。
...
\begin{note}[KV 缓存系统的正确性保证体系]
KV 缓存工程依托三层严格解耦的保证机制：在数学语义层，确保因果掩码与前缀状态等价于完整前向计算；在存储执行层，依托虚拟分页与页表原子更新确保有效逻辑长度与显存生命周期一致；在安全隔离层，依托命名空间校验与权限域判定防止越权命中与跨租户信息泄露。
\end{note}

\section{增量执行过程}

\begin{algorithm}[带缓存的自回归生成]
\AlgoInput{非空提示、冻结模型、生成上限 $G>0$、停止规则与空或兼容前缀缓存。}
\AlgoOutput{最多 $G$ 个词元及最终缓存进度。}
\begin{enumerate}
