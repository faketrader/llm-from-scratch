...
\end{equation}
由于 $V_c$ 为单位矩阵，$\nabla_U\mathcal L=G$。若 $a_1=u_1$，式\ref{eq:mm-normalize-grad}去掉第一坐标方向，得到 $\nabla_{a_1}\mathcal L=(0,0.134471)^{\mathsf T}$。梯度下降会让第一幅图像表示朝远离第二条文本的方向移动；更新方向由正对、负对与归一化共同决定。对数缩放梯度为两个对角项之和，即 $-0.268942$，在此构造数据上下降更新会增大缩放。若批内都是错误标注，则这种扩大间隔也可能扩大错误。
\end{example}
...
\end{align}
$Q\in\mathbb R^{M\times d_k}$ 为可训练查询，输出长度为 $M$。每一输出是值向量的加权组合，不是选择某一个原始块。查询固定时，注意力权重 $A$ 仍随输入键变化。查询间可加入自注意力与前馈层；这些设计改变表达能力，但不改变“输出长度受控”的接口目的。
...
对于固定注意力矩阵 $A$，若 $M<N_v$，其秩至多为 $M$，存在非零方向 $\Delta V$ 使 $A\Delta V=0$。所以仅从 $AV$ 不能恢复任意 $V$。完整重采样器的 $A$ 依赖输入，形成非线性压缩。训练目标与细粒度评估用于确定任务所需信息的保留程度。

\subsection{重采样梯度}
\begin{example}
\label{q:ch38:example:02}
...
统一序列建模将多模态输入放入共同主干，各输出模态使用相应的表示与目标。生成离散声学或视觉词元需要相应词表与解码器；生成连续潜变量可能需要不同损失。架构应说明编码、融合与输出头三条边界，不能只画一个接收所有模态的方框。
...
对于批量中的不同回答长度，按所有受监督词元求平均与先按每样本求平均再对样本平均不同。前者给予长答案更大权重，后者使每个样本总权重相同。训练配置应明确归约方式；全掩码样本的 $N_s=0$ 需要单独处理，不能产生未定义除法，也不能通过加入虚假标签凑数。
\end{example}
...
\end{equation}
例如每秒取2帧，间隔0.5秒，持续0.1秒的事件命中概率仅为0.2。该概率衡量采样是否覆盖事件，覆盖后的识别率由视觉模型决定。没有任何采样帧记录的短暂动作，后续更大语言模型也不能从缺失观测中可靠恢复。
...
\end{tikzpicture}
\captionof{figure}{不同模态通过时间区间对应，而非按数组位置一一配对。}
\label{fig:mm-time}
\end{center}
...
\subsection{依赖及能力边界}
模型制品应保存感知编码器、连接器、语言主干、输出解码器、预处理器、分词器、模板、精度与后端配置。多组件许可和使用条件也应逐一核对，不能把语言主干的许可自动推广到视觉编码器、训练数据和整个组合模型。
