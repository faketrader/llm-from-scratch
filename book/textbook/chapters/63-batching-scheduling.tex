...
\end{equation}
因此最后一个词元到达的时延恰为 $\operatorname{TTFT}_i+(O_i-1)\operatorname{TPOT}_i$。这里的 TTFT 已包含到首次词元之间的排队，再叠加 $W_i=b_i-a_i$ 会重复计入。只有一个词元时 TPOT 不定义；无输出的失败请求进入失败统计，零时延替代不了它。
...
\term{静态批处理}{Static Batching}{}在批次开始时确定成员，通常在当前批次退出后才加入下一组请求。已完成序列可以提前返回结果，但空出的执行位置若不能补入新请求，仍会降低资源利用率。静态批处理与禁止提前返回是两件事。
...
表\ref{tab:rev-batching-paradigms}对比了大语言模型推理服务中批处理调度技术的演进范式。从粗粒度的请求级同步，到步进级的连续动态插入，再到混合分块与抢占，调度机制直接决定了服务的延迟、吞吐与 SLO 达成率。

\begin{table}[htbp]
\centering
\caption{大语言模型推理批处理调度范式对比。}
\label{tab:rev-batching-paradigms}
...
每页为1 MiB。两条活动序列长度分别为257、255，需 $17+16=33$ 页。若为它们各解码一步，长度变为258、256，仍为33页。此轮另处理一个新请求的256词元提示块，需要16页，共49页；若该请求声明最终最多使用1024位置，则其完整上界需64页，与已有33页合计97页，超出64页容量，三者无法同时完整驻留。
...
始终优先解码可以降低活动请求的词元间隔，代价是等待提示长期得不到首词元。始终优先新提示又可能使活动输出停顿。可为预填充保留最低服务份额，并在交互轮时预算下分配剩余额度；当多个目标冲突时，应记录超额负载和拒绝，平均吞吐数掩盖不了其中的取舍。

\section{排队关系的推导与适用条件}
...
式\eqref{eq:schedule-little}不要求泊松到达或指数服务，但要求人数、吞吐和时间来自同一系统边界与同一请求总体。将被拒请求计入到达率、却只对接纳请求计算驻留时间，等式便失去对应关系。例如稳定系统每秒完成20个请求，平均驻留0.4秒，则平均系统内请求数为8；尾延迟和队列上限还需结合驻留分布与突发负载确定。

\subsection{M/M/1 模型解释高利用率风险}
\begin{assumption}
设到达为速率 $\lambda$ 的泊松过程，单服务台独立服务时间服从速率 $\mu$ 的指数分布，先到先服务、无限等待空间且无放弃，$\rho=\frac{\lambda}{\mu}<1$。该模型用于解释机制；连续批处理的服务率随活动集合变化，这些条件对它只是近似。
\end{assumption}
...
\begin{equation}
 \Pr(T>t)=\conste{}^{-(\mu-\lambda)t},\qquad
...
\label{fig:schedule-backpressure}
\end{figure}
...
\item 对本轮计划原子预留页与状态版本。若预留失败，撤销该计划并重新选择；部分成功的预留构不成可执行计划。
...
\term{开放环负载}{Open-Loop Load}{}按独立于完成速度的计划提交请求，能够显露过载积压；\term{闭合环负载}{Closed-Loop Load}{}在固定并发用户收到响应后再提交，其到达率会在服务变慢时自行下降。两种模型回答不同问题。若原目标是固定外部到达流，测试却在每个响应后才继续，就会遗漏拥塞期间本应到达的请求，这种测量偏差常称为\term{协调遗漏}{Coordinated Omission}{}。闭合环测量因此回答不了开放环的过载问题。外部突发稳定性应使用开放环负载及对应到达过程测量。

将满足质量与时延目标的成功请求数除以观察时长，可定义该目标下的\term{有效吞吐}{Goodput}{}。例如某观察窗口内共提交1000个请求，拒绝100个、失败50个、成功850个，其中700个满足所有目标，窗口100秒，则提交率10、成功吞吐8.5、有效吞吐7请求每秒。若只在成功者中报告满足率，得到 $\frac{700}{850}$；若从所有提交者看，则是70\%。两者相差的正是分母。
