...
\begin{document}
\fi
\bookchapter{自回归语言建模}{ch:autoregressive}
...
\begin{equation}\label{eq:ar-chain}
 \begin{aligned}
...
\begin{table}[htbp]
\centering
\caption{自回归建模与解码生成核心数学符号}\label{tab:generation-notations}
...
\begin{equation}\label{eq:ar-head}
 \begin{aligned}
  \vect{z}_t&=\vect{h}_t\mat{W}_{\mathrm{lm}}+\vect{b},\\
  \vect{p}_\theta(x_{t+1}=v\mid x_{\leq t})
  &=\frac{\exp z_{t,v}}{\sum_{u\in\mathcal V}\exp \vect{z}_{t,u}}.
 \end{aligned}
\end{equation}
...
\end{tikzpicture}
\caption[因果网络训练与生成阶段对比]{因果网络训练与生成阶段对比。训练阶段利用所有有效位置已知目标并行监督；生成阶段读取当前前缀末位分布自回归采样新词元。}\label{fig:ar-gpt-flow}
\end{figure}
...
\subsection{输入目标错位}
\term{教师强制}{Teacher Forcing}{}在训练时始终给定数据中的真实前缀，再预测其后一个真实词元。\emph{教师强制避免了误差在训练阶段沿时序累积}；但在实际生成时，模型必须以\emph{自身先前的预测输出}作为后续条件，由此引发训练分布与自回归生成分布的\emph{暴露偏差}（Exposure Bias）。设原序列为
...
\end{tikzpicture}
\caption[因果注意力前缀依赖与监督移位]{因果注意力前缀依赖与监督移位。竖直箭头给出输入位置与预测对象的移位对应，横向虚线表示自回归前缀因果依赖。}\label{fig:ar-shift}
\end{figure}
...
\begin{warning}[自回归标签移位的双重移位陷阱]
自回归训练必须且仅能执行一次标签移位：若数据加载器已完成输入前缀与目标标签的移位对齐，而损失函数内部又调用了自动切片移位逻辑（如常见库的默认行为），训练目标将退化为跨两步预测；反之，若两处均未移位，模型将退化为当前词元的直接恒等复制。
\end{warning}
...
\begin{equation}\label{eq:ar-loss}
 \begin{aligned}
  ...
  \log p_\theta(x^{(b)}_{t+1}\mid x^{(b)}_{\leq t}),\\
  N&=\sum_{b,t}m_{bt}>0.
 \end{aligned}
\end{equation}
...
\AlgoInput{各文档的词元序列、文档边界、填充标志和监督区域。}
\AlgoOutput{标量损失和有效目标数量。}
\AlgoState{固定的当前参数 $\theta$；此过程只计算一次目标，不推进优化器。}
\begin{myalgorithmic}[1]
 \State Within each valid document span, pair input position $x_t$ with target $x_{t+1}$; define boundary supervision according to whether BOS and EOS are included
 \State Build causal and padding visibility and mark valid target $m_t$; whether documents may connect follows the predefined objective rather than an ad hoc batch-boundary decision
 \State Run the causal network to obtain scores aligned with input positions; ensure that shifting occurs exactly once, either in the data path or in the loss
 \State Accumulate negative log-probability over valid positions and target count $N$; if $N=0$, return an explicit unsupervised-batch status without division by zero or a fabricated zero gradient
 \State \Return the accumulated sum divided by $N$; each target may read only its prefix, padding positions contribute no supervision, and input--target indices follow Figure~\ref{fig:ar-shift}
\end{myalgorithmic}
\end{algorithm}
...
\begin{table}[htbp]
\centering
\caption{大语言模型生成事实性异常分类}\label{tab:ar-factuality-criteria}
...
\begin{equation}
 \begin{aligned}
  \frac{\dif H}{\dif\beta}&=-\beta\operatorname{Var}(\vect{z}),\\
...
\begin{equation}
 \begin{aligned}
  m&=\min\left\{r:\sum_{j=1}^{r}p_{(j)}\geq\rho\right\},\\
...
\emph{Top-$k$ 固定保留候选总数}，而在概率分布极度扁平或尖锐时无法自适应动态伸缩；\emph{Top-$p$（核采样）固定累积概率质量门槛}，能随上下文不确定性自适应调整有效候选集。二者截断概率质量的几何差异如图\ref{fig:sampling-geometry-truncation}所示。二者均改变支持集，可能移除 EOS 或任务所需的低概率候选，质量上的代价需要在具体任务中衡量。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth]{\subfix{../../figures/theory/sampling-geometry-truncation.pdf}}
\caption[Top-$k$ 与 Top-$p$ 采样截断对比]{Top-$k$ 与 Top-$p$ 采样截断对比。左图展示固定候选数 $k$ 的硬截断几何选择；右图展示依据动态累积概率质量 $\rho$ 的自适应核集合划分。}\label{fig:sampling-geometry-truncation}
\end{figure}
...
若先降温再做同样的 Top-p，前两项的质量为 $\frac{8}{15}+\frac{3}{10}=\frac{5}{6}>0.75$，只保留两项，最终分布为 $(\frac{16}{25},\frac{9}{25},0,0)$。如果先在原分布上做 Top-p，再降温，支持集仍有三项，最终为 $(\frac{16}{29},\frac{9}{29},\frac{4}{29},0)$。\keyconcept{解码处理器的调用次序不可交换：先温度缩放后做 Top-$p$ 截断，与先 Top-$p$ 截断后温度缩放会得到不同的采样支持集与概率分布。}温度不改变候选排序，所以在没有其他变换、并列规则相同的条件下，它与固定 Top-k 支持集选择的关系不同；一个特例不足以推出所有处理器都可交换。
\end{example}
...
\begin{table}[htbp]
\centering
\caption{贪心解码局部与全局概率不一致示例}\label{tab:greedy-failure}
\begin{tabular}{lc}
\toprule
完整序列 & 概率\\\midrule
...
\begin{tikzpicture}[>=Stealth,node distance=12mm,
 ...
 \draw[->] (b)--(be);\draw[->] (b)--(bc);
\end{tikzpicture}
\caption[局部贪心解码概率树示例]{局部贪心解码概率树示例。节点数值为累计概率，展示局部最优选择 $a$ 错过全局最高概率序列分支 $(b, \mathrm{EOS})$。}\label{fig:ar-beam-tree}
\end{figure}
\end{example}
...
\label{alg:ar-beam}
\AlgoInput{条件 $c$、束宽 $K$、EOS 集合、最大新增长度 $L_{\max}$ 和确定的并列规则。}
\AlgoOutput{一个完成候选及终止原因，或未完成候选及长度截断标记。}
\AlgoState{至多 $K$ 条记录，每条含前缀、累计分数、完成标记及其模型状态。}
\begin{enumerate}
...
最大概率输出与高质量输出不是同一个数学目标。开放式任务有大量合理续写，高概率路径可能重复或过于笼统；有严格约束的翻译与结构化任务也可能因长度或覆盖问题失真。解码选择应围绕任务评价，束宽增大与模型能力提升是两回事。核采样相关研究讨论的退化现象也有特定模型和任务条件，无法外推成所有任务上唯一最优策略\cite{holtzman2020degeneration}。表\ref{tab:decoding-comparison}全面对比了自回归生成中的主流解码与采样策略。

\begin{table}[htbp]
\centering
\caption{自回归语言模型主流解码与采样策略全景对比}\label{tab:decoding-comparison}
...
\AlgoInput{序列化的条件词元、词表与模板、温度 $\tau>0$、核阈值 $\rho$、EOS 与停止字符串，以及最大新增词元数 $L_{\max}$。}
\AlgoOutput{响应词元、文本和停止原因。}
\AlgoState{当前前缀、随机状态、可选键值缓存和增量文本解码状态。}
\begin{myalgorithmic}[1]
\State Validate $\tau>0$, $0<\rho\le1$, nonnegative integer $L_{\max}$, and conditioning-position bounds; fix inference mode
\State Set the current prefix to the conditioning tokens and the response to empty; initialize the stop reason to length truncation
\For{$t=1,\ldots,L_{\max}$}
  \State Compute scores at the current final position and apply the declared hard constraints
  \If{the allowed candidate set is empty}
    \State Set the stop reason to constraint conflict and terminate the loop
  \EndIf
  \State Normalize $\frac{z}{\tau}$ stably and construct the smallest nucleus with cumulative probability at least $\rho$ under the fixed tie-breaking rule
  \State Renormalize within the nucleus and sample token $v$ using the current random state
  \State Append $v$ to the response and prefix, synchronizing cache and logical positions
  \If{$v$ belongs to the EOS set}
    \State Set the stop reason to natural termination and terminate the loop
  \EndIf
  \State Update the incremental text-decoding state
  \If{a stop string matches}
    \State Handle the stop string under the protocol, record the reason, and terminate the loop
  \EndIf
\EndFor
\State \Return response tokens, incrementally decoded text, and the stop reason
\end{myalgorithmic}
...
词元概率仅反映模型在当前前缀下的条件预测偏好，不可等同于客观事实命题的置信度。对于由多个词元构成的完整回答，局部词元的高生成概率并不蕴含整体陈述的客观真实性。最终评价应依据任务要求，严格区分模型原始条件分布、解码采样分布与实际返回文本之间的差异。

\begin{note}[概率模型、解码算法与任务评价的解耦]
