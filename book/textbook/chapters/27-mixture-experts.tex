...
\textbf{输入：}$\mat{X}\in\Real^{N\times d}$，路由器 $\mat{W}_r,b_r$，$E_r$ 个专家，选择数 $k$，容量 $C$；采用选中内归一化、按词元顺序接收、拒绝路径置零且不二次归一化。

\textbf{输出：}$\mat{Y}\in\Real^{N\times d}$，申请数、接收数、拒绝数和路由记录。\textbf{状态：}每专家槽位计数 $c_e=0$，输出 $\mat{Y}=0$，以及接收记录 $(t,e,c,a_{te})$。
\begin{enumerate}
