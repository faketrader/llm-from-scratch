...
\begin{document}
\fi
\bookchapter{混合专家模型}{ch:moe}

稠密前馈层对每个词元应用同一组参数。增大前馈宽度可以增强模型的表征容量，但亦按比例线性增加每个词元的浮点计算量（FLOPs）。混合专家模型解耦了参数容量与单步计算成本：在模型中维护多组参数独立的变换专家，由门控路由器动态决定每个词元激活的稀疏子集。激活参数的缩减并未降低模型权重在显存中的静态驻留开销；且跨设备的分布式通信时延与长尾负载瓶颈受动态路由分布的直接支配。
...
\begin{table}[htbp]
\centering
\caption{混合专家模型核心符号}\label{tab:moe-symbols}
...
路由器先计算
\begin{equation}\label{eq:moe-router}
\begin{aligned}
 \mat{G} &= \mat{X}\mat{W}_r+\mathbf1_Nb_r^{\mathrm{T}}, \\
...
\end{equation}
稠密混合仍需计算所有专家的输出，即使其中一些权重接近零。只有在执行时跳过相应专家，才能减少这部分算术开销。

\term{稀疏专家混合}{Sparse Mixture of Experts}{}只执行选中集合 $S_t$ 中的专家：
\begin{equation}\label{eq:moe-sparse}
\begin{aligned}
 S_t &= \operatorname{TopK}(g_{t,:},k), \\
...
\begin{equation}
\begin{aligned}
 \frac{\partial\mathcal L}{\partial a_e} &= r_e, \\
...
\begin{equation}
\begin{aligned}
 \frac{\partial\mathcal L}{\partial \mat{W}_r} &= \mat{X}^{\mathrm{T}}\mat{G}_G, \\
...
\begin{equation}
\begin{aligned}
 \mat{X}_e &= \mat{D}_e\mat{X}, \\
 \mat{H}_e &= F_e(\mat{X}_e),
\end{aligned}
\end{equation}
...
\end{tikzpicture}
\caption[稀疏 MoE 路由分发与聚合数据流]{稀疏 MoE 路由分发与聚合数据流。门控网络决定词元到专家的动态分发，专家动态批次由路由决定，多专家输出加权聚合回残差主干。}\label{fig:moe-dispatch}
\end{figure}
...
\subsection{专家容量与批次依赖}
容量竞争发生在路由组内。若采用先到先得，后部位置可能更容易被拒绝；若按门控权重排序，则低权重路径更容易失去计算。于是同一词元在不同批次组成中，可能因为其他词元的申请而获得不同输出。这样的函数依赖同批其他词元，与逐词元独立的前馈函数不同。

对于严格自回归训练，还应警惕一种间接依赖：若先观察整段序列的路由权重，再按全局排名决定早期词元能否获得容量，\emph{未来词元可能影响早期位置的接收结果}。避免这一问题需要容量方案和接收规则与因果定义相容，注意力层的因果掩码对此无能为力。实际设计应区分前向内容读取、容量竞争以及训练辅助统计三类关系。

\section{路由训练}

\subsection{路由负载与选择概率}
定义容量截断前的分配比例和平均概率：
\begin{equation}\label{eq:moe-load-stats}
\begin{aligned}
 f_e &= \frac1{Nk}\sum_{t=1}^{N}\mathbf1[e\in S_t], \\
...
若只统计容量截断后的负载，被饱和容量裁平的结果可能看起来很均衡，却掩盖大量溢出。应同时保留申请数、接收数、拒绝数和执行耗时。专家宽度不同时，每次调用的计算量不同，负载统计需按各专家的计算量加权。

\subsection{辅助均衡目标与梯度范围}
Switch 使用硬分配比例与平均概率乘积构造辅助目标。按照式\eqref{eq:moe-load-stats}的 top-$k$ 归一化，本章讨论其一种相应扩展：
\begin{equation}\label{eq:moe-loadloss}
\begin{aligned}
 \mathcal L_{\mathrm{bal}} &= E_r\sum_{e=1}^{E_r}f_e\bar p_e, \\
...
均衡压力过大时，路由可能优先满足平均分配而牺牲任务有效性；压力过小时，少数专家可能成为瓶颈并获得不成比例的更新。除了辅助损失，也可以通过容量分配、专家复制或路由偏置控制负载，各策略机制对比如表\ref{tab:moe-balance-strategies}所示。

\begin{table}[htbp]
\centering
\caption{MoE 负载均衡与正则化策略对比}\label{tab:moe-balance-strategies}
...
\begin{equation*}
 ...
 \end{bmatrix}.
\end{equation*}
因为每行概率之和为一，所以 $\mat{G}=\mat{X}\mat{W}_r=\mat{W}_r$，Softmax 后恰好得到矩阵中的对数自变量。设三个专家为线性映射 $F_1(\vect{x})=\vect{x}$、$F_2(\vect{x})=2\vect{x}$、$F_3(\vect{x})=3\vect{x}$。各词元分配细节如表\ref{tab:moe-example-routing}所示。

\begin{table}[htbp]
\centering
\caption[四词元三专家路由分配计算示例]{四词元三专家路由分配计算示例。展示四词元在 Top-2 门控下的 Softmax 概率分布、选中专家集合与重归一化权重。}\label{tab:moe-example-routing}
\begin{tabular}{clll}
...
截断前的归一化分配比例为
\begin{align*}
 f &= \left(\frac{3}{8},\frac{1}{2},\frac{1}{8}\right), \\
...
\begin{equation}
\begin{aligned}
 P_{\mathrm{total}} &= P_0+P_r+P_s+E_rP_e, \\
...
\begin{equation}
\begin{aligned}
 D_{ts} &= \frac{\conste{}^{Z_{ts}}}{\sum_{u=1}^{N}\conste{}^{Z_{us}}}, \\
...
\begin{table}[htbp]
\centering
\caption{专家路由方式与约束特性对比}\label{tab:moe-routing-modes}
\begin{tabularx}{\linewidth}{lXXX}
...
同一设备可容纳多个专家，单个专家也可进一步进行张量切分。逻辑专家索引、物理设备拓扑编号与张量模型并行分片索引分属于不同的映射层次。模型配置文件定义逻辑专家拓扑，分布式运行时放置表（Placement Map）负责设备与通信域绑定，检查点权重切片必须保存精确的分片元数据以支持跨硬件拓扑的确定性恢复。
...
模型名称中出现 MoE 或激活参数数字，只能提供结构线索。需要按每层核对路由专家数、共享专家数、每词元选择数、中间宽度、门控归一化、路由激活、容量策略及哪些层使用 MoE。再将这些配置与路由矩阵、专家参数张量和前向定义相互对应。主流开源 MoE 模型核心架构与路由配置参数如表\ref{tab:moe-architectures}所示。

\begin{table}[htbp]
\centering
\caption{典型开放 MoE 模型架构参数对比}\label{tab:moe-architectures}
