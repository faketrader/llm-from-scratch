...
\term{验证器}{Verifier}{}可以是确定性规则，也可以是具有误差的学习模型。确定性规则在相同输入与环境下产生相同判定，覆盖范围由规则定义。数学答案检查需要定义等价关系、定义域、单位和数值容差；代码检查需要约束执行环境、测试输入、超时和副作用；工具任务以执行回执及目标系统的实际状态变化确认操作结果。
...
\begin{table}[htbp]
\centering
\caption{可验证推理主要符号与计量对象}
\label{tab:verifiable-symbols}
\begin{tabular}{ll}
...
\subsection{结果与过程监督}
\term{结果监督}{Outcome Supervision}{}根据最终答案或整条完成的结果给出反馈，\term{过程监督}{Process Supervision}{}在中间步骤提供更细粒度标签。\term{结果奖励模型}{Outcome Reward Model}{ORM}与\term{过程奖励模型}{Process Reward Model}{PRM}分别学习这两类信号。过程监督研究明确区分了最终结果标签与步骤标签，两类标签的性能结论各自成立。\cite{lightman2023verify}表\ref{tab:prm-vs-orm}系统对比了两者的监督粒度与工程权衡。

\begin{table}[htbp]
\centering
\caption{过程奖励模型与结果奖励模型对比}
\label{tab:prm-vs-orm}
...
\end{equation}
第 $j$ 步产生的动作影响该步及后续结果，已经发生的前序奖励则不在其影响范围之内。在适当因果和基线条件下，使用后续回报有助于去掉与当前动作无关的历史噪声。将 $R_j$ 分配给该步骤的生成词元时，还需声明步骤边界及其长度权重。把每个步骤的即时分数直接当作所有先前词元的优势，与把未来步骤分数累加为回报，构成两种不同的时间信用分配范式：前者仅度量局部转移质量，后者建模长程累积回报；优势函数还须进一步扣除当前前缀状态下的基准预期回报。过程标签描述步骤质量，优势则需要扣除给定历史下的预期回报。
...
本章采用分母为 $G$ 的组内方差：
\begin{equation}\label{eq:reason-advantage}
\begin{aligned}
 \bar r &= \frac1G\sum_{i=1}^G r_i, \\
 s_r^2 &= \frac1G\sum_i(r_i-\bar r)^2, \\
...
对全组奖励相等的情形，分子均为零，按式\eqref{eq:reason-advantage}得到全零优势。这类\term{零方差组}{Zero-variance Group}{}不提供组内排序信号；它可能\emph{全对}，也可能\emph{全错}。若目标含独立 KL 正则，该组仍可能有正则梯度。\keyconcept{零方差组不提供组内相对排序优势，但若直接丢弃全对或全错组，将引入由问题解题难度介导的选择性采样偏差。}丢弃零方差组会改变问题采样权重，应记录其比例与丢弃规则，并结合问题难度分析这些组的分布。
...
\begin{equation}
\begin{aligned}
 g_i &= \nabla_{\vect{\theta}}\log\pi_{\vect{\theta}}(y_i\mid x), \\
 g &= \mathbb E[r_i g_i],
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:reason-ratio}
\begin{aligned}
 w_i &= \frac{\pi_{\vect{\theta}}(y_i\mid x)}{\pi_{\mathrm{old}}(y_i\mid x)} = \prod_t\rho_{it}, \\
...
\end{equation}
单个 $\rho_{it}$ 仅修正给定历史下的动作分布，不修正此前历史的分布；逐词元概率比的算术平均与完整序列联合重要性权重（连乘积）具有不同的数学期望与方差性质，前者是对自回归转移分布的\emph{局部单步近似}。
...
还必须区分\emph{无偏的数值估计}与\emph{无偏的梯度}。因为采样分布随参数变化，
\begin{equation}
...
\begin{align*}
 s_r^2 &= \frac{4\times0.5^2}{4}=0.25, & s_r &= 0.5.
\end{align*}
...
\begin{table}[htbp]
\centering
\caption{GRPO 词元概率比与裁剪计算示例}
\label{tab:grpo-clipping-example}
...
\begin{equation}
\begin{aligned}
 &\frac1G\sum_i\frac1{T_i}\sum_t L_{it}, \\
 &\frac{\sum_{i,t}L_{it}}{\sum_iT_i}, \\
...
\begin{warning}[过程奖励与格式解析器的投机陷阱]
若仅依赖表面规则（如提取最后一个数字的正则表达式）或缺乏逻辑约束的过程奖励，策略极易学会在长思维链中插入冗长虚假推导、重复生成格式标记或伪造验证结论（Reward Hacking）。形式上的奖励提升不代表逻辑推理能力增强，必须在测试阶段通过隔离环境中的精确执行器、形式证明器与多分支覆盖率进行独立核验。
\end{warning}
...
\end{tikzpicture}
\caption[可验证推理训练数据与策略循环]{可验证推理训练数据与策略循环。独立评估不向训练回流隐藏答案。}\label{fig:reason-grpo}
\end{figure}
...
\subsection{推理强度与硬预算}
\term{推理强度}{Reasoning Effort}{}是推理模型在一次请求中采用多少内部推理工作的离散控制量。它通常以低、中、高等档位暴露，但档位集合、默认值和实现含义由模型及接口版本决定；不同模型的同名档位对应各自的计算策略，资源开销需分别记录推理词元数、延迟和费用。提高档位常会增加推理词元、延迟与费用，也可能改善复杂任务的成功率，逐题单调改进则没有保证。

推理强度需要与三个容易混淆的控制量分开。输出上限是硬约束：以OpenAI Responses API为例，\texttt{max\_output\_tokens}同时约束内部推理词元和可见输出词元；可见答案的详略由输出详细度等参数控制；温度、核采样等参数控制采样分布。推理强度与候选数、搜索深度、工具调用次数、可见思维链是几类不同的控制量。闭源接口即使报告推理词元，也通常不会返回完整内部轨迹；开源模型的轨迹可见性则由模型输出协议和模板定义。
...
\term{自一致性}{Self-Consistency}{}对多条生成轨迹的答案进行聚合，常以规范化后的多数答案作为输出。\cite{wang2022selfconsistency} \term{多候选择优}{Best-of-N}{}则依赖评分器或验证器选择候选。多数票可能因系统性偏差集中于共同错误，择优评分器也可能对表面套路过拟合：前者度量生成策略在给定搜索预算下的\emph{解空间覆盖上限}（即 $\mathrm{pass@}k$），后者度量选择器在含噪候选集中的\emph{判别决策精度}。二者在评测口径上严格解耦。
...
\end{tikzpicture}
\caption[测试时计算生成与验证控制]{测试时计算生成与验证控制。以剩余预算约束继续生成与验证，输出质量由选择结果决定，候选数量只是中间量。}\label{fig:reason-budget}
\end{figure}
...
\end{equation}
工具输出作为\emph{环境观察}影响后续动作，本身不由策略采样产生；直接把工具返回文本加入策略损失，会把模型控制不了的内容归到策略名下。\keyconcept{工具调用返回的观察文本属于环境状态转移，其条件对数概率不得计入模型策略的自回归损失与梯度更新。}
...
\begin{note}[可验证推理训练的机制解耦与评估原则]
奖励函数规定策略优化的期望目标，验证器界定机器可判定的规范边界，采样策略决定模型在解空间中的探索广度，而优势归一化与重要性概率比裁剪决定反向传播的步长与方向。评估可验证推理能力时，必须严格解耦这四项机制，并在相同测试时计算预算与隔离题集上综合考察单次解答通过率、搜索覆盖上限与选择器判别精度。
\end{note}
