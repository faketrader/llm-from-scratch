...
\begin{table}[htbp]
\centering
\caption{分析开源大模型报告案例与基准版本}\label{tab:open-case-dates}
\begin{tabular}{lll}
...
\end{tabular}
\end{table}
表中日期统一采用技术报告首次提交时间，权重发布和产品上线另有各自的时间记录。带版本号的条目固定本章核对到的 arXiv 修订。Llama 4 没有单独的技术报告，其结构与许可以官方模型卡的固定提交为准，见本章相应小节。各报告的正式文献条目在对应案例中引用。

\section{模型分析对象}

\subsection{模型分析维度}
\term{稠密模型}{Dense Model}{}在一次前向中执行相应层的全部前馈参数，\term{稀疏专家模型}{Sparse Mixture-of-Experts Model}{MoE}则按路由选择部分专家。二者属于计算结构。Base、Instruct、Reasoning 等名称主要描述训练阶段或预期行为，后缀与注意力形式之间没有对应关系。一个推理蒸馏检查点可以保留学生模型的普通 GQA，而不继承教师的潜在注意力或专家结构。

输入和输出模态又是另一维度。文本检查点、视觉语言检查点和语音模型即使属于同一家族，也需要分别判断编码器、连接器和处理器。外部 API 可以把多个模型和工具包装为一个产品入口，其能力由底层模型、工具及执行流程共同形成；评估公开权重时需固定实际加载的模型和配套组件。

\subsection{开放程度分层}
\label{sec:open-openness}
“开源”在日常讨论中常被当成二值属性，工程上却至少包含四个可以分别核对的维度：能否取得权重、能否取得运行与训练代码、能否取得数据与训练配方、以及在什么许可下使用。各维度的开放情况可从模型仓库、代码仓库、数据说明和许可文本中核对。开放程度的四个可分别取证的维度如表\ref{tab:open-openness-levels}所示。

\begin{table}[htbp]
\centering
\caption{开源大模型开放程度四个取证维度}\label{tab:open-openness-levels}
...
开放代码、开放训练数据与托管服务可用同样需要分别核对。权重下载权限覆盖制品获取，具体用途的授权范围由对应版本的许可条文与实际使用方式共同确定。\footnote{“开放”在研究论文、模型社区和软件许可证讨论中可能采用不同口径。引用时应说明开放的是权重、代码、数据、配方还是服务访问，单写一个形容词无法传递这些区别。}

\subsection{统一结构描述}
记层数为 $L$，隐藏宽度为 $d$，查询头数为 $h_q$，键值头数为 $h_{kv}$，每头宽度为 $d_h$。专家数为 $E$，每词元激活数为 $k$，共享专家数为 $E_s$。缓存精度每元素占 $b_c$ 字节，总权重和激活权重参数量分别为 $P_{\mathrm{total}}$ 与 $P_{\mathrm{active}}$。核心比较维度如表\ref{tab:open-comparison-dimensions}所示。

\begin{table}[htbp]
\centering
\caption{大语言模型跨家族核心比较维度}\label{tab:open-comparison-dimensions}
...
\end{tabularx}
\end{table}
同一组字段覆盖不全所有结构。例如潜在注意力需要压缩维数，状态空间模型需要状态形状，混合架构需要逐层类型表。遇到不适用字段应解释原因，把所有模型一律还原为普通 MHA 会丢掉关键结构。

\subsection{模型制品比较}
\label{sec:open-fixed-comparison}
为把比较方法落到实物，下面选择三个文本制品。表中B表示十亿参数，参数量采用发布者口径；每参数两字节只用于计算统一精度下的逻辑权重体积，与下载文件大小、可运行显存属于不同口径。对稠密模型不另报稀疏意义的激活参数，因为嵌入查表与全部矩阵乘法的参与口径不同。

\begin{table}[htbp]
\centering
\small
\caption[典型开源模型架构结构与显存比较]{典型开源模型架构结构与显存比较。基于固定发布版本比较，GB 采用 $10^9$ 字节；状态按两字节、单序列、标准键值或潜在缓存计算，未计 MTP 附加状态与工作区。}\label{tab:open-actual-comparison}
...
该DeepSeek制品声明FP8块量化，表中的1342 GB因而只表示把报告参数按两字节展开的统一估算；实际存储还要按量化块、尺度、未量化层和附加预测模块核算。GLM的查询宽度 $96\times128=12288$ 不等于隐藏宽度4096，投影仍可合法地从4096映射到12288，$d=h_qd_h$ 的默认等式在该配置下不成立。
...
设单个位置隐藏向量为列向量 $\vect{h}_t\in\Real^d$，压缩矩阵 $\mat{D}\in\Real^{r\times d}$，得到潜在状态 $\vect{c}_t=\mat{D}\vect{h}_t\in\Real^r$。第 $i$ 个头的内容键和值写为
\begin{equation}\label{eq:open-latent}
 \begin{aligned}
  \vect{k}_{t,i}&=\mat{U}_i \vect{c}_t,\quad \mat{U}_i\in\Real^{d_k\times r},\\
  \vect{v}_{t,i}&=\mat{V}_i \vect{c}_t,\quad \mat{V}_i\in\Real^{d_v\times r}.
 \end{aligned}
\end{equation}
查询内容向量为 $\vect{q}_{t,i}\in\Real^{d_k}$。利用矩阵乘法结合律，某个历史位置 $j$ 的内容分数为
\begin{equation}
 \vect{q}_{t,i}^{\mathrm{T}}\vect{k}_{j,i}
 =\vect{q}_{t,i}^{\mathrm{T}}\mat{U}_i \vect{c}_j
 =(\mat{U}_i^{\mathrm{T}}\vect{q}_{t,i})^{\mathrm{T}}\vect{c}_j.
\end{equation}
因此可先把当前查询映射到 $r$ 维，再与缓存中的 $\vect{c}_j$ 相乘，而不逐一重建全部历史键。注意力权重 $a_{tj,i}$ 确定后，值的加权和同样满足
\begin{equation}
 \vect{o}_{t,i}=\sum_{j\le t}a_{tj,i}\mat{V}_i \vect{c}_j
 =\mat{V}_i\left(\sum_{j\le t}a_{tj,i}\vect{c}_j\right).
\end{equation}
...
\subsection{位置旋转分离}
若直接在完整键上施加旋转矩阵 $\mat{R}_j$，分数变成
\begin{equation}
 (\mat{R}_t\vect{q})^{\mathrm{T}}\mat{R}_j\mat{U}_i\vect{c}_j
 =\vect{q}^{\mathrm{T}}\mat{R}_t^{\mathrm{T}}\mat{R}_j\mat{U}_i\vect{c}_j.
\end{equation}
中间的相对旋转依赖历史位置 $j$，通常不能与 $\mat{U}_i$ 任意交换，预计算一个与 $j$ 无关的变换查询因而不可行。相关位置代数见\bookxref{ch:position-representation}。

MLA 采用独立的位置分量来保留旋转信息：将内容分数与位置分数相加。若共享旋转键为 $\vect{k}_j^R\in\Real^{d_R}$，每头旋转查询为 $\vect{q}_{t,i}^R$，则可写为
\begin{equation}
 s_{tj,i}=\frac{(\mat{U}_i^{\mathrm{T}}\vect{q}_{t,i})^{\mathrm{T}}\vect{c}_j+
 (\vect{q}_{t,i}^{R})^{\mathrm{T}}\vect{k}_j^R}{\sqrt{d_k+d_R}}.
\end{equation}
内容状态与位置状态分别缓存。该分工保留低秩内容路径的结合律，又给位置关系留下明确通道；具体查询压缩与投影布局仍应按所选模型版本读取。缓存分工如图\ref{fig:open-mla-split}所示。
...
\end{tikzpicture}
\caption[MLA 潜在内容与位置分量解耦机制]{MLA 潜在内容与位置分量解耦机制。多头潜在注意力中内容向量与携带 RoPE 的解耦键分工，省略当前查询路径以突出历史缓存结构。}\label{fig:open-mla-split}
\end{figure}
...
\subsection{V3 与 R1 的训练职责}
DeepSeek-V3 报告讨论专家负载控制、多词元预测及训练数值效率\cite{deepseek2024v3}。其中\term{多词元预测}{Multi-Token Prediction}{MTP}在预训练阶段增加若干个轻量级预测头，并行预测未来连续多个词元以增强序列梯度的密集度；该训练目标在推理阶段亦可作为自带的草稿头（Draft Head）配合推测解码加速生成。需明确区分训练时作为正则化损失与推理时作为投机加速两种不同角色的机制边界。
...
\begin{equation}\label{eq:open-v3-routing}
 \begin{aligned}
  S_t &= \operatorname{TopK}_i(s_{ti}+b_i), \\
  g_{ti} &= \frac{\mathbf1[i\in S_t]s_{ti}}{\sum_{j\in S_t}s_{tj}}.
 \end{aligned}
\end{equation}
偏置 $b_i$ 只改变选择集合；训练步结束后，对过载专家降低偏置，对欠载专家提高偏置。该偏置作用于专家选择，专家输出仍按原路由分数加权；偏置调整和均衡损失的梯度更新分别作用于选择状态和路由参数。报告仍保留很小的序列级辅助均衡项，训练中依然存在均衡损失，名称中的“无辅助损失”取的是以偏置为主要手段这一层含义\cite{deepseek2024v3}。实际配置还含节点分组限制和输出缩放，上式只展开选择与加权的关键分工；通用梯度与容量机制见\bookxref{ch:moe}。

DeepSeek-R1 则把重点放在推理行为的后训练与强化学习\cite{deepseek2025r1}。基础网络结构、奖励构造和推断时允许的计算预算是三个不同因素。更长的推理轨迹会增加生成成本，答案正确率由候选质量与验证、选择机制共同决定；奖励与验证器的机制在\bookxrefshort{ch:reinforcement-learning}至\bookxrefshort{ch:verifiable-reasoning}展开。蒸馏到其他基座的 R1 系列检查点按学生架构计算资源，MLA 缓存公式只适用于采用相应结构的模型。
...
令序列长度为 $N$、主干宽度为 $d$，编码器输出为 $\mat{H}^E\in\Real^{N\times d}$。对解码器层 $\ell$，报告中的全局缓存投影可以写成
\begin{equation}
 \begin{aligned}
  \mat{C}_\ell &= \mat{H}^E \mat{W}^{KV}_\ell, \quad \mat{W}^{KV}_\ell\in\Real^{d\times d_c}, \\
  \mat{Z}_\ell &= \mat{H}^E \mat{W}^Z_\ell, \quad \mat{W}^Z_\ell\in\Real^{d\times d_z}.
 \end{aligned}
\end{equation}
这里 $\mat{C}_\ell\in\Real^{N\times d_c}$ 为压缩前缓存条目，$\mat{Z}_\ell\in\Real^{N\times d_z}$ 为相应压缩权重表示，后续按压缩规则聚合；跨层复用还会减少实际独立缓存组。由于生成全局缓存不要求先算出每个解码器层的历史隐藏状态，大部分提示词元可以跳过完整的解码器计算。
...
\begin{equation}
 ...
 \end{aligned}
\end{equation}
该估算用于 $N\gg w$ 的长输入，忽略投影、通信和不同注意力算子的成本差异；例如 $N=64w$ 时比例为 $\frac{65}{128}$，这是计算量的比例，与端到端时延比例相差投影、通信与算子效率带来的开销。生成新词元时仍需经过编码器与解码器。多层局部注意力的理论感受野会随层数扩大，重放一个窗口属于近似恢复，报告中的小幅质量损失结论依赖其评测条件\cite{deepseek2026v41flash}。
...
\begin{equation}
 \begin{aligned}
  \vect{e}_t &= \operatorname{Concat}_{n,k}\bigl(E_{n,k}[h_{n,k}(g_{t,n})]\bigr), \\
  \widetilde{\vect{h}}_t &= \vect{h}_t+\vect{\alpha}_t\odot \mat{V} \vect{e}_t.
 \end{aligned}
\end{equation}
其中 $\vect{h}_t,\widetilde{\vect{h}}_t\in\Real^d$，若拼接宽度为 $D$，则 $\mat{V}\in\Real^{d\times D}$；门控 $\vect{\alpha}_t$ 由上下文状态与所读记忆共同计算，并按形状广播。该式只抽象寻址、投影和门控的职责，具体模型还包含归一化和多分支融合。固定片段阶数与哈希头数后，每词元访问的表项数不随表容量增长；实际访存时间仍受存储层次与访问模式影响。哈希碰撞、低频片段覆盖、分词方式和门控质量都会影响记忆是否有用。

V4.1 Flash采用两个Engram模块，分别位于从零计数的第1层和第14层，合计196B参数。每个模块使用2、3、4阶片段，每阶8个哈希头、合计2048维嵌入，每头表容量约1600万项，取不同素数；嵌入表及键值投影采用FP8。相较原始Engram，该版本省去短因果卷积，并调整嵌入优化方法。因此解释该制品时应以V4.1报告为准，早期论文的对应结构在此已有改动\cite{deepseek2026v41flash}。

表地址由词元确定，可以提前计算，并从主机内存预取\footnote{预取（prefetching），也称预读取。}，使传输与网络计算重叠；收益取决于带宽、批量与调度。仅按196B表参数每参数一字节估算，存储已约为196GB，另需尺度和运行状态。Engram保存训练得到的参数化记忆，RAG访问外部文档，KV缓存保存本次前缀的中间状态；三者的更新方式、来源可追溯性和存储生命周期各不相同。

\subsection{CSA2 的缓存压缩}
...
报告给出的全局缓存为每词元890字节，结合CSA2与FP4主缓存约为V4-Flash的四分之一；有界重放避免把局部窗口缓存持久化至SSD，使持久缓存约为前代的八分之一。全局缓存、运行期全部缓存和持久缓存具有不同统计口径，两个缩减比例相乘得不到有效数字。按一百万词元计算，全局缓存项约为890MB，实际服务还需局部缓存、权重、临时张量、分配器及并发请求空间。以上为发布者报告中的结构与资源结果，指定硬件、后端和任务上的容量测试仍需单独进行。

V4.1 Flash还采用Single-Pass mHC残差混合和DSpark推测解码，并从预训练阶段联合处理视觉与文本。各机制解决的成本不同：CED减少长输入的主干计算，CSA2减少全局缓存及重复索引，Engram扩大条件记忆容量，推测解码减少串行生成等待。能力变化还受预训练数据、后训练与推理预算影响，综合基准的提高无法归因到其中某一个模块。

\section{Qwen 模型体系}

\subsection{稠密模型与专家模型}
...
从机制上看，GQA 改变每个键值头服务多少查询头；查询键归一化约束相应向量尺度；专家层改变前馈参数的选择。这三者作用在不同位置，笼统概括为“采用某优化架构”得不到可核算的收益。

\subsection{推理模式的生成开销}
推理模式和直接回答模式可能采用不同的控制词元、模板开关或检查点，进而改变交互协议和生成过程。不同服务中名称相似的 API 参数也可能有不同含义，需核对序列化后的实际输入和返回协议。

设每个请求的隐藏推理或中间输出长度为 $T_r$，最终答案长度为 $T_a$。若两者均由同一个解码器生成，生成步数约为 $T_r+T_a$，容量规划需要覆盖隐藏输出。比较两个模式时，应分别记录正确率、有效完成率、总生成词元、延迟和失败类型。推理预算是实验条件，本身带着明确的成本。

\subsection{视觉语言分支的额外路径}
Qwen2.5-VL 是另一个明确的版本案例，其报告讨论视觉输入处理以及与语言主干的结合\cite{qwen2025vl}。视觉语言制品必须同时固定图像或视频预处理、视觉编码器、连接模块和语言分词器。文本主干的标称上下文窗口长度与实际可容纳的图像数量存在动态转换关系：视觉编码器根据图像原生分辨率与 Patch 切分策略，将单幅图像编码为动态数量的视觉词元（Visual Tokens）。
...
\subsection{代码模型分支}
Qwen2.5-Coder-7B-Instruct提供一个任务专用案例：固定配置仍是因果Transformer，28层、隐藏宽度3584、28个查询头和4个键值头，具有QKV偏置，输入输出嵌入不共享。它的专用性主要来自代码相关语料、任务构造和后训练，“Coder”后缀对应的正是这几项，并未引入新的注意力算子。\footnote{见官方\href{https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct/tree/c03e6d358207e414f1eca0bb1891e29f1db0e242}{固定制品配置与模型卡}；该卡报告7.61B参数、预训练与后训练阶段，声明Apache-2.0。代码语料及训练方法参见\href{https://arxiv.org/abs/2409.12186}{Qwen2.5-Coder技术报告}。}

代码生成的条件可以是自然语言需求，也可以是已有程序的前缀和后缀。对缺失中段 $m$，目标为 $p_\theta(m\mid c_{\mathrm{pre}},c_{\mathrm{suf}})$；只要把已知两侧放入模型规定的序列协议，因果解码器仍能逐步生成中段。这里的“因果”约束是序列化后的生成顺序，不要求条件只能来自源文件左侧。代码能否编译、测试能否通过以及是否满足需求则是不同评价层次，自然语言流畅度衡量不到它们。

配置还说明长上下文条件要落到具体字段上读：该快照的直接位置上限是32768，模型卡讨论借助YaRN处理更长输入。扩展配置、后端支持和短长任务评估必须一起保留，相关相位条件见\bookxref{ch:long-context}。

\section{Llama 模型体系}
Llama 系列早期检查点确立了开放权重解码器的常见配方：预归一化的 RMSNorm、旋转位置编码、门控前馈与分组查询注意力。这些机制此前已有研究，Llama 及其大量派生模型使其组合成为常见比较基线。模型家族识别需要读取逐层配置；分组查询注意力由多个家族共同采用。

Llama 4 是原生多模态模型，采用专家混合架构与早期融合，把图像与文本在输入阶段接入同一主干\cite{meta2025llama4modelcard}。该系列公开两个检查点：Scout 每词元激活约 $17$ 十亿参数、含16个专家，Maverick 每词元激活约 $17$ 十亿参数、含128个专家，二者上下文窗口均为128K。同代内专家数与计算量已经不同，前代稠密模型的部署经验因此不再适用：状态缓存、处理器依赖和模态预算都已改变。

该系列采用附带可接受使用政策的社区许可，授权范围由具体版本的许可文本确定。规模、分发、衍生训练与地域条款需要逐项核对。\footnote{结构、参数与许可依据\href{https://github.com/meta-llama/llama-models/blob/038acb9fe1e1ddc5cf1b3989fb07b311a0ffcae4/models/llama4/MODEL_CARD.md}{Llama 4模型卡固定提交}；仓库快照另见\href{https://huggingface.co/meta-llama/Llama-4-Scout-17B-16E/tree/14d516bdff6ac06cec40678529222f193386189c}{Scout制品提交}。核对日期为2026年9月12日。“17B”指每词元激活参数，总参数采用另一口径。}

\section{Mistral 模型体系}
Mistral 7B 用滑动窗口注意力限制每个查询可见的历史范围\cite{mistral2023sevenb}。设窗口宽度为 $W$，第 $\ell$ 层的有效感受野 $R_\ell$ 满足
\begin{equation}
 \begin{aligned}
  R_1 &= W, \\
  R_\ell &= R_{\ell-1}+(W-1) \;\Longrightarrow\; R_\ell=1+\ell(W-1).
 \end{aligned}
\end{equation}
...
Mixtral 8x7B 把门控前馈替换为每个词元选择 2 个专家的稀疏前馈，报告的总参数约 $46.7$ 十亿，每词元激活约 $12.9$ 十亿\cite{jiang2024mixtral}。这正是\bookxref{ch:moe}讨论的容量与计算分离：激活参数量描述单次前向涉及的参数，权重驻留仍按总参数量核算。

Mixtral 8x7B 的公开配置把滑动窗口设为空值，采用完整因果注意力；Mistral 7B 的该版本则使用滑动窗口。因此，同一家族的注意力与位置设置也需逐检查点核对。这里的两个制品均以宽松开源许可发布，后续型号的许可与开放范围以各自制品为准。\footnote{配置字段与许可依据固定提交：\href{https://huggingface.co/mistralai/Mistral-7B-v0.1/tree/27d67f1b5f57dc0953326b2601d68371d40ea8da}{Mistral-7B-v0.1}（滑动窗口4096、32查询头、8键值头）与\href{https://huggingface.co/mistralai/Mixtral-8x7B-v0.1/tree/fc7ac94680e38d7348cfa806e51218e6273104b0}{Mixtral-8x7B-v0.1}（滑动窗口为空、8专家选2）。核对日期为2026年9月12日。}

\begin{table}[htbp]
\centering
\small
\caption[Llama 与 Mistral 架构差异对比]{Llama 与 Mistral 架构差异对比。对比两条开放权重路线在注意力、前馈结构与许可条件上的差异，参数规模采用发布报告口径。}\label{tab:open-llama-mistral}
...
\subsection{现代推理与工具能力的训练}
GLM-4.5 报告采用 MoE，并围绕智能体、推理和代码任务组织训练，提供推理与非推理模式\cite{glm2025glm45}。它是现代模型案例，其注意力掩码和空白填充协议与早期论文的差异需要按报告逐项核对。

工具调用能力至少涉及三层：网络输出调用意图，序列协议编码工具名与参数，\emph{外部执行器检查权限并产生观察}。参数模式、工具语义与执行授权由外部执行器分别检查。训练集中的调用与观察必须保持关联，相关系统设计在智能体架构与应用开发章展开。

评估智能体能力时，工具环境、任务终止条件、重试预算与评判器都属于实验定义。同一模型在不同环境中可能得到不同完成率，技术报告中的智能体分数随环境变化，作为通用性能常数使用会失真。

\subsection{视觉分支的输入处理}
GLM-4.5V是独立的视觉语言制品，其模型卡指向GLM-4.5-Air基座，并非任意GLM-4.5权重都能接收图像。固定配置中，视觉编码器深度24、宽度1536，输出映射到4096维语言空间；空间patch宽度14、空间合并因子2，另有时间patch宽度2。\footnote{见\href{https://huggingface.co/zai-org/GLM-4.5V/tree/ed47433b37111465ec527affaaddceff371bca04}{GLM-4.5V固定模型卡及配置}。这些是该提交的结构字段，不表示所有输入固定为同一分辨率；实际网格必须连同图像／视频处理器读取。}

\begin{example}[视觉网格的空间合并]
...
该制品为图像、视频分别设置标记，并配置多维位置分配。即使语言层数与Air相同，视觉处理器、位置协议和停止编号也成为新的兼容条件。其图像推理、文档理解和界面操作能力应在相应输入及工具环境中评估；视觉编码器负责对象表示，外部执行器按独立的授权规则执行操作。多模态融合及视觉序列推导分别见\bookxref{ch:vision-transformer}和多模态篇。
\end{example}
...
\begin{equation}
 \begin{aligned}
  \mat{S}_t &= \mat{S}_{t-1}+\vect{k}_t\vect{v}_t^{\mathrm{T}}, \quad \mat{S}_t\in\Real^{d_k\times d_v}, \\
  \vect{y}_t &= \mat{S}_t^{\mathrm{T}}\vect{q}_t.
 \end{aligned}
\end{equation}
其状态大小不随历史长度增长，代价则是历史被压缩进有限矩阵，任意过去词元无法逐项访问。该式描述线性累积原理；MiniMax 的具体实现还包含门控、归一化等计算环节。

将部分层保留为完整注意力，另一部分采用递推状态，会形成不同的容量与信息访问折中。若完整注意力层数为 $L_a$，状态层数为 $L_s$，缓存近似为
\begin{equation}
 M(T)=b_c\bigl(2L_aT h_{kv}d_h+L_s d_kd_v\bigr),
\end{equation}
其中暂按每个状态层只有一个上述矩阵计算；多头和附加状态需再乘相应系数。当 $L_a>0$ 时，全局全注意力层依然随序列长度 $T$ 产生线性增长的键值状态；混合注意力架构显著降低了每词元缓存的增长斜率，但在超长序列下显存占用仍非严格常数。
...
Gemma 4 采用宽松开源许可\cite{gemmateam2026gemma4}，而该家族早期版本使用自有条款。这说明同一品牌的不同代际必须分别取证：宽松许可不回溯适用于旧检查点，旧条款也不适用于新版本。该系列同时提供稠密与专家版本，注意力在局部滑动窗口与全局之间交替，并规定末层为全局层；以26B A4B为例，它含128个专家、每词元选择8个并另有1个共享专家，激活参数远小于总参数。窗口宽度在不同尺寸上取512或1024，因此部署参数必须按选定检查点读取。
...
\centering
\small
\caption[宽松许可开源模型架构参数对比]{宽松许可开源模型架构参数对比。对比 OLMo、Gemma 与 gpt-oss 的核心架构字段，配置数值取自各制品的固定提交版本。}\label{tab:open-permissive-comparison}
...
Falcon、Pythia、InternLM、ERNIE 及其他开放权重模型可以沿同一方法纳入研究：先选择具体报告和检查点，再确定它带来的新增比较维度，例如领域语料、训练目标、异构专家或模态处理。没有足够来源时应保留为待研究对象，推测规格留待补齐材料后填写。各开源模型家族机制创新与核心议题映射如表\ref{tab:open-case-topics}所示，主流开源大模型旗舰版本核心超参数汇总如表\ref{tab:open-flagships}所示。

\begin{table}[htbp]
\centering
\caption{开源模型家族机制创新与核心议题}\label{tab:open-case-topics}
\begin{tabularx}{\linewidth}{p{27mm}X}
\toprule 模型案例 & 主要比较问题\\\midrule
DeepSeek-V2/V3/R1 与 V4.1 Flash & 缓存压缩、专家计算、条件记忆与后训练的分工\\
Qwen3 / Qwen2.5-VL & 同代结构变体、交互模式与视觉路径\\
...
\begin{table}[htbp]
\centering
\caption{主流开源大模型旗舰版本架构参数}\label{tab:open-flagships}
\begin{tabularx}{\linewidth}{lXXXX}
\toprule
超参数与规格 & Llama-3.1-405B & Qwen-2.5-72B & DeepSeek-V3 & Mistral-Large-2\\\midrule
层数 / 隐藏宽度 & $126$ / $16384$ & $80$ / $8192$ & $61$ / $7168$ & $88$ / $12288$\\
注意力结构 & GQA ($128\text{Q}/8\text{KV}$) & GQA ($64\text{Q}/8\text{KV}$) & MLA ($128\text{Q}$, $r=512$) & GQA ($96\text{Q}/8\text{KV}$)\\
中间层 FFN 维数 & $53248$（稠密） & $29568$（稠密） & $256$选$8$（中间维$2048$） & $36864$（稠密）\\
总参数 / 激活参数 & $405\text{B} / 405\text{B}$ & $72.7\text{B} / 72.7\text{B}$ & $671\text{B} / 37\text{B}$ & $123\text{B} / 123\text{B}$\\
词表大小 / RoPE $\theta$ & $128\text{K}$ / $500\text{K}$ & $152\text{K}$ / $1\text{M}$ & $129\text{K}$ / $10\text{K}$ & $32\text{K}$ / $1\text{M}$\\
原生上下文窗口 & $128\text{K}$ & $128\text{K}$ & $128\text{K}$ & $128\text{K}$\\
预训练 Token 规模 & $\num{15000000000000}+$ & $\num{18000000000000}$ & $\num{14800000000000}$ & 未公开\\
开源许可证 & Llama 3.1 社区许可 & Apache 2.0 & DeepSeek 模型许可 & 商业与研究许可\\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{模型制品的构成}
模型卡提供发布者声明；配置提供机器可读结构；权重张量提供实际存储事实；模型代码定义如何执行这些张量。四者相互补充，判断整套制品是否可用需要合看。

例如配置声明嵌入为 $V\times d$，实际权重却为 $(V+8)\times d$，可能是新增控制词元，也可能是版本错配。应同时读取分词器的有效编号范围和输出投影，差异既可能来自新增控制词元，也可能来自版本错配，判断取决于这些字段是否自洽。对于连续编号，最大词元编号必须小于实际嵌入行数；存在填充词表行时，行数又可能大于有效词元数。

权重分片索引需要把张量映射到实际文件，文件头中的形状、数据类型与偏移必须相容。普通未量化浮点张量的参数量直接等于张量各维度尺寸之乘积；而在低比特量化（如 INT4、FP8 或混合量化）格式下，权重张量经过位打包（Bit-Packing）存储，并附带分块缩放因子（Scales）与零点偏置（Zeros）。因此，显存中实际占用的物理字节数需按量化位宽与附加元数据综合折算，不能直接将压缩存储单元数等同于逻辑参数量。

\subsection{模型加载条件}
\term{模型适配器}{Model Adapter}{}与指定基座参数和目标模块共同组成可执行模型。低秩增量、量化权重、多模态处理器和派生容器都需要保留来源关系。修改基座版本或模块命名后，即使加载程序没有立刻报错，数学组合也可能已改变。

Safetensors 的元数据读取与需要通用对象反序列化的格式不同，但来源可信性通过发布身份与完整性检查确定。静态解析仍需限制头部大小、验证偏移范围和文件完整性。未知模型仓库要求执行自定义代码时，应把代码纳入明确的依赖与审阅范围，其可信性依据代码来源、完整性和审阅结果判定。

\subsection{模型制品清单的构建}
\begin{algorithm}[建立模型结构与协议清单]
\label{alg:open-manifest}
\AlgoInput{固定来源和版本的模型文件、声明文档、允许读取的格式与结构规则。}
\AlgoOutput{证据清单、确定冲突、仍不可判定的项目。}
\begin{enumerate}
...
\subsection{模型选型约束}
先明确输入模态、部署地点、允许用途、可用显存与时延上限，再在可行候选中比较质量。开放范围与许可在这一步就是准入条件，不宜推迟到上线前作为合规检查处理。资源模型至少写成
\begin{equation}
 M_{\mathrm{peak}}=M_{\mathrm{weights}}+M_{\mathrm{state}}+
 M_{\mathrm{workspace}}+M_{\mathrm{runtime}},
\end{equation}
其中峰值工作区随批量、上下文和后端变化。MoE 没有卸载或分片时，权重项仍按总参数计算，按激活参数量估算全部驻留显存会严重低估。
...
\subsection{模型适配的兼容性}
统一业务消息格式有助于上层应用维护，但不同模型仍需要各自的序列化适配。适配器应负责消息角色、工具参数结构、媒体位置、结束标记和错误恢复，同一串提示文本无法通用于所有模型。共同业务语义对应的是不同词元序列。
