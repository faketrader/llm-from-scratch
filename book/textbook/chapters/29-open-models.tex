...
\begin{table}[htbp]
\centering
\caption{本章分析的开源大模型技术报告案例与文献基准版本}
\label{tab:open-case-dates}
\begin{tabular}{lll}
...
\begin{table}[htbp]
\centering
\caption{开放程度的四个可分别取证的维度}
\label{tab:open-openness-levels}
...
\begin{table}[htbp]
\centering
\caption{大语言模型跨架构跨家族核心比较维度与技术影响}
\label{tab:open-comparison-dimensions}
...
\centering
\small
\caption{固定版本的结构、目标与资源比较（GB为$10^9$字节；状态按两字节、单序列、标准键值或潜在缓存计算，未计MTP附加状态、分页和工作区）}
\label{tab:open-actual-comparison}
...
\begin{equation}\label{eq:open-latent}
 \vect{k}_{t,i}=\mat{U}_i \vect{c}_t,\qquad \vect{v}_{t,i}=\mat{V}_i \vect{c}_t,
 \quad \mat{U}_i\in\Real^{d_k\times r},\quad \mat{V}_i\in\Real^{d_v\times r}.
\end{equation}
...
\end{tikzpicture}
\caption{潜在内容与位置分量的分工。图省略当前查询路径和输出投影，突出历史缓存的身份。}
\label{fig:open-mla-split}
\end{figure}
...
DeepSeek-V3 报告讨论专家负载控制、多词元预测及训练数值效率\cite{deepseek2024v3}。其中\term{多词元预测}{Multi-Token Prediction}{MTP}是在训练中增加未来词元的预测约束，与在服务端实现某种推测解码是两件事。训练目标、辅助模块是否保留、部署如何利用它们，需要分别说明。
...
\end{equation}
偏置 $b_i$ 只改变选择集合；训练步结束后，对过载专家降低偏置，对欠载专家提高偏置。该偏置不乘入专家输出，也不等同于用均衡损失梯度修改路由器。报告仍保留很小的序列级辅助均衡项，训练中依然存在均衡损失，名称中的“无辅助损失”取的是以偏置为主要手段这一层含义\cite{deepseek2024v3}。实际配置还含节点分组限制和输出缩放，上式只展开选择与加权的关键分工；通用梯度与容量机制见\bookxref{ch:rev-moe}。
...
\begin{equation}
 \mat{C}_\ell=\mat{H}^E \mat{W}^{KV}_\ell,\qquad \mat{Z}_\ell=\mat{H}^E \mat{W}^Z_\ell,
 \quad \mat{W}^{KV}_\ell\in\Real^{d\times d_c},\quad
 \mat{W}^Z_\ell\in\Real^{d\times d_z}.
\end{equation}
...
\begin{equation}
 ...
 \qquad \widetilde{\vect{h}}_t=\vect{h}_t+\vect{\alpha}_t\odot \mat{V} \vect{e}_t.
\end{equation}
...
\subsection{推理模式与输出预算}
推理模式和直接回答模式改变模型的交互协议及生成过程，可能使用控制词元、模板开关或不同检查点。给 API 传入一个名称相似的参数与目标服务实现相同语义是两回事；必须核对序列化后的实际输入和返回协议。
...
Qwen2.5-VL 是另一个明确的版本案例，其报告讨论视觉输入处理以及与语言主干的结合\cite{qwen2025vl}。视觉语言制品必须同时固定图像或视频预处理、视觉编码器、连接模块和语言分词器。文本主干的上下文上限与可输入的图片数量是两个不同的量，因为每幅图像可能转化为不同数量的视觉位置。
...
\section{Llama 与 Mistral 模型体系}

Llama 与 Mistral 是英文社区中使用最广的两条开放权重路线。它们与前述模型家族在结构上有重叠，也有明确差异；与 DeepSeek、Qwen 并列分析可以看出，“开放权重”覆盖的是结构各异的多个制品。
...
值得注意，Mixtral 8x7B 的公开配置把滑动窗口设为空值，注意力仍是完整因果形式；滑动窗口属于 Mistral 7B 该版本的选择，同家族的专家型号并未沿用这一设置。同一家族的注意力与位置设置需要逐检查点读取，两个制品均以宽松开源许可发布，但其后续型号的许可与开放范围需要另行核对。\footnote{配置字段与许可依据固定提交：\href{https://huggingface.co/mistralai/Mistral-7B-v0.1/tree/27d67f1b5f57dc0953326b2601d68371d40ea8da}{Mistral-7B-v0.1}（滑动窗口4096、32查询头、8键值头）与\href{https://huggingface.co/mistralai/Mixtral-8x7B-v0.1/tree/fc7ac94680e38d7348cfa806e51218e6273104b0}{Mixtral-8x7B-v0.1}（滑动窗口为空、8专家选2）。核对日期为2026年9月12日。}

\begin{table}[htbp]
\centering
\small
\caption{两条开放权重路线（Llama 与 Mistral 系列）的结构差异（参数规模采用报告口径；能力评价需另用统一任务与预算）}
\label{tab:open-llama-mistral}
...
\begin{equation}
 \mat{S}_t=\mat{S}_{t-1}+\vect{k}_t\vect{v}_t^{\mathrm{T}},\qquad
 \vect{y}_t=\mat{S}_t^{\mathrm{T}}\vect{q}_t,
 \quad \mat{S}_t\in\Real^{d_k\times d_v}.
\end{equation}
其状态大小不随历史长度增长，代价则是历史被压缩进有限矩阵，任意过去词元无法逐项访问。该简化式只说明线性累积原理，不等同于 MiniMax 的完整门控、归一化或实现公式。

将部分层保留为完整注意力，另一部分采用递推状态，会形成不同的容量与信息访问折中。若完整注意力层数为 $L_a$，状态层数为 $L_s$，缓存近似为
\begin{equation}
 M(T)=b_c\bigl(2L_aT h_{kv}d_h+L_s d_kd_v\bigr),
\end{equation}
其中暂按每个状态层只有一个上述矩阵计算；多头和附加状态需再乘相应系数。只要 $L_a>0$，整体缓存仍有随 $T$ 线性增长的部分，混合模型的缓存并非对任意长度零增长。
...
\centering
\small
\caption{三个宽松许可开放权重制品的字段比较（参数与许可采用发布者口径，配置数值取自各制品的固定提交；能力评价需另用统一任务与预算）}
\label{tab:open-permissive-comparison}
...
Falcon、Pythia、InternLM、ERNIE 及其他开放权重模型可以沿同一方法纳入研究：先选择具体报告和检查点，再确定它带来的新增比较维度，例如领域语料、训练目标、异构专家或模态处理。没有足够来源时应保留为待研究对象，推测规格留待补齐材料后填写。各开源模型家族机制创新与核心议题映射如表\ref{tab:open-case-topics}所示，主流开源大模型旗舰版本核心超参数汇总如表\ref{tab:rev-open-flagships}所示。

\begin{table}[htbp]
\centering
\caption{开源模型家族机制创新与核心比较议题映射}
\label{tab:open-case-topics}
...
\begin{table}[htbp]
\centering
\caption{主流开源大模型旗舰版本核心架构参数与规格一览}
\label{tab:rev-open-flagships}
...
权重分片索引需要把张量映射到实际文件，文件头中的形状、数据类型与偏移必须相容。普通未量化矩阵的参数数为各维长度之积；量化打包张量还包含尺度和其他状态，其存储元素数与原网络参数量是两个不同的量。

\subsection{格式、适配器与远程代码}
\term{模型适配器}{Model Adapter}{}依赖基座参数和目标模块，单独解释并不构成一个完整模型。低秩增量、量化权重、多模态处理器和派生容器都需要保留来源关系。修改基座版本或模块命名后，即使加载程序没有立刻报错，数学组合也可能已改变。

Safetensors 的元数据读取与需要通用对象反序列化的格式不同，但来源可信性通过发布身份与完整性检查确定。静态解析仍需限制头部大小、验证偏移范围和文件完整性。未知模型仓库要求执行自定义代码时，应把代码纳入明确的依赖与审阅范围，自动加载接口提供的开关并不构成可信性判据。
