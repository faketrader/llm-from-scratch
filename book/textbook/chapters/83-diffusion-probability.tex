...
\State Draw a clean sample $\vect{x}_0$ and apply fixed preprocessing; independently draw time $t$ and standard Gaussian noise $\vect{\epsilon}$
\State Construct $\vect{x}_t=a_t\vect{x}_0+b_t\vect{\epsilon}$ directly from the cumulative schedule without running the complete forward chain
\State Feed $\vect{x}_t$ with the specified time conditioning; construct target $\vect{\epsilon}$, $\vect{x}_0$, or $a_t\vect{\epsilon}-b_t\vect{x}_0$ according to the prediction type
\State Compute weights from the selected target and time-sampling rule, then backpropagate and update parameters; complete variational training also includes the corresponding endpoint and variance terms
\State After the prescribed training-step count or an explicitly defined stopping condition, emit the weights together with the schedule, prediction type, and data-scale configuration
\end{myalgorithmic}
...
\AlgoOutput{生成样本。}
\begin{myalgorithmic}[1]
\State Validate that the time grid is strictly decreasing and that the schedule and prediction type are compatible; fix the inference configuration
\State Draw $\vect{x}_T$ from the specified terminal prior
\For{each adjacent pair $(t,s)$ in the finite time grid, with $t>s\ge0$}
  \State Feed $\vect{x}_t,t$ to the network and convert its output into the noise or clean-sample estimate required by the update rule
  \State Compute the update mean and nonnegative variance from the coefficients for $(t,s)$
  \If{the stochastic sampling rule is active and the current transition requires noise}
    \State Draw fresh independent noise and update $\vect{x}_s$ using the specified variance
  \Else
    \State Update $\vect{x}_s$ using the deterministic or endpoint rule
  \EndIf
\EndFor
\State \Return the generated sample after inverse preprocessing or downstream decoding
\end{myalgorithmic}
