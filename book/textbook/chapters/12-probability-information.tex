...
本章快速复习后续内容反复使用的数学对象。向量承载特征，矩阵实现批量线性变换，梯度描述标量目标对参数的局部变化；概率则规定语言模型输出的含义。读者只需重新建立符号、形状和概念之间的联系，不需要在此完成一门独立的数学课程。随后将这些对象用于推导\term{最大似然估计}{Maximum Likelihood Estimation}{MLE}、\term{熵}{Entropy}{}、\term{交叉熵}{Cross-Entropy}{CE}和\term{库尔贝克—莱布勒散度}{Kullback--Leibler Divergence}{KL}。

\begin{bookassumptions}
词表取有限集合；除明确讨论连续情形外，概率均为离散质量。涉及条件概率的条件事件具有正概率；需要有限对数损失时，预测分布在目标支持集上严格为正。样本独立同分布只在相应估计推导中使用，不作为所有文本片段天然满足的事实。
\end{bookassumptions}
...
\begin{equation}\label{eq:rev-conditional}
 ...
 p_X(x)=\sum_y p(x,y).
\end{equation}
条件化把样本空间限制到已知条件，并重新归一化。它不是删除条件变量，也不是默认条件与结果具有因果关系。网页内容与某个答案同时出现，可以形成统计相关；这并不证明网页内容导致答案所述事件发生。
...
\phantomsection\label{q:ch2:example:01}
...
这里的数值是用于说明条件概率的设定值，不是验证器性能实测。实际系统必须从合适样本中估计这些概率，并进一步考虑验证器与候选生成器的相关性和分布变化。
...
若每个条件分布均归一化，固定长度联合分布也归一化。证明从最后一个位置开始求和：
\begin{align}
...
\end{equation}
经验平均可以严格写成经验分布下的期望。但经验分布并不等于真实分布：未观察到某个取值，不能证明它在总体中的概率为零。后续的训练目标正是由这个可计算的经验平均构成。
...
KL 散度的非负性可以由基本不等式 $-\log u\ge1-u$ 推出。假设在 $p$ 的支持上 $q$ 为正，则
\begin{align}
 D_{\mathrm{KL}}(p\|q)
...
\end{align}
等号成立要求支持上的比值均为一，且 $q$ 没有把额外概率放到支持之外，即 $p=q$。若 $q$ 在 $p$ 的某个正概率位置为零，散度为正无穷，非负性仍成立。由此可以进一步证明，在 $C$ 个取值上，$H(p)\le\log C$：令 $u(x)=\frac{1}{C}$，则 $D_{\mathrm{KL}}(p\|u)=\log C-H(p)\ge0$。最大熵由均匀分布取得。
...
\begin{equation}
 ...
 \approx0.1438.
\end{equation}
二者不同，说明 KL 散度一般不对称。它也不满足距离所需的三角不等式，因此本书不将其称为通常意义的距离。训练和蒸馏中的 KL 项必须说明哪个分布提供采样权重；改变方向便改变了目标。
