...
\end{equation}
$r_t$ 在执行 $a_t$ 后产生，$H$ 为终止步数。从初始状态依策略逐步采样并与环境交互直至终止的这次完整执行，常称为\term{策略轨迹采样}{Rollout}{}\footnote{在语言模型与智能体文献中，rollout 通常指策略从给定提示或初始状态出发，持续生成或与环境交互直到终止的一次完整尝试；对同一提示重复采样可得到多条候选轨迹。产品发布语境中的 model rollout 指模型逐步发布，不属于本章所述含义。}，亦简称“轨迹采样”；其记录结果就是一条轨迹。这里的奖励下标对应当前动作，部分文献将同一量记为 $R_{t+1}$；采用其中一套约定时须全程一致。\footnote{本章区分即时奖励 reward 与累计回报 return。中文资料偶尔都称“奖励”，推导时应以时间下标和求和定义识别具体对象。}
...
结束标记是合法动作，会进入终止状态。提示词元由任务分布提供，不由当前策略采样；工具返回文本由环境产生，其概率分布由外部环境转移驱动，\emph{不计入当前策略的似然优化与梯度求导}。若动作定义为单次结构化工具调用，则动作空间、时间尺度与成本单位均发生变化，调用级与词元级的折扣因子需要分别说明。
...
\begin{table}[htbp]
\centering
\caption{强化学习与策略梯度主要符号}
\label{tab:rl-symbols}
\begin{tabular}{ll}
...
\subsection{价值函数递推}
定义
\begin{equation}\label{eq:rl-return}
\begin{aligned}
 G_t &= \sum_{k=t}^{H-1}\gamma^{k-t}r_k, \\
 G_t &= r_t+\gamma G_{t+1}, \qquad G_H=0.
\end{aligned}
\end{equation}
...
\end{tikzpicture}
\caption[两步马尔可夫决策过程示例]{两步马尔可夫决策过程示例。所有第一步奖励为零，第二步奖励给出完整回报，用于精确计算价值与策略梯度。}\label{fig:rl-two-step}
\end{figure}
...
\end{equation*}
对于 $\psi$，仅访问 $s_L$ 的轨迹贡献梯度：成功得分 $1-q=\frac{1}{4}$，失败得分 $-q=-\frac{3}{4}$，所以 $\tfrac38\times4\times\tfrac14=0.375$。结果与前例直接求导所得梯度一致。
...
当 $\lambda=0$ 时只保留一步 TD；当 $\lambda=1$ 且轨迹真实终止、末状态价值为零时，得到 $G_t-V_{\vect{\phi}}(s_t)$。后一形式对策略梯度仍具有状态基线的无偏性质；较小 $\lambda$ 在价值近似不准确时可能引入偏差，但通常减少远期随机性。若使用精确 $V^\pi$，相应在策略条件下各 $n$ 步估计的条件期望一致。表\ref{tab:advantage-comparison}对比了语言模型策略优化中主流优势估计与更新范式的机制。
...
\caption{策略优化算法机制对比}
\label{tab:advantage-comparison}
...
\begin{align*}
 \delta_0 &= 0+2.8-2.4=0.4, & \delta_1 &= 4-2.8=1.2, \\
 \widehat A_1 &= 1.2, & \widehat A_0 &= 0.4+0.8\times1.2=1.36.
\end{align*}
完整回报减基线在初始状态为 $4-2.4=1.6$。差异来自 $\lambda$ 对后续残差的衰减。成功轨迹的优势估计使用一次采样结果，精确优势0.5则对第二步动作取了期望。
\end{example}

\subsection{终止与截断}
任务终止后没有未来回报，终止状态的价值设为零。若只是采样批次或计算窗口结束，而任务仍会继续，应在最后状态自举。达到输出预算时，也应按任务定义处理：被定义为失败终止时使用终止规则，仅截断采样时保留自举。

实践中需要分别标识“允许从下一状态自举”和“允许沿当前缓冲继续传播残差”。数据切片末尾可以保留下一状态价值，跨到另一个样本的残差则不允许。将截断标记与终止标记简单合并为一个布尔变量（如 `done`），会将截断处的人为中断错误归因为环境终止，破坏值函数自举与广义优势估计的无偏性。

\keyconcept{强化学习轨迹记录中必须严格区分“环境自然终止”（Termination）与“人为窗口截断”（Truncation）：前者未来回报为零（无需自举），而后者未来仍有潜在期望回报，必须利用价值网络进行自举（Bootstrapping），混淆二者将破坏优势估计的无偏性并诱发过早放弃等退化行为。}

\begin{warning}[人为截断与环境终止的混淆陷阱]
在语言模型长文本采样或多步交互中，达到最大生成长度限制属于“时间截断”而非“任务失败终止”。若在计算 TD 残差或 GAE 时将截断状态的未来价值错误置零，将对需要长链路推导的复杂样本施加严重的人为惩罚，导致策略过早输出终止符或拒绝深度推理。
\end{warning}
...
\begin{equation}
\begin{aligned}
 L_{\mathrm{sur}}(\vect{\theta}) &= \mathbb E_{(s_t,a_t)\sim\mu} [\rho_t(\vect{\theta})\widehat A_t], \\
...
\end{tikzpicture}
\caption[PPO 单样本裁剪目标函数曲线]{PPO 单样本裁剪目标函数曲线。裁剪在有利方向上截断增益，概率比本身仍可越出区间。}\label{fig:rl-clip}
\end{figure}
...
\end{tikzpicture}
\caption[策略强化学习系统数据流]{策略强化学习系统数据流。行为策略产生数据，价值模型估计回报，当前策略执行更新，参考策略提供可选约束。更新后须明确何时刷新采样策略。}\label{fig:rl-training-flow}
\end{figure}
...
\begin{note}[策略优化有效性与奖励函数规范性]
策略梯度与优势估计仅确保参数朝最大化当前设计回报函数的方向收敛。奖励函数的完备性、验证器的防作弊能力以及提示分布的覆盖度，决定该标量信号是否真实对齐实际任务需求。策略训练必须配合保留集上的开环生成与客观规则评估，以防发生奖励黑客攻击（Reward Hacking）。
\end{note}

\section{奖励与验证边界}

采样策略、参考概率、裁剪和优势应统一到决策过程与概率比中解释；组相对目标由\bookxref{ch:verifiable-reasoning}展开。终点奖励、步骤证据和轨迹预算承担不同职责：验证器提供外部标量奖惩信号，价值网络建模未来期望累积回报，二者分别构成强化学习的环境反馈与内部批判机制。策略改进通过实际交互中的回报与任务完成率评估。
