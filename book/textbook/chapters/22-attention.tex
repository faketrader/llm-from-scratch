...
\begin{equation}\label{eq:attn-projections}
 \begin{aligned}
  \mat{Q}&=\mat{X}_q\mat{W}_Q,\\
  \mat{K}&=\mat{X}_m\mat{W}_K,\\
  \mat{V}&=\mat{X}_m\mat{W}_V,
 \end{aligned}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{注意力机制核心数学符号与张量形状}\label{tab:attn-notations}
...
\end{assumption}
在此条件下，对 $s=\sum_{r=1}^{d_k}\vect{q}_r\vect{k}_r$，有
\begin{align}
...
\begin{equation}\label{eq:attn-basic}
 \begin{aligned}
  A_{ij}&=\frac{\exp U_{ij}}{\sum_{r=1}^{S}\exp U_{ir}},\\
  \mat{O}&=\mat{A}\mat{V}.
 \end{aligned}
\end{equation}
...
多种硬约束共同存在时，应取\emph{允许集合的交集}。例如位置既需要满足 $j\leq i$，又必须是有效词元、来自允许的文档。等价地，各种禁止条件取并集。将多篇文档拼接成一个序列时，分隔符本身不禁止跨文档读取；是否隔离仍取决于掩码。

\begin{figure}[htbp]
\centering
\includegraphics[width=.92\linewidth]{\subfix{../../figures/theory/attention-visibility-patterns.pdf}}
\caption[注意力可见性模式对比]{注意力可见性模式对比。灰格表示允许读取。右图把前两个与后两个位置视为不同文档，每篇内部保持因果性。}\label{fig:attn-masks}
\end{figure}
...
\State 校验张量形状、键值位置数与数值有限性；令 $\mat{O}\gets0$，若需要则令 $\mat{A}\gets0$
\For{query $i=1,\ldots,T$}
  \If{$b_i=0$}
    \State 保持输出行为零向量，continue 到下一个查询
  \Else
    \If{$\mathcal J_i=\varnothing$}
      \State 报告可见性无效并终止
    \EndIf
    \For{$j\in\mathcal J_i$}
      \State $u_j\gets\frac{\vect{q}_i \vect{k}_j^{\mathrm{T}}}{\sqrt{d_k}}$；若结果非有限则报数值错误终止
    \EndFor
    \State $c\gets\max_{j\in\mathcal J_i}u_j$，$e_j\gets\exp(u_j-c)$，且 $z\gets\sum_{j\in\mathcal J_i}e_j$
    \For{$j\in\mathcal J_i$}
      \State $a_j\gets\frac{e_j}{z}$ 且 $\vect{o}_i\gets \vect{o}_i+a_j\vect{v}_j$；若需要权重则置 $A_{ij}\gets a_j$
    \EndFor
  \EndIf
\EndFor
\State \Return $\mat{O}$（若请求则包含 $\mat{A}$）
\end{myalgorithmic}
...
\caption[单头注意力查询加权汇总]{单头注意力查询加权汇总。点的位置由给定矩阵计算，表示真实二维值空间中的凸组合，不是训练测量。}\label{fig:attn-convex-combination}
\end{figure}
...
\begin{align}
 ...
 \mat{C}&=\operatorname{Concat}_{\mathrm{feature}}(\mat{O}^{(1)},\ldots,\mat{O}^{(H)}),\nonumber\\
 Y&=\mat{C}\mat{W}_O.
\end{align}
...
\caption[两头注意力结构示意]{两头注意力结构示意。每个头分别归一化查询到键的读取权重，拼接发生在输出特征轴，不发生在序列轴。}\label{fig:attn-multihead}
\end{figure}

\subsection{多头特征空间}
恢复批量后，投影输出 $[B,T,Hd_k]$ 可以重排为 $[B,T,H,d_k]$，再交换中间两轴得到 $[B,H,T,d_k]$。键和值同理。头维度与序列维度对应不同的几何轴向：分头是将隐藏状态投影至平行的低维子空间，各头独立在全序列有效位置上执行注意力加权。多头结构在初始化时仅提供多组独立的表征子空间，并未预置固定的句法结构提取或长程指代等功能角色。各头在训练收敛后是否分化出特定的语义模式，需通过注意力头消融实验（Head Pruning）、探测分类器（Probing Classifiers）或神经元激活干预进行实证检验。

\subsection{投影打包和参数共享}
自注意力中，三个投影读取同一个 $\mat{X}$，可以将参数沿输出特征轴拼接为
\begin{equation}
 \begin{aligned}
  \mat{W}_{QKV}&=[\mat{W}_Q\;\mat{W}_K\;\mat{W}_V],\\
...
\caption[自注意力与交叉注意力信息来源]{自注意力与交叉注意力信息来源。右侧的源记忆共享位置索引，查询位置和源位置不要求一一对应。}\label{fig:attn-self-cross-flow}
\end{figure}
...
\begin{equation}
 \begin{aligned}
  \mat{A}&=\begin{bmatrix}\frac{1}{2}&\frac{1}{2}\\\frac{3}{4}&\frac{1}{4}\\\frac{1}{4}&\frac{3}{4}\end{bmatrix},\\
...
编码器结构通常采用双向自注意力，解码器自回归结构采用因果自注意力，编码器--解码器结构则额外采用交叉注意力。可见性与模块组成属于两个独立的维度。完整 Transformer 还需逐位置前馈变换、残差和归一化，并通过输出头与训练目标定义预测问题；这些组成在\bookxref{ch:transformer}中建立。
...
\begin{equation}
 \begin{aligned}
  \overline{\mat{V}}&=\mat{A}^{\mathrm{T}}\mat{G}\in\Real^{S\times d_v},\\
...
\begin{equation}\label{eq:attn-qk-backward}
 \begin{aligned}
  \overline{\mat{Q}}&=\frac{\mat{D}\mat{K}}{\sqrt{d_k}}\in\Real^{T\times d_k},\\
  \overline{\mat{K}}&=\frac{\mat{D}^{\mathrm{T}}\mat{Q}}{\sqrt{d_k}}\in\Real^{S\times d_k}.
 \end{aligned}
\end{equation}
\keyconcept{值路径梯度直接反映被读取内容的更新诉求，查询与键路径梯度则通过分数调整注意力权重的空间分配。}若一行只有一个可见键，其权重恒为一，该行所有分数梯度均为零，对应值梯度可以非零。该行没有可学习的匹配选择，分数梯度为零由结构决定。

\subsection{投影矩阵与输入的梯度}
...
\end{align}
自注意力中 $\mat{X}_q=\mat{X}_m=\mat{X}$，来自查询、键、值三条通路的梯度\emph{必须在输入端累加}；遗漏任意一条通路都会导致反向传播计算图断裂：
\begin{equation}
 \overline{\mat{X}}=\overline{\mat{Q}}\mat{W}_Q^{\mathrm{T}}+
 \overline{\mat{K}}\mat{W}_K^{\mathrm{T}}+\overline{\mat{V}}\mat{W}_V^{\mathrm{T}}.
\end{equation}
多头输出 $Y=\mat{C}\mat{W}_O$ 还需先计算 $\overline{\mat{W}}_O=\mat{C}^{\mathrm{T}}\overline Y$ 与 $\overline{\mat{C}}=\overline Y\mat{W}_O^{\mathrm{T}}$，再按特征轴拆成各头上游梯度。如果启用了权重 Dropout，则在经过 Softmax 反向前，还要乘以同一次前向使用的 $\frac{m}{\vect{q}}$。

\begin{warning}[反向传播中的 Dropout 掩码一致性]
在带有 Dropout 的注意力计算中，反向传播必须严格复用前向传播所生成的随机失活布尔掩码；若在反向计算时重新采样随机掩码，将导致梯度流经与前向计算不一致的激活通路，引入严重数值偏差并破坏优化稳定性。
\end{warning}

\begin{example}[注意力的局部梯度]
...
\end{equation}
因此自注意力在 $T=S$ 时的线性投影计算量为 $\mathcal{O}(BTd^2)$，位置交互计算量为 $\mathcal{O}(BT^2d)$。当序列长度 $T$ 远大于模型宽度 $d$ 时，位置交互项随序列长度呈二次方增长，成为核心计算瓶颈；但在短序列或大模型宽度（$T \ll d$）条件下，参数投影与前馈网络的矩阵乘法在总计算量中占据主导地位。
...
\caption[MHA、GQA 与 MQA 映射关系对比]{MHA、GQA 与 MQA 映射关系对比。GQA 在保持查询多样性的同时显著压缩键值缓存显存，缓解自回归生成的内存带宽瓶颈。}\label{fig:attention-head-groups}
\end{figure}

表\ref{tab:attn-variants}对比了不同注意力头变体（包括现代多头潜在注意力 MLA）的机制差异、相对 KV Cache 显存开销与典型应用。

\begin{table}[htbp]
\centering
\caption{多头注意力变体机制与开销对比}\label{tab:attn-variants}
\begin{tabularx}{\linewidth}{p{16mm}p{14mm}p{14mm}p{20mm}X}
\toprule
结构类型 & 查询头 $H_q$ & 键值头 $H_{kv}$ & KV 相对显存 & 机制特征与典型代表制品 \\\midrule
MHA & $H$ & $H$ & $1.0\times$ (基准) & 每头使用独立的键值投影；长序列解码需为各头分别保存并读取 KV Cache（GPT-3、LLaMA-1） \\
...
查询头仍有 $H_q$ 个，每头都要对允许位置计算权重。显式分数的形状仍是 $[B,H_q,T,S]$，因此键值存储量的缩减比例与匹配、汇总的乘加数量变化不同。实际加速还取决于内核能否复用键值读取、数据布局以及硬件瓶颈。

从 MHA 改为 GQA 引入了参数共享约束，改变了模型的假设空间与可表达函数族。将已训练的 MHA 检查点转换为 GQA 时（如将同一组内的键值矩阵求平均），会导致输出分布产生偏移，通常需要额外的轻量继续预训练进行参数适配；张量形状对齐并不等价于模型行为等价。分组数、单头宽度和投影布局均属于模型结构定义，须与权重共同持久化。
...
\begin{equation}
 \begin{aligned}
  \mat{Q}'&=(\mat{P}\mat{X})\mat{W}_Q=\mat{P}\mat{Q},\\
  \mat{K}'&=\mat{P}\mat{K},\\
  \mat{V}'&=\mat{P}\mat{V}.
 \end{aligned}
\end{equation}
...
注意力权重 $A_{ij}$ 仅表示在当前层该头内部对位置 $j$ 的值向量 $\vect{v}_j$ 赋予的加权系数，不等于该词元对最终模型预测的因果重要性（Saliency / Importance）。首先，当值向量空间存在冗余（例如各 $\vect{v}_j$ 极为接近）时，高度集中的注意力分布与均匀分布可产生近乎相同的加权输出；其次，加权输出后续还经历多头拼接投影、残差连接求和、层归一化及前馈网络的非线性变换。分析词元级别的因果归因时，需借助积分梯度（Integrated Gradients）或基于因果中介分析（Causal Mediation Analysis）的激活干预，而非直接将注意力热力图视为主观解释。

\subsection{局部性质与整体行为}
结构验证应覆盖投影形状、允许集合、归一化行和、输出矩阵乘法及梯度回传。因果结构需验证未来扰动不改变早期输出；共享键值结构需核对各查询头严格映射到指定的分组索引。对框架实现进行数值对齐时，必须统一参数取值、偏置项、缩放系数、位置编码形式、掩码语义以及 Dropout 伪随机数种子。

浮点运算的累加顺序差异可能导致舍入误差，需根据浮点精度（FP32、FP16 或 BF16）设定合理的相对容限与绝对容限。单元算子验证保证基础数值实现与数学定义的一致性，端到端建模能力则依赖预训练数据分布与目标函数的联合优化。


\begin{note}[注意力计算的结构约束]
