...
\textbf{输入：}$\mat{Q}\in\Real^{T\times d_k}$、$\mat{K}\in\Real^{S\times d_k}$、$\mat{V}\in\Real^{S\times d_v}$，每行允许集合 $\mathcal J_i$，查询有效标记 $b_i$。\textbf{输出：}$\mat{O}\in\Real^{T\times d_v}$；需要诊断或显式反向时同时保留权重 $\mat{A}$。本算法关闭权重 Dropout。
\begin{lfsalgorithmic}[1]
\State 检查形状、键值位置数和输入有限性；$\mat{O}\gets0$，需要权重时 $\mat{A}\gets0$
\For{查询 $i=1,\ldots,T$}
  \If{$b_i=0$}
  ...
  \Else
    ...
    \For{$j\in\mathcal J_i$}
      \State $u_j\gets\frac{\vect{q}_i \vect{k}_j^{\mathrm{T}}}{\sqrt{d_k}}$；非有限结果触发数值异常并终止
    \EndFor
    \State $c\gets\max_{j\in\mathcal J_i}u_j$，$e_j\gets\exp(u_j-c)$，$z\gets\sum_{j\in\mathcal J_i}e_j$
    \For{$j\in\mathcal J_i$}
      \State $a_j\gets\frac{e_j}{z}$，$\vect{o}_i\gets \vect{o}_i+a_j\vect{v}_j$；需要权重时 $A_{ij}\gets a_j$
    \EndFor
  \EndIf
\EndFor
\State \Return $\mat{O}$ 及按需保留的 $\mat{A}$
\end{lfsalgorithmic}
