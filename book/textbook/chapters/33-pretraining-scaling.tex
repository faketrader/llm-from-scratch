...
预训练决定模型所拟合的先验文本分布，并在给定计算预算下决定参数容量与训练词元的最优分配。二者紧密耦合：词元总数涵盖不同的语言分布、领域多样性与重复度；参数规模则必须与充分的训练步数匹配才能发挥表征能力。规模定律提供参数与数据在计算预算约束下的经验拟合模型，预训练目标则定义具体的似然优化准则。
...
\term{预训练}{Pretraining}{}通常从随机初始化开始，利用大规模无标注语料学习可迁移的通用表示与条件概率分布。在已有基座上开展的\term{继续预训练}{Continued Pretraining}{CPT}沿用相同的自回归或去噪目标，初始参数已包含源分布的先验知识，其优化轨迹和收敛速度显著不同于从零初始化的训练。

设文档 $x=(x_1,\dots,x_T)$，在给定起始标记与终止规则下，联合概率分解为
\begin{equation}\label{eq:pt-chain}
\begin{aligned}
 p_\theta(x) &= \prod_{t=1}^{T}p_\theta(x_t\mid x_{<t}), \\
 -\log p_\theta(x) &= -\sum_{t=1}^{T}\log p_\theta(x_t\mid x_{<t}).
\end{aligned}
\end{equation}
$\theta$ 为可学习参数，$x_{<t}$ 为当前位置之前的历史词元。该概率分解不要求词元间相互独立，序列相关性直接由条件概率分布建模。训练阶段输入真实历史前缀，称为\term{教师强制}{Teacher Forcing}{}。自回归推理阶段输入包含模型自身生成的历史词元，训练阶段的单步交叉熵损失度量真实分布下的拟合优度，自由生成质量还受到多步自回归解码中误差级联与退化行为的影响。
...
\begin{align}
 ...
 S&=\sum_{b=1}^{B}\sum_{t=0}^{T-2}m_{b,t},\nonumber\\
 \mathcal L&=\frac{1}{S}\sum_{b,t}m_{b,t}\ell_{b,t}.
 \label{eq:pt-masked-loss}
\end{align}
实现对数求和指数时可先减去最大分数再加回，以避免指数溢出。$S=0$ 时批次没有有效监督词元，平均训练目标无定义，必须跳过更新或在构造阶段排除该批次。
...
\end{equation}
因此，分母不仅改变日志数值，也改变梯度权重。\emph{填充掩码}控制哪些位置是实际有效输入，\emph{因果掩码}控制自注意力可见性，\emph{监督掩码}控制哪些配对贡献反向梯度；三个对象需要分别定义。
...
\begin{equation}
\begin{aligned}
 L_{\mathrm{token}} &= \frac{\sum_iT_iL_i}{\sum_iT_i}, \\
...
\end{equation}
同时必须处理跨段目标配对。在自回归因果掩码下，即使注意力机制屏蔽了文档 $a$ 对文档 $b$ 的跨文档读取，$a$ 末尾位置的预测头输出在无额外监督掩码干预时仍默认以 $b$ 的首个词元作为真实标签。通常每段包含显式起始标记，由起始标记预测该段首词元；上一段终止位置到下一段起始位置的跨边界配对应在监督掩码中置零，不计入损失。是否重置绝对位置编号也需在配置中明确固定，以避免破坏预训练位置编码的边际分布。
...
\end{tikzpicture}
\caption[文档装箱的注意力与损失边界]{文档装箱的注意力与损失边界。注意力禁止跨文档读取，损失忽略跨文档预测配对。实线表示有效相邻监督。}\label{fig:pt-packing}
\end{figure}

对于连续章节、代码文件集合或保留上下文的对话，可以按任务需要保留跨片段可见性。是否隔离应依据统计单位，而非存储切片：把同一文档拆段后完全隔离，会损失长距离条件；把不相关文档连接而不隔离，则引入人为前缀分布。
...
\begin{note}[装箱利用率与有效监督密度]
非填充位置占物理上下文窗口的比例描述装箱利用率；有效监督目标占物理位置的比例描述监督密度。边界分隔符、仅充当历史前缀的提示词和填充区域均消耗浮点计算，但不直接产生损失反向梯度。计算预算与吞吐量核算必须同时记录这两类统计量。
\end{note}
...
\begin{equation}\label{eq:pt-mixture}
\begin{aligned}
 P_{\mathrm{train}} &= \sum_i\pi_iP_i, \\
 L(\theta) &= \sum_i\pi_iL_i(\theta), \qquad \left(\sum_i\pi_i=1\right).
\end{aligned}
\end{equation}
...
常用温度平滑以合格可用词元量 $n_i$ 定义 $\pi_i\propto n_i^\tau$，$0\le\tau\le1$。这里 $\tau$ 是混合指数，并非 softmax 温度或规模幂律指数。$\tau=1$ 保留规模自然比例，$\tau=0$ 对非空来源均匀等权。降低指数提高小规模数据源的相对采样份额，但这会导致小数据源的遍历倍数增加与近重复曝光升高，并不增加原始独立信息熵。

\begin{example}[数据混合配比]
设三个来源可用词元量为 90、9、1 十亿。自然比例为 $0.90,0.09,0.01$；取 $\tau=\frac{1}{2}$ 后，权重与 $\sqrt{90},3,1$ 成正比，约为 $0.7034,0.2224,0.0741$，最后一项因四舍五入可作相应校正。计划消费 200 十亿词元时，各来源的名义遍历倍数为
\begin{equation}
 e_i=\frac{D\pi_i}{n_i},
\end{equation}
...
\begin{equation}\label{eq:pt-unique}
...
质量筛选也会改变语言和文体覆盖。低困惑度文本可能更贴近评分模型熟悉的分布，却未必覆盖罕见术语或形式化推导。预训练配方应分别记录准入标准、质量分层、语言语种、专业领域、序列长度和重复程度；单一综合标量分数会掩盖语言、领域与信息密度的独立特征。数据流水线的实现见\bookxref{ch:data-engineering}，本章关心这些决策怎样改变 $P_{\mathrm{train}}$。
...
\begin{table}[htbp]
\centering
\caption{自监督预训练任务条件结构对比}
\label{tab:pretraining-tasks}
...
混合任务可写为 $L=\sum_k\lambda_kL_k$，但 $\lambda_k$ 的物理含义依赖各 $L_k$ 的归一化方式。若某一任务仅对序列中15\%的位置遮蔽并施加监督，而另一任务对近乎全序列执行自回归预测，相同批次抽样概率下对应的有效目标词元贡献可相差数倍。评估与对比不同预训练任务时，必须统一报告实际处理位置量与等效浮点运算量；损失数值差异直接受到任务条件信息量和有效监督词元密度的制约。

\section{预训练计算预算}

\subsection{词元计量口径}
$D_u$ 表示去重后可用内容的\emph{独立语料规模}，$D_s$ 表示累计消耗的\emph{有效目标词元量}，$D_p$ 表示模型实际处理的\emph{物理计算位置量}。重复多个遍历增加 $D_s$ 和 $D_p$，不会同比增加 $D_u$；填充和仅供上下文的位置增加 $D_p$，不一定增加 $D_s$。

设每次参数更新有 $R$ 个数据并行进程，每进程累积 $G$ 个微批次，每批次 $B$ 个长度 $T$ 的块，训练 $K$ 步，在规则相同且无额外掩码时
\begin{equation}\label{eq:pt-budget-count}
\begin{aligned}
 D_p &= KRGBT, \\
 D_s &= KRGB(T-1).
\end{aligned}
...
\keyconcept{Chinchilla 规模定律给出了给定训练计算预算下的理论最优分配比（参数量与词元数近乎等比例扩展）；但当将大模型服务化部署的全生命周期推理成本纳入综合目标时，对较小参数量模型进行远超计算最优配比的“深度超训”（Over-training），能在长期摊薄端到端总成本。}
...
\subsection{规模定律的经验形式}
\term{规模定律}{Scaling Law}{}以经验函数描述模型规模、数据和计算与损失之间的关系。Kaplan 等人的研究系统分析了语言模型交叉熵随这些变量变化的规律\cite{kaplan2020scaling}；后续 Chinchilla 工作重新研究固定训练计算下的参数与训练词元分配\cite{hoffmann2022chinchilla}。经验拟合指数严格受限于特定模型族结构、退火学习率调度与数据清洗标准，跨方案直接拼合指数会破坏前提一致性。
...
\begin{equation}
 ...
 \label{eq:pt-scaling}
\end{equation}
$E$ 表示该经验模型中的渐近底座，$AN^{-\alpha}$ 与 $B_sD^{-\beta}$ 分别描述参数和数据受限时的损失余量。数据、分词器和条件窗口的变化也会改变渐近损失。表\ref{tab:scaling-laws-comparison}对比了代表性规模法则拟合方案的经验模型形式、拟合指数与工程结论。

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{3.5pt}
\caption{主流规模法则参数与拟合结论对比}
\label{tab:scaling-laws-comparison}
...
同一训练运行中的多个检查点共享数据与随机轨迹，将其视为相互独立实验会低估方差并虚增拟合置信度。应按独立训练运行或随机种子组织残差统计，并报告外推区间的误差界。若拟合残差随模型规模、序列长度或数据来源呈现系统性偏离，表明经验模型缺失关键自由度，提高参数数值精度无法解决外推偏差。
...
\begin{example}[计算预算下的参数与数据分配]


例如，定义 $n=\frac{N}{10^9}$、$d=\frac{D}{10^9}$，假设
\begin{equation*}
\begin{aligned}
 L(n,d) &= 2+n^{-\frac{1}{2}}+4d^{-\frac{1}{2}}, \\
 C_0 &= \num{9.6e19}, \qquad \kappa=6.
\end{aligned}
\end{equation*}
...
\begin{table}[htbp]
\centering
\caption{固定计算预算下参数与词元分配对比}
\label{tab:isocompute-allocation}
\begin{tabular}{rrrr}
...
较小模型获得更多数据但容量项较大，较大模型压缩了训练长度而数据项较大。在固定计算预算下，过度偏向参数规模或过度延长训练序列均会因欠拟合或数据重复导致次优损失。预算扩大四倍后，最优配置为 $n=2,d=32$，理论损失为 $2+\sqrt2\approx3.414$。
...
\end{tikzpicture}
\caption[固定计算预算下的损失余量曲线]{固定计算预算下的损失余量曲线。曲线由本节解析函数计算；纵轴从零起算，曲线最低点对应参数与数据的边际收益平衡。}\label{fig:pt-isocompute}
\end{figure}
...
经验式中的数据量通常建立在特定重复范围和质量分布上。重复曝光有时仍能改善优化，可提供的新统计信息却是有限的。合成数据的有效规模需结合新增信息、样本相关性、共同错误和验证覆盖评估，并分别记录生成量与来源数据量。

同样，计算量相同但学习率、批次、衰减与停止时机不同，可能导致不同损失。规模最优讨论要求各候选达到可比的预算内训练效率，各候选模型若简单沿用固定的更新步数，会破坏训练效率的可比性。\bookxref{ch:training-stability}将展开优化器状态与调度；在本章的预算实验中，它们属于必须记录的控制条件。
...
\subsection{损失变化的能力解释}
验证交叉熵衡量预训练分布上的平均预测质量；下游任务还受评分阈值、提示方式、样本量和数据混合影响。模型选择应同时保留可比的预训练损失与领域、语言、长上下文等任务指标，分别观察总体拟合与能力切片。

\begin{note}[规模定律的工程约束与适用边界]
规模定律用于缩小预算候选范围，并提出可被实验检验的经验预测。拟合区间、分布、架构或目标发生变化后，应重新评估系数与残差分布。连续最优解是架构选型的理论基准，工程落地还须满足真实数据库存量、设备显存拓扑与线上推理时延预算。
\end{note}
...
\end{tikzpicture}
\caption[预训练状态演进与断点恢复路径]{预训练状态演进与断点恢复路径。实线表示计算和保存路径，虚线表示恢复或下一轮实验设计；验证结果不直接生成当前训练目标。}\label{fig:pt-system}
\end{figure}
...
训练过程记录的移动平均损失反映历史参数轨迹在训练批次上的累计表现，而最终检查点的独立验证损失衡量收敛参数在标准切片上的泛化性能，二者在评价基准与参数状态上具有不同物理含义。用于选择最佳检查点的验证集已经参与模型超参数选择，最终泛化评估应在严格隔离的保留测试集上进行。
