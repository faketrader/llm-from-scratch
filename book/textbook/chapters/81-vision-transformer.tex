...
\begin{equation}
 ...
 \qquad \mat{X}\mat{W}_E=\begin{bmatrix}14&-2\\22&-2\\46&-2\\54&-2\end{bmatrix}.
\end{equation}
...
\draw[->] (en)--(7.6,-2.55);
\end{tikzpicture}
\caption{ViT 的表示路径。分类读出压缩空间序列；密集预测可以保留补丁表示。}\label{fig:vit-system}
\end{figure}

\subsection{CLS 和平均池化}
...
\begin{equation}
 ...
 \label{eq:vit-pooling}
\end{equation}
其中 $m_n\in\{0,1\}$，且 $N_{\mathrm{valid}}>0$。前者通过专门位置学习信息汇集，后者对最终补丁表示赋相同读出权重，但这些补丁已经经过内容相关的全局交互，GAP 与对原始像素简单取平均是两回事。
...
标准注意力的逻辑分数共有 $BaL^2$ 个元素。若显式以每元素 $s$ 字节存储，单张分数张量占 $sBaL^2$ 字节；训练还包括其他激活、梯度和优化器状态。融合注意力内核可能不物化整张矩阵，故逻辑规模与实际峰值显存是两笔账。相关实现见高效注意力章节。
...
ViT 的补丁投影具有局部性与权重共享的视觉先验。它的编码器允许补丁全局交互，而绝对位置表可使不同位置具有不同作用。一像素平移可能改变多个补丁内部的像素组合，与补丁序列的整数置换是两种不同的变换。因此，式\eqref{eq:vit-equivariance}的序列置换等变与图像平移等变是两条性质。

原始 ViT 强调了预训练数据规模与迁移效果的关系；后续数据高效视觉 Transformer 研究说明，训练配方、正则化及蒸馏也会显著改变数据利用效率\cite{touvron2021deit}。模型容量、目标域和数据增强共同决定泛化条件。视觉 Transformer 与传统卷积神经网络的核心归纳偏置及计算机制对比见表\ref{tab:rev-vit-vs-cnn}。

\begin{table}[htbp]
\centering
\caption{视觉 Transformer (ViT) 与卷积神经网络 (CNN) 的核心机制对比。}
\label{tab:rev-vit-vs-cnn}
...
\begin{myalgorithmic}[1]
 \State Process images using the declared channel order, range, scaling, and normalization; crop or pad under the boundary rule and record valid regions
 \State Determine patch order using Equation~\eqref{eq:vit-index} and apply Equation~\eqref{eq:vit-embedding} or an equivalent convolutional projection; keep weight layout consistent with pixel layout
 \State Check the source grid of the position table; if the target grid differs, separate CLS and interpolate under the declared two-dimensional rule; terminate with a configuration error if patch-projection dimensions are incompatible
 \State Add CLS and position embeddings and build a valid-position mask; require every valid query to have at least one visible key and never pass an all-masked row directly to ordinary softmax
 \State For $t=1,\ldots,T$, apply Equation~\eqref{eq:vit-block} while preserving sequence length and hidden dimension; disable training-time stochastic dropout during inference
 \State Produce $h$ using the fixed CLS or valid-mean readout, compute class scores and probabilities, and interpret the result using the bound label mapping
\end{myalgorithmic}
...
注意力图显示式\eqref{eq:vit-visual-attn}中的内部权重，与像素的因果重要性是两种量。多头投影、值向量、残差和后续非线性共同决定输出。遮挡分析也改变了输入分布，因此应说明遮挡规则和比较对象，并结合任务与扰动结果评价解释。
