...
尺度则不同：将差值乘常数会改变偏好概率。若同时引入未知噪声温度 $\tau$，模型只由 $\frac{r_a-r_b}{\tau}$ 决定，此时奖励尺度和温度不能分别识别。奖励归一化或温度修改会改变后续策略优化的有效强度，应与策略目标一并记录。
...
取 $\beta=0.5$，上述间隔给出 $u=0.549306$，偏好概率为 $\frac{\sqrt3}{1+\sqrt3}\approx0.6340$，损失约0.4557，低于初始相等策略时的 $\log2\approx0.6931$。第三个回答在自由生成中的占比增大，其质量需要另行评价。
...
\end{equation}
将求和除以 $T_y$ 得到平均词元对数概率，改变了回答之间的权重。长度归一化由此定义了不同于式\eqref{eq:pref-regularized}的新目标。
...
\begin{algorithmic}[1]
\State 校验每对共享同一完整提示、标签有明确含义，两回答的目标跨度与终止协议有效；剔除或单独记录截断后失去偏好依据的记录。
\State 在冻结参考上计算两条回答的目标词元对数概率和。若使用缓存，键必须绑定参考权重、模板、分词器及精确词元序列。
\State 在当前策略上计算相同目标位置的对数概率和，依式\eqref{eq:pref-dpo}形成间隔与稳定的 Softplus 损失。
\State 按事先声明的提示或偏好对权重聚合，反向仅更新策略；累积与并行归约保持该权重口径，而非擅自改成词元平均。
\State 记录损失、间隔、两回答对数概率、长度及有效对数；定期在独立提示上自由生成，并与冻结基线进行盲比较。
\end{algorithmic}
\end{algorithm}
...
偏好评估应同时固定候选生成预算、比较基线和呈现方式。胜率需要报告比较数、平局处理、提示分组及不确定性；重复比较同一提示不能简单当作独立扩大样本。长度、语言、任务类别和高严重度失败应分开观察，分别报告各类任务的表现与严重失败。

\begin{note}[对齐结论必须绑定反馈范围]
