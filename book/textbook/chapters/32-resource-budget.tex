...
设备能否容纳模型取决于峰值状态和工作区，训练能否按期完成取决于总工作量与有效处理速度。估算显存和设备数量时，除了参数量，还需要模型结构、序列长度、批量及数值精度。本章从模块形状出发，逐项推导参数量、计算量、存储量与训练时间。
...
对于专家层，若每个专家为门控前馈，独立路由专家总参数为 $E\cdot3dd_e$；每词元执行的专家矩阵规模近似为 $k\cdot3dd_e$。共享专家、路由器和注意力另计。\keyconcept{激活参数影响本次计算，总参数影响需要保存的模型状态。}显存公式中两者各自出现在不同位置。路由不均衡还会使某设备的实际负载偏离平均值，详见\bookxref{ch:moe}。

\begin{example}[模型参数量核算]
...
反向需要 $\overline X=\overline YW^{\mathsf T}$ 与 $\overline W=X^{\mathsf T}\overline Y$，两项各约 $2nab$，前向与反向合计约 $6nab$。这解释了稠密模型训练中常用 $6PD$ 估计的来源：每词元约执行一次前向与两次同阶矩阵反向。各执行阶段的核心计算与显存速查规则如表\ref{tab:compute-rules}所示。

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{3.5pt}
\caption[Transformer 计算与显存估算规则]{Transformer 计算与显存估算规则。自回归 Transformer 在前向、反向、预填充与增量解码阶段的 FLOPs 与显存速查规则。}\label{tab:compute-rules}
...
\begin{table}[htbp]
\centering
\caption[混合精度训练状态显存占用]{混合精度训练状态显存占用。混合精度 Adam 训练中模型参数、梯度与优化器状态各分量每参数显存占用及用途。}\label{tab:training-state-memory}
\begin{tabular}{lcc}
...
\end{tabular}
\end{table}
冻结基座不再为基座计算全部参数梯度和优化器状态，却通常仍需保存\emph{前向激活}以向可训练适配器传递梯度。\keyconcept{参数高效微调仅削减梯度与优化器状态显存，反向传播向适配器传导梯度时仍须保留基座前向激活。}因此 LoRA 或 QLoRA 的显存无法由可训练参数百分比乘全参数训练显存得到。

\begin{warning}[参数高效微调的显存估算陷阱]
虽然 LoRA 等方法仅更新不足 $1\%$ 的参数，大幅消减了梯度和优化器状态，但前向传播依然贯穿整个基座模型；向低秩分支反向传播误差时，未分流或位于适配器之前的基座层前向激活值仍须驻留显存（若未开启激活重计算）。因此，微调显存主要受批量与上下文长度决定的激活显存主导，切忌按可训练参数比例估算显存缩减。
\end{warning}

\subsection{状态分片的显存开销}
...
\end{equation}
一次调用若算术强度低，即使峰值矩阵吞吐很高，也可能受权重或缓存读取限制。自回归预填充具备高算术强度，表现为\emph{算力受限}；单步增量解码每次仅处理单个词元，表现为典型的\emph{访存受限}。\keyconcept{自回归预填充吞吐受张量计算核心峰值算力约束，而增量解码吞吐受高带宽显存读写带宽约束。}提高批量可以复用权重，但同时增加缓存和延迟；峰值 FLOP/s 只是性能问题的一个维度。
...
\begin{example}[计算、带宽与容量的联合预算]
假设某稠密任务含 $P=7\times10^9$ 参数、训练 $D=10^{11}$ 词元，暂用 $F=6PD=4.2\times10^{21}$ 操作的基线。每设备峰值设为 $2\times10^{14}$ 操作每秒，模型利用率假设 $0.4$，期限为10天，即864000秒。计算约束为
\begin{equation*}
 N_{\mathrm{compute}}=\left\lceil
 \frac{4.2\times10^{21}}{(2\times10^{14})(0.4)(864000)}
 \right\rceil=61.
\end{equation*}
...
训练吞吐目标若为每秒 $r_t$ 个词元，物化数据平均每词元占 $b_t$ 字节，则纯输入带宽下界为 $r_tb_t$；索引、压缩、重试和预取会增加其他开销。运行时包含解析和分词时，CPU 核数需根据各步骤的实测处理速率及目标输入吞吐配置。
...
\end{tikzpicture}
\caption[资源供给与状态持久化流水线]{资源供给与状态持久化流水线。训练中计算资源供给、检查点写入与状态持久化构成的闭环流水拓扑。}\label{fig:resource-data-path}
\end{figure}
