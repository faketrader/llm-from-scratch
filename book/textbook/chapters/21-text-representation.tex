...
文本编码决定序列长度、稀有字符串的表示方式、输出空间和对话边界。字符、字节和词元是不同层次的表示单位；合并规则、最长匹配和概率分段则提供了不同的分词方法。分词后的离散编号通过嵌入映射为连续向量，供后续网络计算。大规模训练数据的采集与处理见\bookxref{ch:data-engineering}。
...
嵌入矩阵把 $K$ 转换为连续张量 $X\in\Real^{B\times T_{\max}\times d}$，注意力机制再在可见位置之间汇总这些向量。从文本到上下文表示的过程为
\begin{equation}
...
权重绑定让输入与输出复用参数，但两侧承担不同职责。输入侧回答词元如何参与上下文计算，输出侧回答当前状态与候选词元是否匹配。约束这两个角色可以减少自由参数，也可能限制模型可表达的组合。质量变化取决于数据分布、模型结构与任务要求。
...
嵌入能够在训练中承载有关共现与预测的规律，但向量接近不必表示同义。反义词可能出现在相似句法环境中，词元片段也未必对应完整概念。固定查表将一个编号对应到同一向量；上下文表示 $h_t$ 则同时依赖其他词元和位置。前者的邻近关系与后者的语境相似性不能直接互换，二维投影中的聚类需结合原空间距离与邻域检查。
