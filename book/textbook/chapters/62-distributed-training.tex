...
\subsection{桶大小及通信尾部}
反向沿层逆序产生梯度。将若干梯度合并为通信桶，可以在前面层仍计算时归约已经就绪的桶。大桶减少启动次数，却可能推迟首个通信；小桶更早就绪，却增加时延和调度开销。最后一批计算结束后仍未完成的通信构成暴露在关键路径上的尾部。
...
层内高频通信宜优先映射到适合其带宽和时延需求的互连域，具体放置由通信成本决定。参数分片、专家流量或大规模上下文交换也可能跨节点；必须使用\bookxref{ch:compute-infrastructure}的链路约束计算关键路径和超售影响。
...
若8设备某固定预算耗时100分钟，16设备耗时60分钟，则加速比为 $\frac{5}{3}$，效率约为83.3\%。若模型不能在单设备容纳，应使用可运行的多设备基线并明确 $D_0$，据此计算相对加速比。还应同时报告有效目标词元吞吐、实际序列长度、更新次数、精度、峰值显存和恢复成本。
