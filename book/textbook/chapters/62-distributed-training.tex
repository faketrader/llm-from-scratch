...
模型规模扩大后，激活或优化器状态可能超过单设备显存。分布式训练通过划分数据、模型或训练状态，将计算分配到多个设备，同时引入跨设备通信。设计方案时，需要保持训练目标与参数更新的一致性，再评估容量收益、计算加速和通信成本。
...
\begin{table}[htbp]
\centering
\caption{分布式训练主要符号与设备量}\label{tab:distributed-training-symbols}
\begin{tabular}{ll}
\toprule
符号 & 含义\\
\midrule
...
\term{进程序号}{Rank}{}是通信组内的逻辑成员标识。在多维混合并行中，物理进程通过正交的笛卡尔网格坐标映射至不同的局部通信域（例如数据并行组、张量并行组与流水线并行组），集合通信仅在特定的局部通信子空间内闭环执行，全局物理进程序号无法直接表征局部拓扑边界与数据流向。
...
\begin{equation}\label{eq:dist-average-correction}
 \begin{aligned}
  \mathcal L_r^* &= \frac{R}{N}\sum_jS_{rj}, \\
  \frac{1}{R}\sum_r\nabla\mathcal L_r^* &= \frac{1}{N}\sum_{r,j}\nabla S_{rj}.
 \end{aligned}
\end{equation}
若通信执行求和，则无需分子中的 $R$。损失缩放应与实际通信的求和或平均语义一致。

设两个数据并行副本分别包含 $2$ 个和 $6$ 个有效监督词元，其对应的局部损失和梯度分别为 $2$ 和 $18$。若各副本先执行局部平均计算再对跨副本梯度取等权平均，得到 $\frac{1+3}{2}=2$，偏离了全局加权期望；若依据式\eqref{eq:dist-average-correction}预先乘以权重修正系数 $\frac{R}{N}=\frac{2}{8}=\frac{1}{4}$，局部梯度变为 $0.5$ 和 $4.5$，跨副本平均后精确收敛至正确梯度 $\frac{0.5+4.5}{2}=2.5$。\keyconcept{全局有效监督词元数归约必须严格限定在数据并行通信域内；张量与序列并行切片共享同一样本，跨并行维度重复累加将导致损失加权严重偏离。}全局监督词元总数 $N$ 必须严格仅在数据并行（DP）通信组内归约；张量并行（TP）与序列并行（SP）的进程共同持有相同样本的特征切片，若在统计中跨 TP 组累加将导致 $N$ 被虚假放大。
...
\end{tikzpicture}
\caption[ZeRO 分片状态逻辑所有权]{ZeRO 分片状态逻辑所有权。展示优化器状态、梯度和模型参数的逻辑归属，不包含前向或反向计算期间临时聚合的完整模块。}\label{fig:dist-zero}
\end{figure}

峰值显存还包含激活、当前聚合模块、预取模块、梯度桶、分配器碎片与临时工作区。设最大聚合模块为 $U$ 字节，则即使持久状态很小，至少仍需为该模块的计算表示留出空间；同时预取下一模块还会增加驻留量。过粗的分片单元可能使“全分片”仍然无法执行。

\begin{example}[状态分片的容量与通信量]
设某模型的参数量对应 $P_s=\qty{12}{\gibi\byte}$、$G_s=\qty{12}{\gibi\byte}$、$O_s=\qty{72}{\gibi\byte}$，分片组规模为 $R=8$。四种持久状态分别为 \qty{96}{\gibi\byte}、\qty{33}{\gibi\byte}、\qty{22.5}{\gibi\byte} 和 \qty{12}{\gibi\byte}。若另需保留 \qty{10}{\gibi\byte} 激活与 \qty{6}{\gibi\byte} 工作区，阶段2的峰值显存近似为 \qty{38.5}{\gibi\byte}；阶段3若需要 \qty{4}{\gibi\byte} 聚合参数和 \qty{4}{\gibi\byte} 预取缓冲，则峰值约为 \qty{36}{\gibi\byte}。临时聚合参数与预取缓冲的存在使此例的实际峰值显存仅下降 \qty{2.5}{\gibi\byte}。
...
\end{equation}
前向计算中，第一层的\emph{列切分}（Column Parallel）与第二层的\emph{行切分}（Row Parallel）形成共轭配对：列切分的输出特征通道自然适配行切分的输入权重切片，前向传播中仅需在行切分末尾触发一次跨设备 All-Reduce 聚合即可完成多层感知机的输出计算\autocite{shoeybi2020megatron}。\keyconcept{Megatron 列并行与行并行的共轭配对消除了层间激活重组通信，将前向传播的跨卡全归约严格压缩至行并行输出之后。}这种算子配对严格依赖激活函数 $\phi$ 的逐元素非线性特性；由于 $\phi(\sum_i \mat{Z}_i)\ne \sum_i \phi(\mat{Z}_i)$，若颠倒行切分与列切分顺序，将迫使系统在非线性算子前引入额外的跨卡集合通信。
...
\begin{equation}\label{eq:dist-pipeline-bubble}
 \begin{aligned}
  U &= \frac{m}{m+p-1}, \\
  \rho_{\mathrm{bubble}} &= \frac{p-1}{m+p-1}.
 \end{aligned}
\end{equation}
当 $p=4,m=8$ 时 $U=\frac{8}{11}\approx 72.7\%$。增加微批次数 $m$ 可摊薄\emph{流水气泡}占比，但会按比例增加待反向保留的\emph{在途激活量}，或在固定全局批量约束下迫使单步微批量缩小，进而劣化计算核心的张量运算强度。
...
\end{tikzpicture}
\caption[流水线并行微批次时空调度]{流水线并行微批次时空调度。三个均衡阶段处理四个微批次的前向时序，微批次间的数据传递形成流水线启动与排空空隙。}\label{fig:dist-pipeline}
\end{figure}
...
\begin{warning}[长上下文并行的局部 Softmax 归一化陷阱]
若将序列切分到不同设备执行上下文并行，切忌在各设备局部独立计算 Softmax 并直接归约输出。由于各局部块的最大值 $a_i$ 与归一化分母 $l_i$ 互不相同，必须通过在线 Softmax 状态更新算法（式\eqref{eq:dist-block-softmax}）同步最大值并重标度中间指数和，否则将导致注意力输出分布失真。
\end{warning}
...
\begin{align}
 ...
 \vect{o} &= \conste{}^{a_1-a}\vect{o}_1+\conste{}^{a_2-a}\vect{o}_2, \nonumber \\
 \vect{y} &= \frac{\vect{o}}{l}. \label{eq:dist-block-softmax}
\end{align}
这是把相同全局指数和分块后重组的恒等式，为跨块精确注意力提供数学基础。实际系统仍须处理因果掩码、全掩码块、反向梯度和数据移动；全掩码块应作为零贡献显式处理，避免无穷值相减。

\term{专家并行}{Expert Parallelism}{EP}将 MoE 专家分布在不同成员上，按路由结果分发词元，在专家计算后把输出送回并加权组合。若 $N$ 个词元各选择 $k$ 个专家，隐藏维数为 $d$，每元素 $b_e$ 字节，则一次分发与返回的逻辑载荷约为 $2Nkdb_e$，另有索引、路由权重和容量填充等开销。该式包含本地路由数据；每张网卡的实际流量需根据专家放置、跨节点路由比例和通信算法计算。

当少数专家接收过多词元时，最大负载决定阶段耗时。增加专家设备数可能降低每成员参数量，却增加路由通信及小矩阵计算。路由损失和容量策略见\bookxref{ch:moe}；本章关注这些选择在分布式执行中的负载后果。
...
\end{tikzpicture}
\caption[混合并行多维逻辑网格拓扑]{混合并行多维逻辑网格拓扑。以八设备为例，展示数据并行、流水线并行与张量并行的三维正交划分与通信映射关系。}\label{fig:dist-grid}
\end{figure}

层内高频通信宜优先映射到适合其带宽和时延需求的互连域，具体放置由通信成本决定。参数分片、专家流量或大规模上下文交换也可能跨节点；关键路径和超售影响须借助\bookxref{ch:compute-infrastructure}的链路约束计算。

表\ref{tab:distributed-paradigms}比较了各并行方式的切分对象、通信原语与互连要求。大规模训练可组合数据、张量和流水并行，并按状态容量与序列长度选择 ZeRO 或上下文并行；组合时需协调进程组、通信调度和内存布局。

\begin{table}[htbp]
\centering
\caption{分布式并行范式与通信特征对比}\label{tab:distributed-paradigms}
...
若在 $8$ 台设备上完成基准工作量耗时 \qty{100}{\minute}，在 $16$ 台设备上耗时 \qty{60}{\minute}，则相对加速比为 $\frac{100}{60}=\frac{5}{3}$，并行强扩展效率为 $\frac{5/3}{16/8}\approx 83.3\%$。若模型规模超出单设备显存容量，应使用可稳定容纳该架构的最小设备拓扑作为基准节点数 $D_0$，并明确标注相对加速比的计算原点。基准报告应完整呈现有效监督词元吞吐（Tokens/s）、实际序列长度分布、参数更新步数、数值精度类型、物理显存峰值及故障恢复耗时。
