...
\begin{solution}
令 $\mat{Y}=\mat{W}\mat{X}$，$\mat{W}\in\mathbb R^{d_o\times d_i}$、$\mat{X}\in\mathbb R^{d_i\times n}$、上游 $\mat{G}\in\mathbb R^{d_o\times n}$。前向约 $2d_od_in$ FLOP；$\nabla_{\mat{W}}L=\mat{G}\mat{X}^{\mathrm{T}}$ 形状 $d_o\times d_i$，$\nabla_{\mat{X}}L=\mat{W}^{\mathrm{T}}\mat{G}$ 形状 $d_i\times n$，二者各约同样操作量。冻结 $\mat{W}$ 可省其参数梯度及状态，却不能在前面有可训练层时省去 $\nabla_{\mat{X}}L$。
\end{solution}
\end{exercise}
