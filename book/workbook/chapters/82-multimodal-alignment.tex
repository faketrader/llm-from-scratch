...
\begin{exerciselist}
\exerciseitem 推导不重叠块投影与卷积等价所需的权重排列，说明增加补齐、重叠或不同步幅后哪些关系改变。
...
\exerciseitem 一幅图像两边都放大1.5倍，块大小不变且尺寸仍可整除，推导词元数和完整自注意力位置对数的比例。
...
\exerciseitem 从按行、按列交叉熵分别推导式\tbobject{eq:mm-logit-grad}，说明 $R$ 和 $C$ 一般不相同。
...
\exerciseitem 将双样本例题温度改为0.5，重新计算损失、逻辑值梯度和对数缩放梯度；比较温度对概率和梯度链的影响。
...
\exerciseitem 推导向量归一化雅可比，验证其输出与单位向量正交，说明零向量处为何需要另行约定。
...
\exerciseitem\optional 构造一图多文本的正对集合，比较均匀多正对交叉熵与正对总概率损失的梯度方向。
...
\exerciseitem 在重采样例题中将查询改为0，计算输出及值梯度，并解释此时为何表现为平均但一般重采样并非平均。
...
\exerciseitem\optional 对 $M<N_v$ 的固定注意力矩阵构造一个非零零空间扰动，说明固定映射的信息损失与非线性重采样器整体能力的区别。
...
\exerciseitem 根据表\tbobject{tab:mm-labels}写出输出逻辑值与目标的三个配对，解释把提示位置输出全部删除会丢失哪一个答案监督。
...
\exerciseitem 语言主干被冻结、连接器可训练时，画出必要梯度路径；说明在语言主干周围整体停止自动微分为何会改变训练。
...
\exerciseitem\optional 对3秒、16000样本/秒的音频，采用400样本窗口与160样本步长且不补齐，计算窗口数，再说明中心补齐为何改变结果。
...
\exerciseitem\optional 构造持续0.2秒的事件，在0.8秒均匀采样间隔和随机相位假设下计算命中概率，并说明它与视觉识别正确率的关系。
...
\exerciseitem\optional 用有偏移和漂移的两台设备设计时间戳校正，给出窗口代表时间和实际可用时刻的区别。
...
\exerciseitem 为视觉计数或声画匹配任务设计受控样本对、缺失模态对照和任务指标，说明仅验证输出变化为何不充分。
