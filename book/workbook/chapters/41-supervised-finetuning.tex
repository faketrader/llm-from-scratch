...
\begin{exerciselist}
\exerciseitem 从联合概率分解推导全序列损失与回答条件损失的差异。给出一个有理由监督提示的任务，并说明代价。
...
\exerciseitem 在本章移位算例中，将回答扩为三个词元并在尾部共设置两位填充，列出 $r_j$、$m_t$ 与全部有效预测配对。
...
\exerciseitem 若 PAD 与 EOT 使用同一词元编号，设计一个不会误删真实终止监督的位置判定规则。

\begin{workbookanswers}
由结构化消息跨度和实际有效长度判定角色，而不是按token ID过滤。真实消息结束位置即便编号等于PAD，也保留 $r=1$；长度之外填充 $r=0$。再以 $m_t=r_{t+1}$ 构造监督，并结合注意力可见性，避免删除全部同编号词元。
\end{workbookanswers}

\exerciseitem 两个样本的回答长度为4与12、平均损失为2与1，分别计算词元等权、样本等权与序列损失，并解释梯度权重。
...
\exerciseitem 对三轮对话展开全部前缀，并在每个前缀监督所有助手轮次，求各轮被计权的次数；给出每轮恰好一次的构造方案。

\begin{workbookanswers}
三个累积前缀分别包含助手轮1、轮1至2、轮1至3，若每个前缀监督全部助手，轮1出现3次、轮2出现2次、轮3出现1次。方案一仅用完整三轮序列并监督三轮；方案二用三个前缀但各只监督新增末轮。若按样本均值再归约，仍需核对各轮有效长度造成的权重差异。
\end{workbookanswers}

\exerciseitem 利用式\tbobject{eq:sft-context-gradient}解释提示位置标签被忽略时，其嵌入参数为什么仍可能更新。
...
\exerciseitem 候选正确率为0.7，过滤器正确接受率为0.8，错误接受率为0.1，计算保留率和保留集合正确率，并讨论误差相关性的影响。
...
\exerciseitem\optional 设计多轮数据的一致性检查维度，至少包含一次用户修正、一次必要澄清和一次工具返回。
...
\exerciseitem 举例说明回答字符串匹配为何不能可靠确定监督跨度，并给出结构化替代方案。

\begin{workbookanswers}
用户问题可以引用与回答相同的句子，助手也可能多次重复它；字符串首次匹配会选中提示。模板还可能插入控制标记，子词边界也不等于字符边界。应从结构化消息和生成区间记录目标跨度，映射到词元索引，并验证模板渲染一致及停止标记归属。
\end{workbookanswers}

\exerciseitem 构造两组方向相同和相反的任务梯度，用式\tbobject{eq:sft-forgetting}比较其局部保持趋势。
