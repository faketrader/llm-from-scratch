...
\begin{exerciselist}
\exerciseitem\optional 某 SM 具有65536个寄存器、64 KiB共享存储、2048线程上限与16块上限。每块256线程，每线程64寄存器，每块24 KiB共享存储。忽略分配粒度，求驻留块数上界；将共享存储改为16 KiB后，哪个约束成为主导？
...
\exerciseitem 对 $X\in\mathbb R^{B\times4096}$、$W\in\mathbb R^{4096\times4096}$，每元素两字节，求理想算术强度达到50 FLOP/byte所需的最小整数 $B$。解释为何这不足以保证达到峰值算力。
...
\exerciseitem 一项算子包含 $4\times10^{12}$ FLOP，跨 HBM 传输 $10^{11}$ 字节。峰值为 $2\times10^{14}$ FLOP/s，带宽为 $2\times10^{12}$ byte/s。求时间下界，并比较操作量减半与流量减半的效果。

\begin{workbookanswers}
计算下界0.02秒，搬运下界0.05秒，可充分重叠时总下界为0.05秒。只将操作量减半，计算项变0.01秒，总下界不变；只将流量减半，搬运项0.025秒，总下界降到0.025秒。不能据此保证真实速度翻倍，因为原来是否重叠及其他开销尚未知。
\end{workbookanswers}

\exerciseitem CPU 绑定到 NUMA 0，而输入页已经在 NUMA 1 分配，GPU位于 NUMA 0。说明仅更改线程亲和性为何不足，并画出可能的数据路径。
...
\exerciseitem 读取、整理、传输和计算分别需要10、20、8、16毫秒。求串行批次时间与理想稳定流水间隔；若整理与读取共享瓶颈资源，说明原流水估计应怎样修正。

\begin{workbookanswers}
串行时间 $10+20+8+16=54$ 毫秒。独立资源、足够缓冲下稳定间隔为20毫秒，首批仍约54毫秒。读取和整理若争用同一不可重叠资源，应合成30毫秒阶段，间隔至少30毫秒；若只部分争用，应按共享资源占用率求界，而非继续取各阶段最大值。
\end{workbookanswers}

\exerciseitem 使用式\tbobject{eq:infra-ring}，比较 $p=8$ 与 $p=64$ 在相同消息和链路参数下的启动项与传输项。为什么全局设备数增加不代表一次集合通信变快？
...
\exerciseitem\optional 给出一个端口总带宽很高、跨割带宽却很低的交换拓扑。说明专家全互换为什么可能比局部邻近通信更容易暴露此问题。
...
\exerciseitem 一个存储路径支持1 GiB/s，随机请求能力为每秒5000次。分别计算4 KiB与1 MiB请求的理论有效带宽上界，并说明增加并发为何不能无限提高吞吐。
...
\exerciseitem 将本章检查点写入窗口从120秒改为90秒，重新计算集群及每节点所需带宽。若允许分层暂存，分别指出前台暂停和远端持久化的完成条件。
...
\exerciseitem 为跨64个分片的检查点设计提交清单，说明如何处理第63个分片完成后进程失效，以及恢复时发现一个分片摘要不匹配的情形。
...
\exerciseitem\optional 从式\tbobject{eq:infra-checkpoint-loss}推导最优间隔。若作业平均故障间隔缩短到原来的四分之一，最优间隔怎样变化？哪些实际条件会使该近似失效？
...
\exerciseitem\optional 某训练作业设备忙碌率提高，但单位有效词元能耗也提高。至少给出三种机制，并说明需补充哪些测量才能区分它们。
...
\exerciseitem\optional 为一个国产加速器迁移任务列出兼容矩阵的最小字段，明确“模型加载成功”“算子正确”“通信可用”“端到端达标”各自能够支持什么结论。
