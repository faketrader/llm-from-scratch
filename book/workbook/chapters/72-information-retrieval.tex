...
\begin{exercise} 对式\tbobject{eq:ir-bm25}计算词频因子的两个导数，分析 $b=0$ 与 $b=1$ 的含义。
...
\begin{solution}
点积分数$s_j=\vect{u}^{\mathrm{T}}v_j$，$\mathcal L=-\frac{s_+}{\tau}+\log\sum_j \conste{}^{\frac{s_j}{\tau}}$，故 $\nabla_{\vect{u}}\mathcal L=\frac{\sum_jp_jv_j-\vect{v}_+}{\tau}$。高分假负例具有较大$p_j$，梯度下降将查询推离它，损害真实相关邻域。应识别多正例、控制负例污染；不能仅以训练损失下降断言召回更好。
\end{solution}
\end{exercise}
...
\begin{solution}
取查询$\vect{q}=(1,0)$，文档$\vect{a}=(10,10),\vect{b}=(0.9,0)$。内积10与0.9使a优先；余弦$\frac{1}{\sqrt2}$与1使b优先；到q的距离分别$\sqrt{181}$与0.1也使b优先。若都单位化，余弦和内积等价且平方欧氏为$2-2\vect{q}^{\mathrm{T}}\vect{d}$，上述分歧依赖范数未统一。
\end{solution}
\end{exercise}
...
\begin{solution}
令重建误差$\vect{e}=\hat{\vect{x}}-\vect{x}$，平方距离差为 $\|\vect{q}-\hat{\vect{x}}\|^2-\|\vect{q}-\vect{x}\|^2=-2(\vect{q}-\vect{x})^{\mathrm{T}}\vect{e}+\|\vect{e}\|^2$，绝对值不超过 $2\|\vect{q}-\vect{x}\|\|\vect{e}\|+\|\vect{e}\|^2$。若A和B各有误差界$\epsilon_A,\epsilon_B$，原距离间隔大于二者和可保持排序，否则不能保证。非平方距离还满足反三角不等式界$|\|\vect{q}-\hat{\vect{x}}\|-\|\vect{q}-\vect{x}\||\le\|\vect{e}\|$。
\end{solution}
\end{exercise}
