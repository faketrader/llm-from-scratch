...
\begin{exerciselist}
\exerciseitem 为带表格、版本日期和访问限制的文档设计片段记录，说明哪些上下文必须随片段保留。
...
\exerciseitem 从二元条件独立假设推导词项对数证据，并指出它与完整 BM25 公式之间还需要哪些建模选择。
...
\exerciseitem 对式\tbobject{eq:ch32-bm25}计算词频因子的两个导数，分析 $b=0$ 与 $b=1$ 的含义。
...
\exerciseitem 改变评分例题中文档 B 的长度，求使其得分高于 A 的一个长度，并解释其他条件是否保持一致。
...
\exerciseitem 推导对比损失对查询向量的梯度；如果某个高分负例其实相关，梯度会造成什么后果？
...
\exerciseitem 构造两个未归一化文档向量，使内积、余弦和欧氏距离产生至少两种不同排名。
...
\exerciseitem\optional 解释为什么 HNSW 增加查询探索宽度可能提高召回，却不能对所有过滤条件保证精确结果。

\begin{workbookanswers}
更宽探索可访问更多候选、减少局部贪心遗漏，但有限预算和图连通性仍限制结果。权限过滤可能断开原图或消除近邻桥梁，先截断再过滤也可能返回不足K项。应与同权限集合内的精确搜索比较，必要时过滤感知搜索或小集合精确回退；探索宽度不是普适精确性证明。
\end{workbookanswers}

\exerciseitem\optional 给定 $h=768,m=48,k=256$，计算每条 PQ 码的字节数及全部子码本中心的元素数，不含原始精排向量。
...
\exerciseitem\optional 推导 PQ 距离误差界，并说明候选间距很小时为何容易改变排名。
...
\exerciseitem 使用本章两路排名，分别取 $c=1$ 和 $c=60$ 计算 RRF，比较顶部差异的影响。
...
\exerciseitem 为前四名等级 $(1,0,3,2)$ 计算 DCG 与 nDCG，并与完整算例比较 Recall、MRR 的不同关注点。
...
\exerciseitem 设计一个 ANN 召回很高但任务召回很低的例子，说明应改索引还是改表示。
