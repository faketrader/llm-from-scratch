...
\begin{exercises}
\begin{exercise} 构造三副本、每副本两个不等长微批次的计数和梯度，比较三种归一化：微批均值平均、副本均值平均、全局词元平均。
...
\begin{exercise} 在采用平均归约的通信组中，证明式\tbobject{eq:dist-average-correction}；若梯度已被框架额外除以累积步数，应如何修正？
...
\begin{exercise} 用反例说明先裁剪局部梯度再归约不等于先归约再裁剪。
...
\begin{exercise} 在重计算近似模型中令 $L=100$、每激活单位20 MiB，求最优整数区间及近似存储，并列出未计入的内存项。
...
\begin{exercise} 推导环式 Reduce-Scatter 的轮次和每成员发送量，再说明如何与 All-Gather 组成式\tbobject{eq:dist-ring}。
...
\begin{exercise} 对状态算例把分片组改为4，重新计算各阶段持久状态并讨论峰值可行性。
...
\begin{exercise} 为张量并行例题计算各权重片梯度，并验证拼接后与完整矩阵微分一致。

\begin{solution}
沿用上游梯度1，所有ReLU输入为正。$\nabla_{\mat{V}_1}L=(1,2)^{\mathrm{T}}$，$\nabla_{\mat{V}_2}L=(3,1)^{\mathrm{T}}$；$\nabla_{\mat{W}_1}L=\mat{X}^{\mathrm{T}}(1,2)=\begin{pmatrix}1&2\\2&4\end{pmatrix}$，$\nabla_{\mat{W}_2}L=\mat{X}^{\mathrm{T}}(-1,1)=\begin{pmatrix}-1&1\\-2&2\end{pmatrix}$。按原列/行布局拼接等于完整矩阵微分，输入梯度为两成员贡献 $(1,2)+(-2,0)=(-1,2)$。
\end{solution}
\end{exercise}

\begin{exercise} 证明非线性一般不能跨越部分和归约，指出式\tbobject{eq:dist-paired-mlp}为什么允许本地执行非线性。
...
\begin{exercise} 流水阶段数为8时，需要多少微批次才能使理想气泡比例不超过10\%？讨论该选择的显存代价。
...
\begin{exercise}\optional 由两个块的 Softmax 合并式推导三个块的组合，并解释全掩码块的处理。

\begin{solution}
对块i保存 $(m_i,\ell_i,u_i)$，令 $m=\max_i m_i$，则 $\ell=\sum_{i=1}^3\conste{}^{m_i-m}\ell_i$、$u=\sum_{i=1}^3\conste{}^{m_i-m}u_i$，输出 $\frac{u}{\ell}$。精确算术下均等于完整指数和，故可先合并任意两块。全掩码块用空状态跳过；若三块全空则进入明确的全掩码行规则，不计算负无穷之差。
\end{solution}
\end{exercise}

\begin{exercise}\optional 给出64设备的合法逻辑网格，明确不同通信组与有效词元计数范围。
...
\begin{exercise} 设计改变数据并行度后的续训声明，区分资产完整、目标一致、数值接近和逐位复现。
