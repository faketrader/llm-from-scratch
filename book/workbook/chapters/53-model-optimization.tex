...
\begin{exercise}\optional 从约束二次目标推导式\tbobject{eq:ch26-gptqcomp}，说明阻尼改变了什么，以及输入完全不相关时为何不发生跨坐标补偿。
...
\begin{exercise} 用式\tbobject{eq:ch26-scaling}说明 AWQ 与 SmoothQuant 的共同基础，再分别写出二者关注的误差对象。
...
\begin{exercise} 从式\tbobject{eq:ch26-dot}推导对称权重量化可省略的修正项，并讨论分组尺度为何改变累加组织。
...
\begin{exercise}\optional 对 $H=\begin{bmatrix}3&1\\1&2\end{bmatrix}$、$w=(1,1)^\mathsf T$，计算分别删除两个权重时的二阶补偿与代价。
