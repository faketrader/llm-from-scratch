...
\begin{exerciselist}
\exerciseitem 对目标范围 $[-0.7,1.8]$ 与无符号三比特码计算初始尺度、取整零点及实际恢复端点，说明端点为何发生偏移。
...
\exerciseitem 推导均匀误差模型的 $\frac{s^2}{12}$，并构造一个全部输入恰在格点上的反例，说明其为何不是恒等式。
...
\exerciseitem 比较取偶数最近舍入与向零截断在正负半格点附近的误差。为一个可复现制品写出完整舍入约定。
...
\exerciseitem 对本章异尺度矩阵计算逐张量和逐行的整体均方误差，再解释为何误差改善不能直接推出任务改善。
...
\exerciseitem 给定 $4096\times4096$ 权重、四比特码、每行每 $128$ 个元素一组、每组四字节元数据，计算忽略对齐的存储与相对十六位基线压缩率。
...
\exerciseitem\optional 从约束二次目标推导式\tbobject{eq:ch26-gptqcomp}，说明阻尼改变了什么，以及输入完全不相关时为何不发生跨坐标补偿。

\begin{workbookanswers}
最小化 $\frac12\delta^\mathsf TH\delta$ 且 $e_i^\mathsf T\delta=d$。拉格朗日式导数 $H\delta+\lambda e_i=0$，得到 $\delta=\frac{dH^{-1}e_i}{(H^{-1})_{ii}}$，最小代价 $\frac{d^2}{[2(H^{-1})_{ii}]}$。阻尼把H换H+lambda I，改变目标及不可观测方向的代价；若H对角，各非i坐标补偿为0。实际X不相关要对应零交叉二阶矩而非口头“不同特征”。
\end{workbookanswers}

\exerciseitem\optional 若 GPTQ 先处理另一坐标，离散结果是否必然相同？用两维例子或明确论证说明处理顺序的作用。
...
\exerciseitem 用式\tbobject{eq:ch26-scaling}说明 AWQ 与 SmoothQuant 的共同基础，再分别写出二者关注的误差对象。
...
\exerciseitem 从式\tbobject{eq:ch26-dot}推导对称权重量化可省略的修正项，并讨论分组尺度为何改变累加组织。
...
\exerciseitem 构造一个模型权重减少四倍但请求时延改善很小的负载，按带宽、计算、缓存和其他时间解释原因。
...
\exerciseitem 对键值量化的一阶误差式说明：为何只测值缓存的逐元素均方误差不能覆盖注意力分配变化？

\begin{workbookanswers}
$o=\mathrm{softmax}(qK^\mathsf T)V$，一阶有 $\delta o\approx J_{\mathrm{softmax}}(s)\,\delta s\,V+p\,\delta V$，其中 $\delta s$含键误差与查询的内积。只测值MSE覆盖第二项，漏掉键导致的注意力重分配及两误差相关性；不同查询/上下文还可放大相同逐元素误差。
\end{workbookanswers}

\exerciseitem\optional 证明在保留数值不变、仅固定非零数量的条件下，幅值剪枝最小化权重平方误差；给出不最小化任务损失的反例。
...
\exerciseitem 使用式\tbobject{eq:compress-sensitivity}分析非驻点时一阶项的作用。为什么接近训练结束也不能未经测量就认定所有 $g_q=0$？
...
\exerciseitem\optional 对 $H=\begin{bmatrix}3&1\\1&2\end{bmatrix}$、$w=(1,1)^\mathsf T$，计算分别删除两个权重时的二阶补偿与代价。
...
\exerciseitem 写出门控前馈层删除中间通道的三个矩阵索引关系，并说明偏置和优化器状态应如何同步。

\begin{workbookanswers}
列向量约定下 $U,G\in\mathbb R^{d_f\times d}$、$D\in\mathbb R^{d\times d_f}$。保留中间集合S时取 $U[S,:],G[S,:],D[:,S]$；上投影/门控偏置取S，输出偏置维度不变。梯度、Adam矩、主权重和分片索引同步裁剪，残差输出d不改；若使用行向量实现应转置对应索引。
\end{workbookanswers}

\exerciseitem 两字节数值配两字节索引时，忽略行指针求稀疏存储低于稠密存储的非零比例阈值。为什么满足它仍不保证加速？

\begin{workbookanswers}
稠密2P字节，非零比例rho的数值+索引为4rho P，节省要求rho<$\frac{1}{2}$。行指针、对齐和稀疏元数据会收紧阈值；不规则访问、缺少匹配内核和低矩阵单元利用率仍可使稀疏计算更慢。
\end{workbookanswers}

\exerciseitem 对奇异值 $(10,4,3,1)$，计算秩1、2、3近似的绝对与相对 Frobenius 误差，讨论给定参数预算能否实现某个误差目标。
...
\exerciseitem\optional 从列空间投影出发复现截断 SVD 的最优性证明，指出截断奇异值重合时唯一性为何可能失效。

\begin{workbookanswers}
任意秩$\le$r近似A的列空间投影P满足 $\|W-A\|_F^2\ge\|(I-P)W\|_F^2$。设 $WW^\mathsf T$特征值为 $\sigma_i^2$，秩r正交投影最多捕获前r个特征值之和，故误差至少谱尾和；取前r奇异向量即达界。若截断处 $\sigma_r=\sigma_{r+1}$，等特征子空间中可选择不同基/子空间而同样最优，矩阵近似可能不唯一。
\end{workbookanswers}

\exerciseitem 对 $m=4096,n=11008$，求减少因子参数所需的秩阈值。分别计算 $r=512$ 与 $r=2048$ 的参数比例。
...
\exerciseitem\optional 对 $W=\operatorname{diag}(4,1)$、$C_x=\operatorname{diag}(1,25)$，比较普通秩1 SVD 与输入加权近似的输出误差。
...
\exerciseitem 推导温度平方缩放后的学生 logit 梯度，并说明高温近似需要中心化 logits 的原因。

\begin{workbookanswers}
$q=\mathrm{softmax}(\frac{z_S}{\tau})$，$p$为冻结教师，$\frac{\partial[\tau^2\mathrm{KL}(p\|q)]}{\partial z_S}=\tau(q-p)$。高温下 $q_i\approx\frac{1}{K}+\frac{z_i-\bar z}{K\tau}$，故梯度近似师生中心化logit差/K。共同平移logits不改变概率，未经中心化的欧氏logit差会惩罚一个不可辨识方向。
\end{workbookanswers}

\exerciseitem 在蒸馏数例中将 $\lambda$ 改为0.8，求损失与学生梯度；若硬标签改为第二类，两项监督如何竞争？
...
\exerciseitem 教师与学生使用不同分词器，列出逐词元 KL 不成立的两个原因，并解释序列蒸馏保留和丢失了哪些信息。

\begin{workbookanswers}
第一，词表事件不同，不能直接把同索引概率当同一类别；第二，文本被分成不同位置数，条件前缀也不同。需要显式词表/跨度映射或改用序列级教师文本。序列蒸馏保留采样出的可读答案与任务结构，但丢失未采样候选的完整概率和不确定性，受教师采样偏差影响。
\end{workbookanswers}

\exerciseitem\optional 对因子变换 $A'=10A,B'=\frac{B}{10}$，说明乘积不变而量化误差可能变化的原因。
...
\exerciseitem 某方案报告权重文件缩小四倍、单层 FLOPs减少两倍。设计能够确认端到端实际收益的最小证据集合，并明确不能从已有两个数字推出什么。
...
\exerciseitem 剪枝、低秩分解和量化分别作用于原始模型时均达到质量要求。说明这些结果能否证明顺序组合后的模型也达标，并设计同时覆盖累计误差、处理顺序与实际部署收益的验收方案。
