...
\begin{exerciselist}
\exerciseitem 提示长度为5、输出长度为4，列出每次前向处理的词元位置、产生的预测与最终缓存长度。
...
\exerciseitem 对提示8、输出4，分别计算全量重算与缓存方案的线性处理位置数和因果合法分数对数。
...
\exerciseitem 设40层、16个 KV 头、键维128、值维64，元素均为2字节，缓存16384词元，计算裸容量。

\begin{workbookanswers}
每词元字节为 $40\times16\times(128+64)\times2=245760$；乘16384得4026531840字节，即3.75 GiB。键值维不相等时不能用 $2d_k$ 替代二者和，分页元数据和预留另计。
\end{workbookanswers}

\exerciseitem 保持32个查询头，比较4个 KV 头与1个 KV 头的容量；解释为什么计算不能简单按容量同比减少。

\begin{workbookanswers}
固定层、长度、键值维及精度，缓存正比KV头数，4头比1头为4倍。32个查询头仍各产生分数和输出，查询投影与前馈也未按KV头数缩减；广播和矩阵布局影响实际带宽，不能把缓存缩减4倍称为全模型计算缩减4倍。
\end{workbookanswers}

\exerciseitem\optional 推导式\tbobject{eq:cache-absorb-key}，并用维度说明 $U_K$ 不能与任意位置旋转交换。

\begin{workbookanswers}
若 $k_j=U_Kc_j$，则 $q_t^\mathsf Tk_j=q_t^\mathsf TU_Kc_j=(U_K^\mathsf Tq_t)^\mathsf Tc_j$。$U_K\in\mathbb R^{d_k\times r_c}$，矩形矩阵不能与任意 $d_k\times d_k$旋转交换；例如 $R_jU_K$ 与 $U_KR_j$ 后者甚至维度不合法。即使方阵也一般不交换，位置旋转须保持其真实分工。
\end{workbookanswers}

\exerciseitem 块大小8、长度19、页表 $(4,1,7)$，求位置17的物理块和偏移及尾部空位。

\begin{workbookanswers}
逻辑位置17在第 $\lfloor\frac{17}{8}\rfloor=2$ 个逻辑块，页表映射为物理块7，偏移1。长度19共分配24槽位，尾部空5槽。物理块编号7不意味着位置编码使用7或57，应保留逻辑时间17。
\end{workbookanswers}

\exerciseitem 四条请求共享16词元前缀、各有5词元后缀、块大小8，计算共享前后块数与尾部碎片。
