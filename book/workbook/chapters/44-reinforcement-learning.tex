...
\begin{exerciselist}
\exerciseitem 为一个“搜索—读取—回答”的工具任务定义状态、观测与动作，说明哪些环境信息不可见。
...
\exerciseitem 将两步算例改为 $\gamma=0.9$，重新计算 $V(s_0)$ 和两个参数梯度，明确外部折扣因子。
...
\exerciseitem\optional 从回报递推推导 Bellman 方程，并说明 $\gamma=1$ 时压缩映射证明失效的原因。
...
\exerciseitem 用三条轨迹枚举证明两步算例中 $\psi$ 的得分函数梯度与直接求导一致。
...
\exerciseitem 将 GAE 数例的 $\lambda$ 分别改为0和1，计算两步优势并解释差异。
...
\exerciseitem 对相同奖励、不同回答长度，比较原始序列回报、$\frac{R}{T}$、折扣回报与每步成本四种目标。
...
\exerciseitem 说明为什么参考策略概率不能替代采样行为概率，以及硬词表截断如何影响支持条件。
