...
\begin{exerciselist}
\exerciseitem 推导在线统计量合并的三块表达，并说明精确算术下合并顺序无关的原因。
...
\exerciseitem 某算子占总延迟20\%，加速10倍，求端到端理论加速上限。

\begin{workbookanswers}
其余80\%不变，总时间比例 $0.8+\frac{0.2}{10}=0.82$，加速 $\frac{1}{0.82}\approx1.2195$。即使该算子无限快也不超过1.25倍；需确认优化没有改变其他时间项或引入额外开销。
\end{workbookanswers}

\exerciseitem 三个形状桶的编译成本分别为2、10、1秒，预计调用次数分别为1000、100、20次，单次节省分别为3、50、10毫秒。忽略缓存维护等额外成本，判断哪些桶值得预编译。
...
\exerciseitem 用 $p=(0.1,0.6,0.3)$、$q=(0.5,0.2,0.3)$ 计算接受率、残差及最终输出质量。

\begin{workbookanswers}
p=(.1,.6,.3)，q=(.5,.2,.3)，条件接受率为(.2,1,1)，接受质量(.1,.2,.3)，总接受.6。正残差(0,.4,0)，拒绝后只出第二词元；加回拒绝质量后为(.1,.6,.3)，恰为p。只有抽到q正支持的词元才计算p/q。
\end{workbookanswers}

\exerciseitem\optional 证明接受率等于1减去总变差距离，并讨论草稿与目标支持集不相同的情况。
...
\exerciseitem 构造温度不同的草稿和目标，说明精确采样为何不要求温度相同，却要求使用实际概率。
...
\exerciseitem\optional 推导窗口截断的输出误差上界，解释它为何不直接提供下游任务质量保证。
...
\exerciseitem 同一负载下，方案 A 每轮草稿生成、目标验证及同步共耗时 \qty{12}{ms}，平均提交3个词元；方案 B 的接受率更高，但每轮耗时 \qty{20}{ms}，平均提交4个词元。按长期平均成本判断应选哪一方案，并说明迁移到高并发服务时还应检查哪些条件。
