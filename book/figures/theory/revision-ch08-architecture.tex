...
\node[box] (esa) at (0,3.1) {多头自注意力\\$Q,K,V\leftarrow C_{\ell-1}$};
\node[box] (eff) at (0,6.1) {逐位置前馈网络\\$d\to f\to d$，ReLU};
\node[box] (dsa) at (7,3.1) {掩码多头自注意力\\$Q,K,V\leftarrow X_{\ell-1}$};
\node[box] (dca) at (7,6.1) {多头交叉注意力\\$Q\leftarrow U_\ell$，$K,V\leftarrow C$};
...
\draw[->,thick] (epos)--node[left,font=\footnotesize] {$C_0$} (esain);
\draw[->,thick] (dpos)--node[right,font=\footnotesize] {$X_0$} (dsain);
\draw[->,thick] (esaln)--(effin);
\draw[->,thick] (dsaln)--node[left,font=\footnotesize] {$U_\ell$} (dcain);
\draw[->,thick] (dcaln)--node[left,font=\footnotesize] {$Z_\ell$} (dffin);
\node[note] (mem) at (0,8.7) {最终编码器记忆\\$C=C_{N_e}$，$[B,S,d]$};
\draw[->,thick] (effln)--(mem);
\draw[->,thick,blue!70!black] (mem.east)--(3.6,8.7)--(3.6,6.1)--node[above,font=\footnotesize] {$K,V$} (dca.west);
\node[note,text width=24mm] at (3.6,9.6) {同一份源记忆\\送入每个解码层};
\node[box] (linear) at (7,12) {线性词表投影\\$[B,T,V_{\mathrm{vocab}}]$};
\node[box] (softmax) at (7,13.2) {Softmax（词表维）};
\node[note] (out) at (7,14.3) {$p(y_t\mid y_{<t},x_{1:S})$\\训练时与 $y_{1:T}$ 对齐};
\draw[->,thick] (dffln)--node[right,font=\footnotesize] {$X_{N_d}$} (linear);
