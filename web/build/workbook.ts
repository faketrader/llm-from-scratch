...
export function parseWorkbook(path: string, chapters: Chapter[]): void {
  ...
  $(".lfs-solution").each((_, node) => {
    const label = $(node).attr("data-question") ?? "";
    const number = Number(label.slice(label.lastIndexOf("-") + 1));
    answers.set(number, $(node).html() ?? "");
  });
  let chapterIndex = -1;
  $(".chapterHead, .lfs-workbook-chapter, .lfs-exercise").each((_, node) => {
    const item = $(node);
    if (item.hasClass("lfs-workbook-chapter")) {
      chapterIndex = Number(item.attr("data-chapter")) - 1;
      return;
    }
    if (!item.hasClass("lfs-exercise") || chapterIndex < 0 || !chapters[chapterIndex]) return;
